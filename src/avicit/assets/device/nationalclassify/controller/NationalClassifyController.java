package avicit.assets.device.nationalclassify.controller;
...
import avicit.assets.device.nationalclassify.service.NationalClassifyService;
...
@Controller
@Scope("prototype")
@RequestMapping("assets/device/nationalclassify/nationalClassifyController")
public class NationalClassifyController implements LoaderConstant {
	private static final Logger LOGGER = LoggerFactory.getLogger(NationalClassifyController.class);

	@Autowired
	private NationalClassifyService nationalClassifyService;
	 ...
	 * @return ModelAndView
	 */
	@RequestMapping(value = "toNationalClassifyManage")
	public ModelAndView toNationalClassifyManage(HttpServletRequest request, HttpServletResponse reponse) {
...
//		    mav.addObject("classifyTree", ReadFile(jsonFilePath));
			...
			List<NationalClassifyTreeDTO> classifyDataList = nationalClassifyService.getNationalClassifyTree("");
		...
		try {
			...
			List<UserCollectTreeDTO> collectDataList = userCollectService.getMyCollectList(user.getId());
			String tempStr = "[";
			if(collectDataList != null && collectDataList.size() > 0){
				for(int i=0; i<collectDataList.size(); i++){
					if((collectDataList.get(i).getName() != null) && !("".equals(collectDataList.get(i).getName()))){
						...
						NationalClassifyDTO dto = nationalClassifyService.queryNationalClassifyByPrimaryKey(collectDataList.get(i).getId());
		...
		mav.setViewName("avicit/assets/device/nationalclassify/NationalClassifyManage");
		request.setAttribute("url", "platform/assets/device/nationalclassify/nationalClassifyController/operation/");
		return mav;
	}
	 ...
	 */
	@RequestMapping(value = "/operation/getNationalClassifysByPage")
	@ResponseBody
	public Map<String, Object> togetNationalClassifyByPage(PageParameter pageParameter, HttpServletRequest request) {
		QueryReqBean<NationalClassifyDTO> queryReqBean = new QueryReqBean<NationalClassifyDTO>();
		...
		if (sidx != null && sord != null && !"".equals(sord) && !"".equals(sidx)) {
			String cloumnName = ComUtil.getColumn(NationalClassifyDTO.class, sidx);
			if (cloumnName != null) {
				oderby = " " + cloumnName + " " + sord;
			}
		}

		NationalClassifyDTO param = null;
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		QueryRespBean<NationalClassifyDTO> result = null;
		if (json != null && !"".equals(json)) {
			param = JsonHelper.getInstance().readValue(json, dateFormat, new TypeReference<NationalClassifyDTO>() {
			});
			queryReqBean.setSearchParams(param);
		}
		try {
			if (!"".equals(keyWord)) {
				result = nationalClassifyService.searchNationalClassifyByPageOr(queryReqBean, oderby);
			} else {
				result = nationalClassifyService.searchNationalClassifyByPage(queryReqBean, oderby);
			}
		} catch (Exception ex) {
			return map;
		}
	...
	public ModelAndView toOpertionPage(@PathVariable String type, @PathVariable String id, HttpServletRequest request)
		...
		if (!"null".equals(id)) {// 编辑窗口或者详细页窗口
			...
			NationalClassifyDTO dto = nationalClassifyService.queryNationalClassifyByPrimaryKey(id);

			mav.addObject("nationalClassifyDTO", dto);
		}
		mav.setViewName("avicit/assets/device/nationalclassify/" + "NationalClassify" + type);
		return mav;
	}
	...
	public ModelAndView toSaveNationalClassifyDTO(HttpServletRequest request) {
		ModelAndView mav = new ModelAndView();
		...
		try {
			...
			NationalClassifyDTO nationalClassifyDTO = JsonHelper.getInstance().readValue(jsonData, dateFormat, new TypeReference<NationalClassifyDTO>() {});
			
			if ("".equals(nationalClassifyDTO.getId())) {// 新增
				//获取当前父节点的最后一个直系子节点
				NationalClassifyDTO lastChildDto = nationalClassifyService.getLastChildNodeByPID(nationalClassifyDTO.getParentid());
				...
				{	//不存在兄弟节点，则编号为+1
					...
					lastChildDto = nationalClassifyService.queryNationalClassifyByPrimaryKey(nationalClassifyDTO.getParentid());
				}
				
				//存储节点数据
				NationalClassifyDTO dto = nationalClassifyService.insertNationalClassify(nationalClassifyDTO);
				...
				nationalClassifyService.updateNationalClassifySensitive(nationalClassifyDTO);
				...
				//创建需要被替换的json
				NationalClassifyDTO oldDto = nationalClassifyService.queryNationalClassifyByPrimaryKey(nationalClassifyDTO.getId());
	...
	public ModelAndView toPasteNationalClassifyDTO(@RequestBody String[] ids, HttpServletRequest request) {
		...
		try {
			...
			//获取当前父节点的最后一个直系子节点
			NationalClassifyDTO lastChildDto = nationalClassifyService.getLastChildNodeByPID(targetId);
			NationalClassifyDTO targetDto = nationalClassifyService.queryNationalClassifyByPrimaryKey(targetId);
			
			//获取待复制包节点及其下属所有节点
			List<NationalClassifyDTO> pasteList = nationalClassifyService.getNodesByPackageNodeId(sourceId);
			...
			if((pasteList != null) && (pasteList.size()>0)){
				...
				for(int i=0; i<pasteList.size(); i++){
					NationalClassifyDTO dto = pasteList.get(i);
					...
					if(i == 0){
						...
						//更新当前节点的id
						dto.setId(nationalClassifyService.GetRandomString(10));
	...
	public ModelAndView toDeleteNationalClassifyDTO(@RequestBody String[] ids, HttpServletRequest request) {
		...
		try {
			...
			for(String id : ids){
				//LOGGER.info(id);
				NationalClassifyDTO oldDto = nationalClassifyService.queryNationalClassifyByPrimaryKey(id);
				String oldNodeStr = "{\"id\":\"" + oldDto.getId() 
									+ "\",\"itemNum\":\"" + oldDto.getItemNum()
									+"\",\"name\":\"" + oldDto.getName()
									+"\",\"pId\":\"" + oldDto.getParentid()
