 ...
 */
function DynElect(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
 ...
 * 初始化操作
 */
DynElect.prototype.init=function(){
	var _self = this;
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getDynElectsByPage.json',
        mtype: 'POST',
        datatype: "json",
        toolbar: [true,'top'],
        ...
        pager: "#dynElectJqGridPager"
    });
    $(_self._jqgridToolbar).append($("#dynElectTableToolbar"));
 ...
 * 添加页面
 */
DynElect.prototype.insert=function(){
	var _self = this;
	avicAjax.ajax({
		url:_self.getUrl()+"checkStatus",
		data : '',
		type : 'get',
		dataType : 'json',
		success : function(r){
			if(r.flag == "success"){
				if (r.status == 0){
                    _self.insertIndex = layer.open({
						...
						content: _self.getUrl()+'Add/null'
					});
				}else{
					layer.alert('上一轮选举活动还未结束!', {
							icon: 7,
							area: ['400px', ''], //宽高
							closeBtn: 0,
							btn: ['关闭'],
 ...
 * 编辑页面
 */
DynElect.prototype.modify=function(){
 ...
 * 详情页面
 */
DynElect.prototype.detail=function(id){
 ...
 * 打开高级查询框
 */
DynElect.prototype.openSearchForm = function(searchDiv){
	var _self = this;
 ...
 */
DynElect.prototype.formValidate=function(form){
	form.validate({
		rules: {
			name:{
                required:true
 ...
 * 保存方法
 */
DynElect.prototype.save=function(form,id){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
DynElect.prototype.del=function(){
	var _self = this;
	var rows = $(_self._datagridId).jqGrid('getGridParam','selarrrow');
	var ids = [];
	var l =rows.length;
 ...
 * 重载数据
 */
DynElect.prototype.reLoad=function(){
 ...
 * 关闭对话框
 */
DynElect.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 后台查询	
 */
DynElect.prototype.searchData =function(){
 ...
 * 快速查询
 */
DynElect.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
DynElect.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
DynElect.prototype.clearData =function(){
 ...
 * 返回主表id
 */
DynElect.prototype.getSelectID=function(){
