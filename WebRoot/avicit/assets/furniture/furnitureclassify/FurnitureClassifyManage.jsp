...
<script type="text/javascript">
    ...
    function onClick(event, treeId, treeNode, clickFlag) {
        ...
        if (isTestDevice == "Y") {
            //是测试设备台账条件下的分类树筛选
            classData = JSON.stringify({isTestDevice: "Y", furnitureCategory: nationClass});
            sessionStorage.setItem("classData", classData);//以session存储分类数据
        }
        else if (isPc == "Y") {
            //是计算机条件下的分类树筛选
            classData = JSON.stringify({isPc: "Y", furnitureCategory: nationClass});
            sessionStorage.setItem("classData", classData);//以session存储分类数据
        }
        else {
            //仅按分类树筛选
            classData = JSON.stringify({furnitureCategory: nationClass});
            sessionStorage.setItem("classData", classData);//以session存储分类数据
        }
        document.getElementById('assetsFurnitureAccountManageIframe').contentWindow.location.reload(true);
    ...
    function pasteNode() {
        ...
        if (!pasteNodes) {
            ...
            $.ajax({
                url: 'assets/furniture/furnitureclassify/furnitureClassifyController/operation/paste',
    ...
    function addNode(e) {
        ...
        $.ajax({
            url: "assets/furniture/furnitureclassify/furnitureClassifyController/operation/save",
    ...
    function deleteNode() {
        ...
        if ((currentNode != null) && (currentNode != undefined)) {
            layer.confirm('确认要删除选中的数据吗?', {icon: 3, title: "提示", area: ['400px', '']}, function (index) {
                ...
                $.ajax({
                    url: 'assets/furniture/furnitureclassify/furnitureClassifyController/operation/delete',
                    data: JSON.stringify(ids),
                    contentType: 'application/json',
                    type: 'post',
                    dataType: 'json',
    ...
    function addCollect(e) {
        document.getElementById('treeMenu').style.display = "none";
        $.ajax({
            url: "assets/furniture/furniturecollect/furnitureCollectController/operation/save",
    ...
    function deleteCollect(e) {
        document.getElementById('treeMenu').style.display = "none";
        $.ajax({
            url: "assets/furniture/furniturecollect/furnitureCollectController/operation/delete",
    ...
    function upDownNode(moveType) {
        ...
        $.ajax({
            url: "assets/furniture/furniturecollect/furnitureCollectController/operation/upDownNode",
            data: {
                sourceId: nodes[0].id,
    ...
    function beforeRename(treeId, treeNode, newName) {
        $.ajax({
            url: "assets/furniture/furniturecollect/furnitureCollectController/operation/editNodeName",
            data: {
                nodeId: treeNode.id,
                newName: newName,
            },
...
<div style='background-color:#F2F2F2; height:965px;'>
    ...
    <div id="left_tree" style="float:left; width: 10.5%; background-color:#FFFFFF; margin-top:5px;">
        <p style="margin-top:5px; height:25px">
            ...
            <img style='position:absolute; top:15px; left: 9%; width:20px;height:20px;'
                 src='avicit/assets/furniture/furnitureclassify/rightmenu/icon042.gif'>
        </p>
    ...
    <!-- 台账列表内容 -->
    <div id="main_frame"
         style='width: 89%; height:954px; background-color:#FFFFFF; float:left; margin-left:5px; margin-top:5px;'>
        <iframe name="assetsFurnitureAccountManageIframe" id="assetsFurnitureAccountManageIframe"
                src="assets/furniture/assetsfurnitureaccount/assetsFurnitureAccountController/toAssetsFurnitureAccountManage"
                frameborder="0" align="left" width="100%" height="100%" scrolling="no">
        </iframe>
    </div>
</div>
...
<div id="treeMenu" style='width: 100px; position: absolute; display: none;'>
    <div id='rMenuDetail'>
        <div style='display: block; width: 100px'>
            ...
            <div style='width: 100px; height:20px; background-color: #DCDCDC' onclick="goAddNode();">&nbsp;&nbsp;新增
            </div>
        </div>
        <div style='display: block; width: 100px'>
            <div style='width: 19px; height:20px; float:left; background-color: #C9C9C9'></div>
        ...
        <div style='display: block; width: 100px'>
            <div style='width: 19px; height:20px; float:left; background-color: #C9C9C9'></div>
            <div style='width: 100px; height:20px; background-color: #DCDCDC' onclick="pasteNode();">&nbsp;&nbsp;粘贴
            </div>
        </div>
        <div style='display: block; width: 100px'>
            <div style='width: 19px; height:20px; float:left; background-color: #C9C9C9'></div>
            <div style='width: 100px; height:20px; background-color: #DCDCDC' onclick="deleteNode();">&nbsp;&nbsp;删除
            </div>
        </div>
        <div style='display: block; width: 100px'>
            <div style='width: 19px; height:20px; float:left; background-color: #C9C9C9'></div>
            <div style='width: 100px; height:20px; background-color: #DCDCDC' onclick="addCollect();">&nbsp;&nbsp;收藏
            </div>
        </div>
        <div style='display: block; width: 100px'>
            <div style='width: 19px; height:20px; float:left; background-color: #C9C9C9'></div>
        ...
        <div style='display: block; width: 100px'>
            <div style='width: 19px; height:20px; float:left; background-color: #C9C9C9'></div>
            <div style='width: 100px; height:20px; background-color: #DCDCDC' onclick="orignPath();">&nbsp;&nbsp;原路径
            </div>
        </div>
        <div style='display: block; width: 100px'>
            <div style='width: 19px; height:20px; float:left; background-color: #C9C9C9'></div>
        ...
        <div style='display: block; width: 100px'>
            <div style='width: 19px; height:20px; float:left; background-color: #C9C9C9'></div>
            <div style='width: 100px; height:20px; background-color: #DCDCDC' onclick="editName();">&nbsp;&nbsp;修改名称
            </div>
        </div>
    </div>
</div>
...
<!-- 编辑指南 -->
<div id="addNodeDiv"
     style='width: 400px; position: absolute; border:2.5px solid #F2F2F2; display: none; background-color:#FFFFFF;'>
    <p style='margin-top:5px; height:25px; font-family:微软雅黑; font-weight:bold; color:#3399FF; margin-left:5px; border-bottom:1px solid #E9E9E9;'>
        添加节点</p>
            ...
            <table class="tableForm" style="border: 0; cellspacing: 1; width: 100%">
                <tr>
                    <td width="50%" style="padding-right: 4%;" align="right">
                        <a onclick="addNode()" class="btn btn-primary form-tool-btn typeb btn-sm" role="button"
                           title="保存">保存</a>
                        <a onclick="hiddenElement('addNodeDiv')" class="btn btn-grey form-tool-btn btn-sm" role="button"
                           title="返回">返回</a>
                    </td>
                </tr>
            </table>
