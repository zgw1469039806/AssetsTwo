...
<div id="main_frame"
     style='width:1680px; height:954px; background-color:#FFFFFF; float:left; margin-left:5px; margin-top:5px;'>
    <!-- 列表页下方Tab页begin -->
    <div class="eform-tab" style="margin-left: 0%; margin-top: 10px;" contenteditable="false">
        <ul class="nav nav-tabs" role="tablist">
            <li role="presentation" onclick="$(window).trigger('resize');" class="presentation active"
                contenteditable="false"><a href="#BasicInfo" aria-controls="BasicInfo" role="tab"
...
<script src="avicit/assets/furniture/assetsfurnitureaccount/js/AssetsFurnitureAccount.js"
        type="text/javascript"></script>
<script type="text/javascript">
    var assetsFurnitureAccount;

    function formatValue(cellvalue, options, rowObject) {
        return '<a href="javascript:void(0);" onclick="assetsFurnitureAccount.detail(\'' + rowObject.id + '\');">' + cellvalue + '</a>';
    }

    function formatDateForHref(cellvalue, options, rowObject) {
        var thisDate = format(cellvalue);
        return '<a href="javascript:void(0);" onclick="assetsFurnitureAccount.detail(\'' + rowObject.id + '\');">' + thisDate + '</a>';
    }
    ...
    $(document).ready(function () {
        var dataGridColModel = [
            ...
            , {label: '家具状态', name: 'furnitureState', width: 150}
            // , {label: '父级家具ID', name: 'parentId', width: 150}
            , {label: '父级家具名称', name: 'parentName', width: 150}
        ];
        ...
        assetsFurnitureAccount = new AssetsFurnitureAccount('assetsFurnitureAccountjqGrid', '${url}', 'searchDialog', 'form', 'assetsFurnitureAccount_keyWord', searchNames, dataGridColModel);
        //添加按钮绑定事件
        $('#assetsFurnitureAccount_insert').bind('click', function () {
            assetsFurnitureAccount.insert();
        });
        //编辑按钮绑定事件
        $('#assetsFurnitureAccount_modify').bind('click', function () {
            assetsFurnitureAccount.modify();
        });
        //删除按钮绑定事件
        $('#assetsFurnitureAccount_del').bind('click', function () {
            assetsFurnitureAccount.del();
        });
        //查询按钮绑定事件
        $('#assetsFurnitureAccount_searchPart').bind('click', function () {
            assetsFurnitureAccount.searchByKeyWord();
        });
        //查询框回车事件
        ...
        $('#assetsFurnitureAccount_searchAll').bind('click', function () {
            assetsFurnitureAccount.openSearchForm(this);
        });
