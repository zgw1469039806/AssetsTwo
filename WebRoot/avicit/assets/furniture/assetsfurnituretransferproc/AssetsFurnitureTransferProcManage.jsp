...
<html>
<head>
    <!-- ControllerPath = "assets/furniture/assetsfurnituretransferproc/assetsFurnitureTransferProcController/toAssetsFurnitureTransferProcManage" -->
    <title>管理</title>
...
<div id="panelnorth"
     data-options="region:'north',height:fixheight(1),onResize:function(a){$('#assetsFurnitureTransferProc').setGridWidth(a);$('#assetsFurnitureTransferProc').setGridHeight(getJgridTableHeight($('#panelnorth')));$(window).trigger('resize');}">
    <div id="toolbar_assetsFurnitureTransferProc" class="toolbar">
        <div class="toolbar-left">
            <sec:accesscontrollist hasPermission="3" domainObject="formdialog_assetsFurnitureTransferProc_button_add"
                                   permissionDes="添加">
                <a id="assetsFurnitureTransferProc_insert" href="javascript:void(0)"
                   class="btn btn-default form-tool-btn btn-sm" role="button" title="添加"><i class="fa fa-plus"></i>
                    添加</a>
            </sec:accesscontrollist>
            <sec:accesscontrollist hasPermission="3" domainObject="formdialog_assetsFurnitureTransferProc_button_edit"
                                   permissionDes="编辑">
                <a id="assetsFurnitureTransferProc_modify" href="javascript:void(0)"
                   class="btn btn-default form-tool-btn btn-sm" style="display:none;" role="button" title="编辑"><i
                        class="fa fa-file-text-o"></i> 编辑</a>
            </sec:accesscontrollist>
            <sec:accesscontrollist hasPermission="3" domainObject="formdialog_assetsFurnitureTransferProc_button_delete"
                                   permissionDes="删除">
                <a id="assetsFurnitureTransferProc_del" href="javascript:void(0)"
        ...
        <div class="toolbar-right">
            ...
            <div class="input-group form-tool-search" style="width:125px">
                <input type="text" name="assetsFurnitureTransferProc_keyWord" id="assetsFurnitureTransferProc_keyWord"
                       style="width:125px;" class="form-control input-sm" placeholder="请输入查询条件">
                <label id="assetsFurnitureTransferProc_searchPart"
                       class="icon icon-search form-tool-searchicon"></label>
            </div>
            <div class="input-group-btn form-tool-searchbtn">
                <a id="assetsFurnitureTransferProc_searchAll" href="javascript:void(0)" class="btn btn-defaul btn-sm"
                   role="button">高级查询 <span class="caret"></span></a>
            </div>
        </div>
    </div>
    <table id="assetsFurnitureTransferProc"></table>
    <div id="assetsFurnitureTransferProcPager"></div>
</div>
<div id="centerpanel"
     data-options="region:'center',split:true,onResize:function(a){$('#assetsFurnitureTransferSub').setGridWidth(a); $('#assetsFurnitureTransferSub').setGridHeight(getJgridTableHeight($('#centerpanel')));$(window).trigger('resize');}">
    <div id="toolbar_assetsFurnitureTransferSub" class="toolbar">
        <div class="toolbar-right">
            <div class="input-group form-tool-search" style="width:125px">
                <input type="text" name="assetsFurnitureTransferSub_keyWord" id="assetsFurnitureTransferSub_keyWord"
                       style="width:125px;" class="form-control input-sm" placeholder="请输入查询条件">
                <label id="assetsFurnitureTransferSub_searchPart" class="icon icon-search form-tool-searchicon"></label>
            </div>
            <div class="input-group-btn form-tool-searchbtn">
                <a id="assetsFurnitureTransferSub_searchAll" href="javascript:void(0)" class="btn btn-defaul btn-sm"
                   role="button">高级查询 <span class="caret"></span></a>
            </div>
        </div>
    </div>
    <table id="assetsFurnitureTransferSub"></table>
    <div id="assetsFurnitureTransferSubPager"></div>
</div>
</body>
<!-- 主表高级查询 -->
    ...
    <form id="form">
        ...
        <table class="form_commonTable">
            ...
            <tr>
                ...
                <td width="15%">
                    <input title="接收人电话" class="form-control input-sm" type="text" name="receiverByTel"
                           id="receiverByTel"/>
                </td>
            ...
            <tr>
                ...
                <th width="10%">家具完好情况:</th>
                <td width="15%">
                    <textarea class="form-control input-sm" rows="3" title="家具完好情况" name="furnitureCondition"
                              id="furnitureCondition"></textarea>
                </td>
                <th width="10%">家具附件和数量:</th>
                <td width="15%">
                    <textarea class="form-control input-sm" rows="3" title="家具附件和数量" name="furnitureAppendix"
                              id="furnitureAppendix"></textarea>
                </td>
                <th width="10%">备注:</th>
                <td width="15%">
...
<script src="avicit/assets/furniture/assetsfurnituretransferproc/js/AssetsFurnitureTransferProc.js"
        type="text/javascript"></script>
<script src="avicit/assets/furniture/assetsfurnituretransferproc/js/AssetsFurnitureTransferSub.js"
        type="text/javascript"></script>
<script type="text/javascript">
    var assetsFurnitureTransferProc;
    var assetsFurnitureTransferSub;

    function formatValue(cellvalue, options, rowObject) {
        return '<a href="javascript:void(0);" onclick="assetsFurnitureTransferProc.detail(\'' + rowObject.id + '\');">' + cellvalue + '</a>';
    }

    function formatDateForHref(cellvalue, options, rowObject) {
        var thisDate = format(cellvalue);
        return '<a href="javascript:void(0);" onclick="assetsFurnitureTransferProc.detail(\'' + rowObject.id + '\');">' + thisDate + '</a>';
    }

    //刷新本页面
    window.bpm_operator_refresh = function () {
        assetsFurnitureTransferProc.reLoad();
    };
    ...
    $(document).ready(function () {
        ...
        $('#assetsFurnitureTransferProc_keyWord').attr('placeholder', '请输入' + searchMainTips[0] + searchMainC);
        $('#assetsFurnitureTransferProc_keyWord').attr('title', '请输入' + searchMainTips[0] + searchMainC);
        ...
        $('#assetsFurnitureTransferSub_keyWord').attr('placeholder', '请输入' + searchSubTips[0] + searchSubC);
        $('#assetsFurnitureTransferSub_keyWord').attr('title', '请输入' + searchSubTips[0] + searchSubC);
        var assetsFurnitureTransferProcGridColModel = [
            ...
            // , {label: '接收人电话', name: 'receiverByTel', width: 150}
            , {label: '接收安装地点ID', name: 'receivePositionId', width: 150}
            , {label: '移交原因', name: 'transferReason', width: 150}
            , {label: '家具完好情况', name: 'furnitureCondition', width: 150}
            , {label: '家具附件和数量', name: 'furnitureAppendix', width: 150}
        ...
        assetsFurnitureTransferProc = new AssetsFurnitureTransferProc('assetsFurnitureTransferProc', '${url}', 'form', assetsFurnitureTransferProcGridColModel, 'searchDialog',
            function (pid) {
                assetsFurnitureTransferSub = new AssetsFurnitureTransferSub('assetsFurnitureTransferSub', '${surl}', "formSub", assetsFurnitureTransferSubGridColModel, 'searchDialogSub', pid, searchSubNames, "assetsFurnitureTransferSub_keyWord");
            },
            function (pid) {
                assetsFurnitureTransferSub.reLoad(pid);
            },
            searchMainNames,
            "assetsFurnitureTransferProc_keyWord");
        //主表操作
        //添加按钮绑定事件
        $('#assetsFurnitureTransferProc_insert').bind('click', function () {
            assetsFurnitureTransferProc.insert();
        });
        //编辑按钮绑定事件
        $('#assetsFurnitureTransferProc_modify').bind('click', function () {
            assetsFurnitureTransferProc.modify();
        });
        //删除按钮绑定事件
        $('#assetsFurnitureTransferProc_del').bind('click', function () {
            assetsFurnitureTransferProc.del();
        });
        //打开高级查询框
        $('#assetsFurnitureTransferProc_searchAll').bind('click', function () {
            assetsFurnitureTransferProc.openSearchForm(this, $('#assetsFurnitureTransferProc'));
        });
        //关键字段查询按钮绑定事件
        $('#assetsFurnitureTransferProc_searchPart').bind('click', function () {
            assetsFurnitureTransferProc.searchByKeyWord();
        });

        //根据流程状态加载数据
        $('#workFlowSelect').bind('change', function () {
            assetsFurnitureTransferProc.initWorkFlow($(this).val());
        });
        //子表操作
        //打开高级查询
        $('#assetsFurnitureTransferSub_searchAll').bind('click', function () {
            assetsFurnitureTransferSub.openSearchForm(this, $('#assetsFurnitureTransferSub'));
        });
        //关键字段查询按钮绑定事件
        $('#assetsFurnitureTransferSub_searchPart').bind('click', function () {
            assetsFurnitureTransferSub.searchByKeyWord();
        });
