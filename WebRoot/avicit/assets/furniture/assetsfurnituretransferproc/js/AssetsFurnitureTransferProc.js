 ...
 */
function AssetsFurnitureTransferProc(datagrid, url, searchForm, dataGridColModel,searchDialog, afterInit, clickRowLoad, searchMainNames,demoMainDept_KeyWord) {
	...
	this._formCode = "furnitureTransfer"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsFurnitureTransferProc.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	var issubinit = false;
	$(_self._datagridId).jqGrid({
				url : _self.getUrl() + 'getAssetsFurnitureTransferProcsByPage.json',
				mtype : 'POST',
				multiselect : true,
				datatype : "json",
				toolbar : [ true, 'top' ],
 ...
 * 添加页面
 */
AssetsFurnitureTransferProc.prototype.insert = function() {
	//添加流程
 ...
 * 编辑页面
 */
AssetsFurnitureTransferProc.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsFurnitureTransferProc.prototype.detail = function(id, value) {
	flowUtils.detail(id);
};
/**
 * 控件校验   规则：表单字段name与rules对象保持一致
 */
AssetsFurnitureTransferProc.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  				  				  				  				  		     			    				   					procName:{
						maxlength: 255
 ...
 * 保存方法
 */
AssetsFurnitureTransferProc.prototype.save=function(form,windowName,dataSub){
	var _self = this;
 ...
 * 删除方法
 */
AssetsFurnitureTransferProc.prototype.del=function(){
    var _self = this;
 ...
 * 重载数据
 */
AssetsFurnitureTransferProc.prototype.reLoad=function(){
    var searchdata = {
		keyWord : null,
 ...
 * 关闭对话框
 */
AssetsFurnitureTransferProc.prototype.closeDialog=function(windowName){
	layer.close(layer.getFrameIndex(windowName));
};
/**
 * 打开高级查询框
 */
AssetsFurnitureTransferProc.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询	
 */
AssetsFurnitureTransferProc.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsFurnitureTransferProc.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsFurnitureTransferProc.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 根据流程状态查询
 */
AssetsFurnitureTransferProc.prototype.initWorkFlow = function(status) {
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsFurnitureTransferProc_modify").show();
		$("#assetsFurnitureTransferProc_del").show();
	}else{
		$("#assetsFurnitureTransferProc_modify").hide();
		$("#assetsFurnitureTransferProc_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsFurnitureTransferProc.prototype.clearData =function(){
