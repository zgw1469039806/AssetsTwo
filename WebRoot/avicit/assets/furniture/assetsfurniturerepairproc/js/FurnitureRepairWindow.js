...
window.onload=function() {
    $("#ASSETS_FURNITURE_REPAIR_SUB_insertBtn").unbind("click");
    $("#ASSETS_FURNITURE_REPAIR_SUB_insertBtn").on("click",function () {
...
function addAssetsRow(rowJson) {
    //获取待移交台账列表中的数据
    var allData = $('#ASSETS_FURNITURE_REPAIR_SUB').jqGrid('getRowData');
debugger;
    //判断待移交台账列表中的数据是否为空
    ...
    $('#ASSETS_FURNITURE_REPAIR_SUB').jqGrid('endEditCell');
    var parameters = {
        rowID: newRowId,
        initdata:rowJson,
        position: "first",
    ...
    $('#ASSETS_FURNITURE_REPAIR_SUB').jqGrid('addRow', parameters);
    $("#ASSETS_FURNITURE_REPAIR_SUB").editRow(newRowIndex, true);
    newRowIndex++;
};
...
DefaultForm.prototype.beforeSubmit = function(data) {
    ...
    var Rowdata = ($('#ASSETS_FURNITURE_REPAIR_SUB').jqGrid('getRowData'))
    if(Rowdata.length<1){
        layer.msg('请选择家具再提交！');
        return false;
    }else{
