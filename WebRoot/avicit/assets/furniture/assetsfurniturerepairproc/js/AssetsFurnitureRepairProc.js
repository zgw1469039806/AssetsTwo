 ...
 */
function AssetsFurnitureRepairProc(datagrid, url, searchForm, dataGridColModel,searchDialog, afterInit, clickRowLoad, searchMainNames,demoMainDept_KeyWord) {
	...
	this._formCode = "furnitureRepair"; /* 电子表单生成manage页面需要将电子表单编码复制到这 */
 ...
 * 初始化操作
 */
AssetsFurnitureRepairProc.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	var issubinit = false;
	$(_self._datagridId).jqGrid({
				url : _self.getUrl() + 'getAssetsFurnitureRepairProcsByPage.json',
				mtype : 'POST',
				multiselect : true,
				datatype : "json",
				toolbar : [ true, 'top' ],
 ...
 * 添加页面
 */
AssetsFurnitureRepairProc.prototype.insert = function() {
	//添加流程
 ...
 * 编辑页面
 */
AssetsFurnitureRepairProc.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsFurnitureRepairProc.prototype.detail = function(id, value) {
	flowUtils.detail(id);
};
/**
 * 控件校验   规则：表单字段name与rules对象保持一致
 */
AssetsFurnitureRepairProc.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  				  				  				  				  		     			    				   					procName:{
						maxlength: 255
 ...
 * 保存方法
 */
AssetsFurnitureRepairProc.prototype.save=function(form,windowName,dataSub){
	var _self = this;
 ...
 * 删除方法
 */
AssetsFurnitureRepairProc.prototype.del=function(){
    var _self = this;
 ...
 * 重载数据
 */
AssetsFurnitureRepairProc.prototype.reLoad=function(){
    var searchdata = {
		keyWord : null,
 ...
 * 关闭对话框
 */
AssetsFurnitureRepairProc.prototype.closeDialog=function(windowName){
	layer.close(layer.getFrameIndex(windowName));
};
/**
 * 打开高级查询框
 */
AssetsFurnitureRepairProc.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询	
 */
AssetsFurnitureRepairProc.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsFurnitureRepairProc.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsFurnitureRepairProc.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 根据流程状态查询
 */
AssetsFurnitureRepairProc.prototype.initWorkFlow = function(status) {
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsFurnitureRepairProc_modify").show();
		$("#assetsFurnitureRepairProc_del").show();
	}else{
		$("#assetsFurnitureRepairProc_modify").hide();
		$("#assetsFurnitureRepairProc_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsFurnitureRepairProc.prototype.clearData =function(){
