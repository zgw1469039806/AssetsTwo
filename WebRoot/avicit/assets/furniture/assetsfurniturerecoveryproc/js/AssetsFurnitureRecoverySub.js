function AssetsFurnitureRecoverySub(datagrid, url, formSub, dataGridColModel,searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload){
 ...
 * 初始化操作
 */
AssetsFurnitureRecoverySub.prototype.init=function(pid){
	var _self = this;
	if(_self.isReload == "true"){
			$(_self._datagridId).jqGrid({
			url: _self.getUrl()+'getAssetsFurnitureRecoverySub',
	    	postData : {pid : _self.pid},
	        mtype: 'POST',
	        datatype: "json",
	        toolbar: [true,'top'],
	        colModel: assetsFurnitureRecoverySubGridColModel, 
	...
	//初始化校验字段
				     ...
				     _self.notnullFiledComment.push("统一编号");
				      ...
				      _self.lengthValidFiledComment.push("家具分类");
				     				    															   					     				      _self.lengthValidFiled.push("furnitureSpec");
				      _self.lengthValidFiledSize.push(50);
				      _self.lengthValidFiledComment.push("家具规格");
				      ...
				      _self.lengthValidFiledComment.push("安装地点ID");
				     				    															   					     				      _self.lengthValidFiled.push("furniturePhoto");
				      _self.lengthValidFiledSize.push(50);
				      _self.lengthValidFiledComment.push("家具照片");
...
AssetsFurnitureRecoverySub.prototype.insert = function(){
	var _self = this;
 ...
 * @param 
 */
AssetsFurnitureRecoverySub.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
AssetsFurnitureRecoverySub.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 长度和非空校验
 */
 AssetsFurnitureRecoverySub.prototype.valid=function(){
	    var _self = this;
	    $(_self._datagridId).jqGrid('endEditCell');
  ...
  * @param pid
  */
 AssetsFurnitureRecoverySub.prototype.save = function(pid){
 	var _self = this;
 	_self.pid = pid;
 	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');	
  ...
  * 删除
  */
AssetsFurnitureRecoverySub.prototype.del = function(){
 	var _self = this;
 	//删除之前结束单元格
 	$(_self._datagridId).jqGrid('endEditCell');
	...
	if(isAddRow && notAddRow){
		...
		$('#cb_assetsFurnitureRecoverySub').click();
		return false;
	}
 	if(l > 0){
 ...
 * 重载数据
 */
AssetsFurnitureRecoverySub.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
	pid = this.pid;
 ...
 * 获取子表数据
 */
AssetsFurnitureRecoverySub.prototype.getSubData=function(){
	var _self = this;
	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 关闭对话框
 */
AssetsFurnitureRecoverySub.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框
 */
AssetsFurnitureRecoverySub.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsFurnitureRecoverySub.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsFurnitureRecoverySub.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsFurnitureRecoverySub.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
AssetsFurnitureRecoverySub.prototype.clearData =function(){
