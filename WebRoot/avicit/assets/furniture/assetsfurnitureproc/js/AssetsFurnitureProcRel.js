function AssetsFurnitureProcRel(datagrid, url, formSub, dataGridColModel,searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload){
 ...
 * 初始化操作
 */
AssetsFurnitureProcRel.prototype.init=function(pid){
	var _self = this;
	if(_self.isReload == "true"){
			$(_self._datagridId).jqGrid({
			url: _self.getUrl()+'getAssetsFurnitureProcRel',
	    	postData : {pid : _self.pid},
	        mtype: 'POST',
	        datatype: "json",
	        toolbar: [true,'top'],
	        colModel: assetsFurnitureProcRelGridColModel, 
...
//禁止时间和日期格式手输
	...
	//初始化校验字段
    																					   					     _self.notnullFiled.push("furId");
				     _self.notnullFiledComment.push("申购单ID");
				     				      _self.lengthValidFiled.push("furId");
				      _self.lengthValidFiledSize.push(50);
				      _self.lengthValidFiledComment.push("申购单ID");
...
AssetsFurnitureProcRel.prototype.insert = function(){
	var _self = this;
 ...
 * @param 
 */
AssetsFurnitureProcRel.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
AssetsFurnitureProcRel.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 长度和非空校验
 */
 AssetsFurnitureProcRel.prototype.valid=function(){
	    var _self = this;
	    $(_self._datagridId).jqGrid('endEditCell');
  ...
  * @param pid
  */
 AssetsFurnitureProcRel.prototype.save = function(pid){
 	var _self = this;
 	_self.pid = pid;
 	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');	
  ...
  * 删除
  */
AssetsFurnitureProcRel.prototype.del = function(){
 	var _self = this;
 	//删除之前结束单元格
 	$(_self._datagridId).jqGrid('endEditCell');
	...
	if(isAddRow && notAddRow){
		...
		$('#cb_assetsFurnitureProcRel').click();
		return false;
	}
 	if(l > 0){
 ...
 * 重载数据
 */
AssetsFurnitureProcRel.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
	pid = this.pid;
 ...
 * 获取子表数据
 */
AssetsFurnitureProcRel.prototype.getSubData=function(){
	var _self = this;
	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 关闭对话框
 */
AssetsFurnitureProcRel.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框
 */
AssetsFurnitureProcRel.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsFurnitureProcRel.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsFurnitureProcRel.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsFurnitureProcRel.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
AssetsFurnitureProcRel.prototype.clearData =function(){
