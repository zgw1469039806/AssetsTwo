function AssetsFurnitureScrapSub(datagrid, url, formSub, dataGridColModel,searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload){
 ...
 * 初始化操作
 */
AssetsFurnitureScrapSub.prototype.init=function(pid){
	var _self = this;
	if(_self.isReload == "true"){
			$(_self._datagridId).jqGrid({
			url: _self.getUrl()+'getAssetsFurnitureScrapSub',
	    	postData : {pid : _self.pid},
	        mtype: 'POST',
	        datatype: "json",
	        toolbar: [true,'top'],
	        colModel: assetsFurnitureScrapSubGridColModel, 
...
AssetsFurnitureScrapSub.prototype.insert = function(){
	var _self = this;
 ...
 * @param 
 */
AssetsFurnitureScrapSub.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
AssetsFurnitureScrapSub.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 长度和非空校验
 */
 AssetsFurnitureScrapSub.prototype.valid=function(){
	    var _self = this;
	    $(_self._datagridId).jqGrid('endEditCell');
  ...
  * @param pid
  */
 AssetsFurnitureScrapSub.prototype.save = function(pid){
 	var _self = this;
 	_self.pid = pid;
 	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');	
  ...
  * 删除
  */
AssetsFurnitureScrapSub.prototype.del = function(){
 	var _self = this;
 	//删除之前结束单元格
 	$(_self._datagridId).jqGrid('endEditCell');
	...
	if(isAddRow && notAddRow){
		...
		$('#cb_assetsFurnitureScrapSub').click();
		return false;
	}
 	if(l > 0){
 ...
 * 重载数据
 */
AssetsFurnitureScrapSub.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
	pid = this.pid;
 ...
 * 获取子表数据
 */
AssetsFurnitureScrapSub.prototype.getSubData=function(){
	var _self = this;
	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 关闭对话框
 */
AssetsFurnitureScrapSub.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框
 */
AssetsFurnitureScrapSub.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsFurnitureScrapSub.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsFurnitureScrapSub.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsFurnitureScrapSub.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
AssetsFurnitureScrapSub.prototype.clearData =function(){
