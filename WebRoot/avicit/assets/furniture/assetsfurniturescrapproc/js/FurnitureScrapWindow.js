...
window.onload=function() {
    $("#ASSETS_FURNITURE_SCRAP_SUB_insertBtn").unbind("click");
    $("#ASSETS_FURNITURE_SCRAP_SUB_insertBtn").on("click",function () {
...
function addAssetsRow(rowJson) {
    //获取待移交台账列表中的数据
    var allData = $('#ASSETS_FURNITURE_SCRAP_SUB').jqGrid('getRowData');

    //判断待移交台账列表中的数据是否为空
    if((allData != null) && (allData != undefined) && (allData.length>0)){
        for(j=0; j<allData.length; j++){
            //判断待移交的设备是否已添加
            if(allData[j].unifiedId == rowJson.UNIFIED_ID){
    ...
    $('#ASSETS_FURNITURE_SCRAP_SUB').jqGrid('endEditCell');
    // var newRowId = newRowStart_ASSETS_FURNITURE_TRANSFER_SUB + newRowIndex_ASSETS_FURNITURE_TRANSFER_SUB;
    var parameters = {
        rowID: newRowId,
        initdata:rowJson,
        position: "first",
    ...
    $('#ASSETS_FURNITURE_SCRAP_SUB').jqGrid('addRow', parameters);
    $("#ASSETS_FURNITURE_SCRAP_SUB").editRow(newRowIndex, true);
    newRowIndex++;
};
    ...
    switch (requestType) {
        case "productModelSelect":
            for(i=0; i<rows.length; i++){
                ...
                rowJson.ORIGINAL_VALUE = row.originalValue;
                rowJson.NET_VALUE = row.netValue;
                rowJson.GUARANTEE_DATE = row.guaranteeDate;
...
DefaultForm.prototype.beforeSubmit = function(data) {
    ...
    var Rowdata = ($('#ASSETS_FURNITURE_SCRAP_SUB').jqGrid('getRowData'))
    if(Rowdata.length<1){
        layer.msg('请选择家具再提交！');
        return false;
    }else{
