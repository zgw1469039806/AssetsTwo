function AssetsFurAcceptanceRel(datagrid, url, formSub, dataGridColModel,searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload){
 ...
 * 初始化操作
 */
AssetsFurAcceptanceRel.prototype.init=function(pid){
	var _self = this;
	if(_self.isReload == "true"){
			$(_self._datagridId).jqGrid({
			url: _self.getUrl()+'getAssetsFurAcceptanceRel',
	    	postData : {pid : _self.pid},
	        mtype: 'POST',
	        datatype: "json",
	        toolbar: [true,'top'],
	        colModel: assetsFurAcceptanceRelGridColModel, 
...
//禁止时间和日期格式手输
	...
	//初始化校验字段
    																					   					     _self.notnullFiled.push("acceptanceId");
				     _self.notnullFiledComment.push("验收单ID");
				      ...
				      _self.lengthValidFiledSize.push(50);
				      _self.lengthValidFiledComment.push("规格");
...
AssetsFurAcceptanceRel.prototype.insert = function(){
	var _self = this;
 ...
 * @param 
 */
AssetsFurAcceptanceRel.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
AssetsFurAcceptanceRel.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 长度和非空校验
 */
 AssetsFurAcceptanceRel.prototype.valid=function(){
	    var _self = this;
	    $(_self._datagridId).jqGrid('endEditCell');
  ...
  * @param pid
  */
 AssetsFurAcceptanceRel.prototype.save = function(pid){
 	var _self = this;
 	_self.pid = pid;
 	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');	
  ...
  * 删除
  */
AssetsFurAcceptanceRel.prototype.del = function(){
 	var _self = this;
 	//删除之前结束单元格
 	$(_self._datagridId).jqGrid('endEditCell');
	...
	if(isAddRow && notAddRow){
		...
		$('#cb_assetsFurAcceptanceRel').click();
		return false;
	}
 	if(l > 0){
 ...
 * 重载数据
 */
AssetsFurAcceptanceRel.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
	pid = this.pid;
 ...
 * 获取子表数据
 */
AssetsFurAcceptanceRel.prototype.getSubData=function(){
	var _self = this;
	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 关闭对话框
 */
AssetsFurAcceptanceRel.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框
 */
AssetsFurAcceptanceRel.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsFurAcceptanceRel.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsFurAcceptanceRel.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsFurAcceptanceRel.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
AssetsFurAcceptanceRel.prototype.clearData =function(){
