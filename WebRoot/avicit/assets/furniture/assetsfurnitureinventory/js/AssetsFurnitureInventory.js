 ...
 */
function AssetsFurnitureInventory(datagrid, url, searchForm, dataGridColModel, searchDialog, afterInit, clickRowLoad, searchMainNames, demoMainDept_KeyWord) {
    ...
    this._formCode = "assetsFurnitureInventory"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsFurnitureInventory.prototype.init = function () {
    var _self = this;
    //初始化流程帮助类
    flowListEditor = new FlowListEditor(this._formCode);
    var issubinit = false;
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getAssetsFurnitureInventorysByPage.json',
        mtype: 'POST',
        multiselect: true,
        datatype: "json",
        toolbar: [true, 'top'],
 ...
 * 添加页面
 */
AssetsFurnitureInventory.prototype.insert = function () {
    //添加流程
    ...
    this.openIndex = layer.open({
        ...
        content: "avicit/assets/furniture/assetsfurnitureinventory/AssetsFurnitureInventoryAdd.jsp"
...
function callBackFn01() {
	layer.closeAll();
	$("#assetsFurnitureInventory").jqGrid('setGridParam', {
		datatype: 'json'
	}).trigger("reloadGrid");
}
 ...
 * 编辑页面
 */
AssetsFurnitureInventory.prototype.modify = function () {
 ...
 * 详情页面
 */
AssetsFurnitureInventory.prototype.detail = function (id, value) {
    flowUtils.detail(id);
};
/**
 * 控件校验   规则：表单字段name与rules对象保持一致
 */
AssetsFurnitureInventory.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            inventoryName: {
                maxlength: 255
 ...
 * 保存方法
 */
AssetsFurnitureInventory.prototype.save = function (form, windowName, dataSub) {
    var _self = this;
 ...
 * 删除方法
 */
AssetsFurnitureInventory.prototype.del = function () {
    var _self = this;
 ...
 * 重载数据
 */
AssetsFurnitureInventory.prototype.reLoad = function () {
    var searchdata = {
        keyWord: null,
 ...
 * 关闭对话框
 */
AssetsFurnitureInventory.prototype.closeDialog = function (windowName) {
    layer.close(layer.getFrameIndex(windowName));
};
/**
 * 打开高级查询框
 */
AssetsFurnitureInventory.prototype.openSearchForm = function (searchDiv, par) {
    var _self = this;
    par = null;
    //if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsFurnitureInventory.prototype.searchData = function () {
 ...
 * 关键字段查询
 */
AssetsFurnitureInventory.prototype.searchByKeyWord = function () {
 ...
 * 隐藏查询框
 */
AssetsFurnitureInventory.prototype.hideSearchForm = function () {
    layer.close(searchDialogindex);
};
/**
 * 根据流程状态查询
 */
AssetsFurnitureInventory.prototype.initWorkFlow = function (status) {
    $('#bpmState').val(status);
    if (status == "start") {
        $("#assetsFurnitureInventory_modify").show();
        $("#assetsFurnitureInventory_del").show();
    } else {
        $("#assetsFurnitureInventory_modify").hide();
        $("#assetsFurnitureInventory_del").hide();
    }
    this.searchData();
};
/**
 * 清空查询条件
 */
AssetsFurnitureInventory.prototype.clearData = function () {
