 ...
 */
function AssetsLabOrder(datagrid, url, searchD, form, keyWordId, searchNames, dataGridColModel) {
    ...
    this._formCode = "labDeviceOrder"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsLabOrder.prototype.init = function () {
    var _self = this;
    /*获取session的筛选数据-begin*/
    var labDeviceOrderData = sessionStorage.getItem("labDeviceOrderData");
    if(sessionStorage.getItem("labDeviceOrderData")!=""){
        sessionStorage.removeItem("labDeviceOrderData");
    }
    var searchdata = {
        keyWord: null,
        param: labDeviceOrderData
    }
    /*获取session的筛选数据-end*/
    ...
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getAssetsLabOrdersByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsLabOrder.prototype.insert = function () {
    //添加流程
 ...
 * 编辑页面
 */
AssetsLabOrder.prototype.modify = function () {
 ...
 * 详情页面
 */
AssetsLabOrder.prototype.detail = function (id, value) {
    flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsLabOrder.prototype.openSearchForm = function (searchBtn, contentWidth, contentHeight) {
    var _self = this;
    var _resizeFunc;
 ...
 */
AssetsLabOrder.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            orderNumber: {
                maxlength: 255
 ...
 * 保存方法
 */
AssetsLabOrder.prototype.save = function (form, windowName, callback) {
    var _self = this;
    avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsLabOrder.prototype.del = function () {
    var _self = this;
 ...
 * 重载数据
 */
AssetsLabOrder.prototype.reLoad = function () {
    var searchdata = {
        keyWord: null,
 ...
 * 关闭对话框
 */
AssetsLabOrder.prototype.closeDialog = function (windowName, msg) {
    layer.close(layer.getFrameIndex(windowName));
    if (msg && msg !== '') {
        layer.msg(msg);
    }
 ...
 * 后台查询
 */
AssetsLabOrder.prototype.searchData = function () {
 ...
 * 关键字段查询
 */
AssetsLabOrder.prototype.searchByKeyWord = function () {
 ...
 * 根据流程状态查询
 */
AssetsLabOrder.prototype.initWorkFlow = function (status) {
    $('#bpmState').val(status);
    if (status == "start") {
        $("#assetsLabOrder_modify").show();
        $("#assetsLabOrder_del").show();
    } else {
        $("#assetsLabOrder_modify").hide();
        $("#assetsLabOrder_del").hide();
    }
    this.searchData();
};
/**
 * 清空查询条件
 */
AssetsLabOrder.prototype.clearData = function () {
