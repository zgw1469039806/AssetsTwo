 ...
 */
function AssetsAttachment(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "assetsAttachment"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsAttachment.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getAssetsAttachmentsByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsAttachment.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
AssetsAttachment.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsAttachment.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsAttachment.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
AssetsAttachment.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  		     			    				   					assetsAttName:{
						maxlength: 255
 ...
 * 保存方法
 */
AssetsAttachment.prototype.save=function(form,windowName,callback){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsAttachment.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
AssetsAttachment.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
AssetsAttachment.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
AssetsAttachment.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsAttachment.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
AssetsAttachment.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsAttachment_modify").show();
		$("#assetsAttachment_del").show();
	}else{
		$("#assetsAttachment_modify").hide();
		$("#assetsAttachment_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsAttachment.prototype.clearData =function(){
