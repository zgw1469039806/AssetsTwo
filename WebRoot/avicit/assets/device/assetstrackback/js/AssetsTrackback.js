 ...
 */
function AssetsTrackback(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "assetsTrackback"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsTrackback.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getAssetsTrackbacksByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsTrackback.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
AssetsTrackback.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsTrackback.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsTrackback.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
AssetsTrackback.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  				  				  				  				  		     			    				   				   meteringId:{
						required:true,
 ...
 * 保存方法
 */
AssetsTrackback.prototype.save=function(form,windowName,callback){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsTrackback.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
AssetsTrackback.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
AssetsTrackback.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
AssetsTrackback.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsTrackback.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
AssetsTrackback.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsTrackback_modify").show();
		$("#assetsTrackback_del").show();
	}else{
		$("#assetsTrackback_modify").hide();
		$("#assetsTrackback_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsTrackback.prototype.clearData =function(){
