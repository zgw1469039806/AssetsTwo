 ...
 *
 */
function DynSdCollecMain(datagrid, url, searchD, form, keyWordId, searchNames, dataGridColModel) {
    if (!datagrid || typeof(datagrid) !== 'string' && datagrid.trim() !== '') {
        throw new Error("datagrid不能为空！");
    }
    var _url = "platform/assets/device/dynsdcollecmain/dynSdCollecMainController/operation/";
    this.getUrl = function () {
        return _url;
    }
    this._formCode = "assetssdequipcollectntc"; //表单的code，启动流程时需要
...
//初始化操作
var param = JSON.stringify({businessstate_:'ended'});
var searchData={keyWord:"",param:param}
DynSdCollecMain.prototype.init = function () {
    var _self = this;
    //初始化流程帮助类
    flowListEditor = new FlowListEditor(this._formCode);
    $(_self._datagridId).jqGrid({
        url: this.getUrl() + 'getDynSdCollecMainsByPage.json',
        mtype: 'POST',
        datatype: "json",
...
//打开高级查询框
DynSdCollecMain.prototype.openSearchForm = function (searchBtn, contentWidth, contentHeight) {
    var _self = this;
    var _resizeFunc;
...
//保存功能
DynSdCollecMain.prototype.save = function (form, windowName, callback) {
    var _self = this;
    avicAjax.ajax({
...
//关闭对话框
DynSdCollecMain.prototype.closeDialog = function (windowName, msg) {
    layer.close(layer.getFrameIndex(windowName));
    if (msg && msg !== '') {
        layer.msg(msg);
    }
...
DynSdCollecMain.prototype.submit = function(){
    var _self = this;
    if(!callBackFn){
        layer.alert('找不到回调函数！', {
            icon: 7,
