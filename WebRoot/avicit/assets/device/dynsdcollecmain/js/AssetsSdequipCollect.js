function AssetsSdequipCollect(datagrid, url, formSub, dataGridColModel, searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload) {
 ...
 * 初始化操作
 */
AssetsSdequipCollect.prototype.init = function (pid) {
    var _self = this;
    if (_self.isReload == "true") {
        $(_self._datagridId).jqGrid({
            url: _self.getUrl() + 'getAssetsSdequipCollect',
            postData: {pid: _self.pid},
            mtype: 'POST',
            datatype: "json",
            toolbar: [true, 'top'],
            colModel: assetsSdequipCollectGridColModel,
...
AssetsSdequipCollect.prototype.insert = function () {
    var _self = this;
 ...
 * @param
 */
AssetsSdequipCollect.prototype.nullvalid = function (data, item, nullfiled, notnullFiledComment) {
    var msg = "";
 ...
 * @param
 */
AssetsSdequipCollect.prototype.lengthvalid = function (data, item, lengthValidFiled, lengthValidFiledComment, lengthValidFiledSize) {
    var msg = "";
    $.each(data, function (i, dataitem) {
 ...
 * 长度和非空校验
 */
AssetsSdequipCollect.prototype.valid = function () {
    var _self = this;
    $(_self._datagridId).jqGrid('endEditCell');
 ...
 * @param pid
 */
/* AssetsSdequipCollect.prototype.save = function(pid){
 	var _self = this;
 	_self.pid = pid;
 	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');
...
function save(form, id) {
    var _self = this;
    avicAjax.ajax({
        url: 'platform/assets/device/assetssdequipcollect/assetsSdequipCollectController/operation/save',
 ...
 * 删除
 */
AssetsSdequipCollect.prototype.del = function () {
    var _self = this;
    //删除之前结束单元格
    $(_self._datagridId).jqGrid('endEditCell');
 ...
 * 重载数据
 */
AssetsSdequipCollect.prototype.reLoad = function (pid) {
    if (pid != undefined) {
        this.pid = pid;
    }
    pid = this.pid;
 ...
 * 打开高级查询框
 */
AssetsSdequipCollect.prototype.openSearchForm = function (searchDiv, par) {
    var _self = this;
    par = null;
    //if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsSdequipCollect.prototype.searchData = function () {
 ...
 * 关键字段查询
 */
AssetsSdequipCollect.prototype.searchByKeyWord = function () {
 ...
 * 隐藏查询框
 */
AssetsSdequipCollect.prototype.hideSearchForm = function () {
    layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
AssetsSdequipCollect.prototype.clearData = function () {
