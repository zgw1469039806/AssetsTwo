 ...
 * formcode
 */
EformFlow.prototype.formcode = "dynSdCollecMain";
...
EformFlow.prototype.start = function (defineId, callback) {
	...
	//子表数据
	$('#assetsSdequipCollect').jqGrid('endEditCell');
	var dataSub = JSON.stringify($('#assetsSdequipCollect').jqGrid('getRowData'));
			...
			if (result.flag == "success") {
				...
				if (files.length != 0) {
					...
					afterUploadEvent = function () {
						...
						$('#assetsSdequipCollect').jqGrid('setGridParam', {datatype: 'json', postData: searchdata}).trigger("reloadGrid");
