...
<script type="text/javascript">
	...
	$(document).ready(function () {
		myModelJson.push('modify');
		myModelJson.push('viewName');
		myModelJson.push('${orignViewName}');
		myModelJson.push('${belongTable}');
		document.getElementById('viewName').value = '${orignViewName}';
		...
		//获取待修改的表头json
		var temp = ${myModelJson};
		for(i=0; i<temp.length; i++){
			myModelJson.push(temp[i]);
		}


		//构建初始的选中字段列表
		var tBody = document.getElementById('selectedTbody');
		for(i=4; i<myModelJson.length; i++){
	...
	function delAllField() {
		for(i=(myModelJson.length-1); i>3; i--){
			if(myModelJson[i].hidden == true){
				continue;
			}
