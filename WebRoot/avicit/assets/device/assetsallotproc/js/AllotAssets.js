function AllotAssets(datagrid, url, formSub, dataGridColModel, searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload) {
 ...
 * 初始化操作
 */
AllotAssets.prototype.init = function (pid) {
    var _self = this;
    if (_self.isReload == "true") {
        $(_self._datagridId).jqGrid({
            url: _self.getUrl() + 'getAllotAssets',
            postData: {pid: _self.pid},
            mtype: 'POST',
            datatype: "json",
            toolbar: [true, 'top'],
            colModel: allotAssetsGridColModel,
    ...
    _self.lengthValidFiled.push("allotProcId");
    _self.lengthValidFiledSize.push(50);
    _self.lengthValidFiledComment.push("调拨流程ID");
...
AllotAssets.prototype.insert = function (rowJson) {
    var _self = this;
 ...
 * @param
 */
AllotAssets.prototype.nullvalid = function (data, item, nullfiled, notnullFiledComment) {
    var msg = "";
 ...
 * @param
 */
AllotAssets.prototype.lengthvalid = function (data, item, lengthValidFiled, lengthValidFiledComment, lengthValidFiledSize) {
    var msg = "";
    $.each(data, function (i, dataitem) {
 ...
 * 长度和非空校验
 */
AllotAssets.prototype.valid = function () {
    var _self = this;
    $(_self._datagridId).jqGrid('endEditCell');
 ...
 * @param pid
 */
AllotAssets.prototype.save = function (pid) {
    var _self = this;
    _self.pid = pid;
    $(_self._datagridId).jqGrid('endEditCell');
    var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 删除
 */
AllotAssets.prototype.del = function () {
    var _self = this;
    //删除之前结束单元格
    $(_self._datagridId).jqGrid('endEditCell');
 ...
 * 重载数据
 */
AllotAssets.prototype.reLoad = function (pid) {
    if (pid != undefined) {
        this.pid = pid;
    }
    pid = this.pid;
 ...
 * 获取子表数据
 */
AllotAssets.prototype.getSubData = function () {
    var _self = this;
    $(_self._datagridId).jqGrid('endEditCell');
    var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 关闭对话框
 */
AllotAssets.prototype.closeDialog = function (id) {
    if (id == "insert") {
 ...
 * 打开高级查询框
 */
AllotAssets.prototype.openSearchForm = function (searchDiv, par) {
    var _self = this;
    par = null;
    //if(!par) par = $(window);
 ...
 * 高级查询
 */
AllotAssets.prototype.searchData = function () {
 ...
 * 关键字段查询
 */
AllotAssets.prototype.searchByKeyWord = function () {
 ...
 * 隐藏查询框
 */
AllotAssets.prototype.hideSearchForm = function () {
 ...
 * 清空查询条件
 */
AllotAssets.prototype.clearData = function () {
...
function addAssetsRow(rowJson) {
    //获取待调拨台账列表中的数据
    var allData = $('#allotAssets').jqGrid('getRowData');
    ...
    $('#allotAssets').jqGrid('addRow', parameters);
    ...
    switch (requestType) {
        case "productModelSelect":
            for(i=0; i<rows.length; i++){
                ...
                rowJson.secretLevelName = row.secretLevel;
                rowJson.isSafetyProduction = row.isSafetyProduction;
                rowJson.isSafetyProductionName = row.isSafetyProduction;
