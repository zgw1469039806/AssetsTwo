 ...
 * formcode
 */
EformFlow.prototype.formcode = "assetsAllotProc";
    ...
    avicAjax.ajax({
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($("#form"), result.assetsAllotProcDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
...
EformFlow.prototype.start = function (defineId, callback) {
	...
	//子表数据
	$('#allotAssets').jqGrid('endEditCell');
	var dataSub = JSON.stringify($('#allotAssets').jqGrid('getRowData'));
			...
			if (result.flag == "success") {
				...
				if (files.length != 0) {
					...
					afterUploadEvent = function () {
						...
						$('#allotAssets').jqGrid('setGridParam', {datatype: 'json', postData: searchdata}).trigger("reloadGrid");
