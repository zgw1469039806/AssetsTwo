 ...
 */
function AssetsUstdtempAcceptanceDetail(form, AcceptanceDeviceComponent) {
    this._formId = "#" + form;
    this.AcceptanceDeviceComponent = AcceptanceDeviceComponent;
...
AssetsUstdtempAcceptanceDetail.prototype.start = function (defineId, callback) {
    ...
    //子表校验
    var msg = _self.AcceptanceDeviceComponent.valid();
    if (msg && msg != "") {
        layer.alert(msg, {
            icon: 7,
            area: ['400px', ''], //宽高
    ...
    //子表数据
    var dataSub = _self.AcceptanceDeviceComponent.getSubData();
    avicAjax.ajax({
            ...
            if (result.flag == "success") {
                ...
                if (files.length != 0) {
                    ...
                    afterUploadEvent = function () {
                        ...
                        //子表刷新,不然更新时会缺少子表ID
                        _self.AcceptanceDeviceComponent.reLoad(result.startResult.formId);
                        // 启动成功后回调流程刷新按钮
            ...
            if (result.flag == "success") {
                ...
                if (files.length != 0) {
                    ...
                    afterUploadEvent = function () {
                        ...
                        layer.close(maskId);
                        //子表刷新,不然更新时会缺少子表ID
                        _self.AcceptanceDeviceComponent.reLoad(result.pid);
                        // 启动成功后回调流程刷新按钮
                        callback();
                        _self.initFormData();
                    };
 ...
 */
AssetsUstdtempAcceptanceDetail.prototype.controlSelfElement = function (tagId, operability, obj) {
    if (operability) {
        if (tagId == "acceptanceDeviceComponent_editable") {//判断是否允编辑子表数据
            //获取colModel数组对象
            var columnArray = $('#acceptanceDeviceComponent').jqGrid('getGridParam', 'colModel');

            //给每个colModel属性列设置可编辑
            $.each(columnArray, function (i, item) {
                $('#acceptanceDeviceComponent').setColProp(item.name, {editable: true});
            });
        } else {
            $("#" + tagId).show();
        }
 ...
 * @param obj 参数对象
 */
AssetsUstdtempAcceptanceDetail.prototype.controlCustomSubTableForAccess = function (tagId, operability, obj) {
 ...
 * @param obj 参数对象
 */
AssetsUstdtempAcceptanceDetail.prototype.controlCustomSubTableForOperability = function (tagId, operability, obj) {
    var tagArr = obj.tag.split("__");
    var subTableName = tagArr[0];
    var subTableId = $("table[name=" + subTableName + "]")[0].id
    var tag = "#" + subTableId;
 ...
 * @param obj 参数对象
 */
AssetsUstdtempAcceptanceDetail.prototype.controlCustomSubTableForRequired = function (tagId, operability, obj) {
    //获取必填字段数组
    var notnullFiledArr = this.AcceptanceDeviceComponent.notnullFiled;
    var notnullFiledCommentArr = this.AcceptanceDeviceComponent.notnullFiledComment;
    var tagValue = tagId;
