...
EformFlow.prototype.start = function (defineId, callback) {
    ...
    //子表校验
    var msg = subValid('acceptanceDeviceComponent');
    if (msg && msg != "") {
        //去掉遮罩
        layer.close(maskId);
	...
	//子表数据
    $('#acceptanceDeviceComponent').jqGrid('endEditCell');
	var dataSub = JSON.stringify($('#acceptanceDeviceComponent').jqGrid('getRowData'));
			...
			if (result.flag == "success") {
				...
				if (files.length != 0) {
					...
					afterUploadEvent = function () {
                        ...
                        $('#acceptanceDeviceComponent').jqGrid('setGridParam', {datatype: 'json', postData: searchdata}).trigger("reloadGrid");
...
$(function () {
    //创建业务操作JS
    var eformFlow = new EformFlow('form');
...
var lengthValidFiled = ["acceptanceId", "deviceId", "parentUnifiedId", "componentName", "componentCategory", "componentModel"];//除时间,数字等类型长度校验字段
this.lengthValidFiledComment = ["验收单ID", "主设备ID", "主设备统一编号", "组件名称", "组件类别", "组件型号"];//除时间,数字等类型长度校验字段注释
...
function insertComponent() {
    ...
    $('#acceptanceDeviceComponent').jqGrid('addRow', parameters);
    newRowIndex++;

    /*将子表的单元格置为可编辑——开始*/
    var columnArray = $('#acceptanceDeviceComponent').jqGrid('getGridParam', 'colModel'); //获取colModel数组对象
...
$('#acceptanceDeviceComponent_insert').bind('click', function () {	//子表添加按钮绑定事件
    insertComponent();
});
