 ...
 * formcode
 */
EformFlow.prototype.formcode = "assetsDeviceUnused";
    ...
    avicAjax.ajax({
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($("#form"), result.assetsDeviceUnusedDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
...
EformFlow.prototype.start = function (defineId, callback) {
	...
	//子表数据
	$('#assetsDeviceUnusedsub').jqGrid('endEditCell');
	var dataSub = JSON.stringify($('#assetsDeviceUnusedsub').jqGrid('getRowData'));

    //判断子表是否有数据，如果没有数据禁止保存提交
    var Rowdata = ($('#assetsDeviceUnusedsub').jqGrid('getRowData'))
    if(Rowdata.length<1){
        layer.msg('请选择设备！');
        var maskId = layer.load();
                ...
                if (result.flag == "success") {
                    ...
                    if (files.length != 0) {
                        ...
                        afterUploadEvent = function () {
                            ...
                            $('#assetsDeviceUnusedsub').jqGrid('setGridParam', {datatype: 'json', postData: searchdata}).trigger("reloadGrid");
