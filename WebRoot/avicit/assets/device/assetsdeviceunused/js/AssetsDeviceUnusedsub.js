function AssetsDeviceUnusedsub(datagrid, url, formSub, dataGridColModel,searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload){
...
/**
 * 计算子表数据某一列的和，并赋值给总闲置金额
 * 通过gridComplete函数，只要子表发生变化就触发这个函数
 */
function TotalunusedMoney(){
	...
	*	这个方法返回一个包含列所对应的值的数组。
	*	参数：
	*	1.colname可以是一个代表列的索引的数字,或者是一个colModel里面的name
	*	2.returntype决定了返回的数组的类型，当设置为false(默认值)时数组只包含值，当设置成true时数组包含一组对象。
	*		对象被定义为{ id:rowid,value:cellvalue } ,其中rowid表示是哪一行的id,cellvalue表示的是这一个单元格的值。
	*		例如，输出可以是这样[{ id:1,value:1 },{ id:2,value:2 }…)]。
	*	3.mathoperation的有效选项是 'sum, 'avg', 'count'
	*		如果这个参数被设置了并且是有效的，就表示该操作的返回值是操作该列中的所有值之后得到的一个标量值。
	*		如果该参数是无效的，那么返回值为空数组。
	* */
    var total = $("#assetsDeviceUnusedsub").getCol('netValue',false,'sum');
    $("#totalUnusedMoney").val(total);	//赋值给总闲置金额
}
 ...
 * 初始化操作
 */
AssetsDeviceUnusedsub.prototype.init=function(pid){
	var _self = this;
	if(_self.isReload == "true"){
			$(_self._datagridId).jqGrid({
			url: _self.getUrl()+'getAssetsDeviceUnusedsub',
	    	postData : {pid : _self.pid},
	        mtype: 'POST',
	        datatype: "json",
	        toolbar: [true,'top'],
	        colModel: assetsDeviceUnusedsubGridColModel, 
            ...
            gridComplete: TotalunusedMoney	//gridComplete 每次加载完数据或者jqGrid中的数据变动都会加载一次里面的函数
	    });
...
AssetsDeviceUnusedsub.prototype.insert = function(){
	var _self = this;
 ...
 * @param 
 */
AssetsDeviceUnusedsub.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
AssetsDeviceUnusedsub.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 长度和非空校验
 */
 AssetsDeviceUnusedsub.prototype.valid=function(){
	    var _self = this;
	    $(_self._datagridId).jqGrid('endEditCell');
  ...
  * @param pid
  */
 AssetsDeviceUnusedsub.prototype.save = function(pid){
 	var _self = this;
 	_self.pid = pid;
 	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');	
  ...
  * 删除
  */
AssetsDeviceUnusedsub.prototype.del = function(){
 	var _self = this;
 	//删除之前结束单元格
 	$(_self._datagridId).jqGrid('endEditCell');
	...
	if(isAddRow && notAddRow){
		...
		$('#cb_assetsDeviceUnusedsub').click();
		return false;
	}
 	if(l > 0){
 ...
 * 重载数据
 */
AssetsDeviceUnusedsub.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
	pid = this.pid;
 ...
 * 获取子表数据
 */
AssetsDeviceUnusedsub.prototype.getSubData=function(){
	var _self = this;
	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 关闭对话框
 */
AssetsDeviceUnusedsub.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框
 */
AssetsDeviceUnusedsub.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsDeviceUnusedsub.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsDeviceUnusedsub.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsDeviceUnusedsub.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
AssetsDeviceUnusedsub.prototype.clearData =function(){
