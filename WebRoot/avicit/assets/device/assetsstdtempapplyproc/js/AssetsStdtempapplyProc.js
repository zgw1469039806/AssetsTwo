 ...
 */
function AssetsStdtempapplyProc(datagrid, url, searchForm, dataGridColModel, searchDialog, afterInit, clickRowLoad, searchMainNames, demoMainDept_KeyWord) {
    ...
    this._formCode = "assetsStdempapply"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsStdtempapplyProc.prototype.init = function () {
    var _self = this;
    //初始化流程帮助类
    flowListEditor = new FlowListEditor(this._formCode);
    var issubinit = false;
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getAssetsStdtempapplyProcsByPage.json',
        mtype: 'POST',
        multiselect: true,
        datatype: "json",
        toolbar: [true, 'top'],
 ...
 * 添加页面
 */
AssetsStdtempapplyProc.prototype.insert = function () {
    //添加流程
 ...
 * 编辑页面
 */
AssetsStdtempapplyProc.prototype.modify = function () {
 ...
 * 详情页面
 */
AssetsStdtempapplyProc.prototype.detail = function (id, value) {
    flowUtils.detail(id);
};
/**
 * 控件校验   规则：表单字段name与rules对象保持一致
 */
AssetsStdtempapplyProc.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            stdId: {
                required: true,
 ...
 * 保存方法
 */
AssetsStdtempapplyProc.prototype.save = function (form, id) {
    var _self = this;
            ...
            if (r.flag == "success") {
                ...
                if (files.length != 0) {
                    subFrame.afterUploadEvent = function () {
                        _self.reLoad();
                        var msg = subFrame.assetsSupplier.valid();
                        if (msg && msg != "") {
                            layer.alert(msg, {
                                icon: 7,
                                area: ['400px', ''], //宽高
                        ...
                        subFrame.assetsSupplier.save(r.pid);
                    };
 ...
 * 删除方法
 */
AssetsStdtempapplyProc.prototype.del = function () {
    var _self = this;
 ...
 * 重载数据
 */
AssetsStdtempapplyProc.prototype.reLoad = function () {
    var searchdata = {
        keyWord: null,
 ...
 * 关闭对话框
 */
AssetsStdtempapplyProc.prototype.closeDialog = function (windowName) {
    layer.close(layer.getFrameIndex(windowName));
};
/**
 * 打开高级查询框
 */
AssetsStdtempapplyProc.prototype.openSearchForm = function (searchDiv, par) {
    var _self = this;
    par = null;
    //if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsStdtempapplyProc.prototype.searchData = function () {
 ...
 * 关键字段查询
 */
AssetsStdtempapplyProc.prototype.searchByKeyWord = function () {
 ...
 * 隐藏查询框
 */
AssetsStdtempapplyProc.prototype.hideSearchForm = function () {
    layer.close(searchDialogindex);
};
/**
 * 根据流程状态查询
 */
AssetsStdtempapplyProc.prototype.initWorkFlow = function (status) {
    $('#bpmState').val(status);
    if (status == "start") {
        $("#assetsStdtempapplyProc_modify").show();
        $("#assetsStdtempapplyProc_del").show();
    } else {
        $("#assetsStdtempapplyProc_modify").hide();
        $("#assetsStdtempapplyProc_del").hide();
    }
    this.searchData();
};
/**
 * 清空查询条件
 */
AssetsStdtempapplyProc.prototype.clearData = function () {
