 ...
 */
function AssetsShiftProc(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "assetsShiftProc"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsShiftProc.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getAssetsShiftProcsByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsShiftProc.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
AssetsShiftProc.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsShiftProc.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsShiftProc.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
AssetsShiftProc.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  		     			    				   				   shiftNo:{
						required:true,
 ...
 * 保存方法
 */
AssetsShiftProc.prototype.save=function(form,windowName,callback){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsShiftProc.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
AssetsShiftProc.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
AssetsShiftProc.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
AssetsShiftProc.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsShiftProc.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
AssetsShiftProc.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsShiftProc_modify").show();
		$("#assetsShiftProc_del").show();
	}else{
		$("#assetsShiftProc_modify").hide();
		$("#assetsShiftProc_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsShiftProc.prototype.clearData =function(){
