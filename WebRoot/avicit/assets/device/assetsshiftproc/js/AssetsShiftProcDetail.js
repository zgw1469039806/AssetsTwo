 ...
 */
function AssetsShiftProcDetail(form) {
    this._formId = "#" + form;
    DefaultForm.call(this);
};
AssetsShiftProcDetail.prototype = new DefaultForm();
//formcode
AssetsShiftProcDetail.prototype.formcode = "assetsShiftProc";
//初始化操作
AssetsShiftProcDetail.prototype.initFormData = function () {
    var _self = this;
    avicAjax.ajax({
        url: 'platform/assets/device/assetsshiftproc/assetsShiftProcController/toDetailJsp.json',
        data: {
            id: _self.id
        },
        type: 'POST',
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($(_self._formId), result.assetsShiftProcDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
...
//启动流程
AssetsShiftProcDetail.prototype.start = function (defineId, callback) {
    var _self = this;
    //表单校验
    ...
    avicAjax.ajax({
        url: 'platform/assets/device/assetsshiftproc/assetsShiftProcController/operation/saveAndStartProcess',
        data: {
...
//更新数据
AssetsShiftProcDetail.prototype.save = function (callback) {
    var _self = this;
    //表单校验
    var isValidate = $("#form").validate();
    if (!isValidate.checkForm()) {
    ...
    var dataVo = JSON.stringify(serializeObject($(_self._formId)));
    avicAjax.ajax({
        url: 'platform/assets/device/assetsshiftproc/assetsShiftProcController/operation/save',
        data: {
            data: dataVo
        },
        type: 'POST',
...
//表单重载json对象数据
AssetsShiftProcDetail.prototype.setForm = function (formObj, jsonValue) {
    var obj = formObj;
...
//控制附件
AssetsShiftProcDetail.prototype.setAttachMagic = function (attachMagic) {
...
//流程控制-多附件必填
AssetsShiftProcDetail.prototype.setAttachRequired = function (tagId, required, obj) {
    eval("this.attachRequiredMap." + tagId + " = " + required);
};

AssetsShiftProcDetail.prototype.attachRequiredMap = {};
AssetsShiftProcDetail.prototype.validAttachRequired = function () {
...
//多附件增删控制
AssetsShiftProcDetail.prototype.setAttachCanAddOrDel = function (tagId, operability, obj) {
    if (operability) {
...
AssetsShiftProcDetail.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            shiftNo: {
                maxlength: 50
