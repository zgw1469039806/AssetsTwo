 ...
 */
function AssetsUserLog(datagrid, url, searchD, form, keyWordId, searchNames, dataGridColModel) {
 ...
 * 初始化操作
 */
AssetsUserLog.prototype.init = function () {
    var _self = this;
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getAssetsUserLogsByPage.json',
        mtype: 'POST',
        datatype: "json",
        toolbar: [true, 'top'],
 ...
 * 添加页面
 */
AssetsUserLog.prototype.insert = function () {
 ...
 * 编辑页面
 */
AssetsUserLog.prototype.modify = function () {
 ...
 * 详情页面
 */
AssetsUserLog.prototype.detail = function (id) {
 ...
 * 打开高级查询框
 */
AssetsUserLog.prototype.openSearchForm = function (searchDiv) {
    var _self = this;
 ...
 */
AssetsUserLog.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            userid: {
                maxlength: 128,
 ...
 * 保存方法
 */
AssetsUserLog.prototype.save = function (form, id) {
    var _self = this;
    avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsUserLog.prototype.del = function () {
    var _self = this;
 ...
 * 重载数据
 */
AssetsUserLog.prototype.reLoad = function () {
 ...
 * 关闭对话框
 */
AssetsUserLog.prototype.closeDialog = function (id) {
    if (id == "insert") {
...
//隐藏查询框
AssetsUserLog.prototype.getSearchDate =function(){
    var searchdata = JSON.stringify(serializeObject($(this._formId)));
    return searchdata;
};
/**
 * 后台查询
 */
AssetsUserLog.prototype.searchData = function () {
...
function exportAll(){
    $.messager.confirm('确认','是否确认导出Excel文件?',function(r) {
        if (r) {
            //封装参数
            var rows= $("#assetsUserLogjqGrid").jqGrid("getRowData");
            var rownum = $("#assetsUserLogjqGrid").jqGrid('getGridParam','colModel')
            // var dataGridFields = JSON.stringify(columnFieldsOptions[0]);
            // varrows = $('#dgUser').datagrid('getRows');
            ...
            if(assetsUserLog.getSearchDate()){
                expSearchParams.param = assetsUserLog.getSearchDate();
            }else{
            ...
            var url = "assets/device/assetsuserlog/assetsUserLogController/exportServer";
 ...
 * 关键字段查询
 */
AssetsUserLog.prototype.searchByKeyWord = function () {
 ...
 * 隐藏查询框
 */
AssetsUserLog.prototype.hideSearchForm = function () {
    layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
AssetsUserLog.prototype.clearData = function () {
