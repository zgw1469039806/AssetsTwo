 ...
 * 提交和退回前是否自动保存
 */
DefaultForm.prototype.isAutoSave = false;
...
FlowEditor.prototype.controlFormInput = function() {
	...
	if(this.flowModel.userIdentityKey == "7" && this.bpmSave.isEnable()){
		...
		$(".panel-body").eq(0).find("textarea").not(".bpm_nosec_class").attr("disabled", "disabled");
		//禁用图标事件
		...
		// 全部显示，包括按钮
		$(".panel-body").eq(0).find("select").not(".bpm_nosec_class").show();
		$(".panel-body").eq(0).find(':checkbox').not(".bpm_nosec_class").not(".bpm_nosec_class :checkbox").show();
		$(".panel-body").eq(0).find(':radio').not(".bpm_nosec_class").show();
        $(".panel-body").eq(0).find(':checkbox').parent("label").not(".bpm_nosec_class").not(".bpm_nosec_class :checkbox").show();
        $(".panel-body").eq(0).find(':radio').parent("label").not(".bpm_nosec_class").show();
		$(".panel-body").eq(0).find(':text').not(".bpm_nosec_class").show();
		$(".panel-body").eq(0).find("textarea").not(".bpm_nosec_class").show();
		//显示图标
		$(".panel-body").eq(0).find(".input-group-addon").not(".bpm_nosec_class").show();
            ...
            if(n.elementType == "bpm_self_class"
				...
				if(colModel!=null && colModel.length>0){
					for(var m=0;m<colModel.length;m++){
						...
						if (colModel[m].name!=colNameName && colModel[m].name!=colName && colModel[m].hasOwnProperty("editable") && colModel[m].editable == true && colModel[m].hidden == false && colModel[m].colmenu == true){
                            hasColEditable = true;
                        }
					}
				}
