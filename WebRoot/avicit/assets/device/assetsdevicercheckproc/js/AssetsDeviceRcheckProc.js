 ...
 */
function AssetsDeviceRcheckProc(datagrid, url, searchForm, dataGridColModel, searchDialog, afterInit, clickRowLoad, searchMainNames, demoMainDept_KeyWord) {
    ...
    this._formCode = "assetsDeviceRcheckProc"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsDeviceRcheckProc.prototype.init = function () {
    var _self = this;
    //初始化流程帮助类
    flowListEditor = new FlowListEditor(this._formCode);
    var issubinit = false;
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getAssetsDeviceRcheckProcsByPage.json',
        mtype: 'POST',
        multiselect: true,
        datatype: "json",
        toolbar: [true, 'top'],
    ...
    $(_self.Toolbardiv).append($("#tableToolbar"));
 ...
 * 添加页面
 */
AssetsDeviceRcheckProc.prototype.insert = function () {
    //添加流程
 ...
 * 编辑页面
 */
AssetsDeviceRcheckProc.prototype.modify = function () {
 ...
 * 详情页面
 */
AssetsDeviceRcheckProc.prototype.detail = function (id, value) {
    flowUtils.detail(id);
};
/**
 * 控件校验   规则：表单字段name与rules对象保持一致
 */
AssetsDeviceRcheckProc.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            checksNo: {
                maxlength: 255
 ...
 * 保存方法
 */
AssetsDeviceRcheckProc.prototype.save = function (form, windowName, dataSub) {
    var _self = this;
 ...
 * 删除方法
 */
AssetsDeviceRcheckProc.prototype.del = function () {
    var _self = this;
 ...
 * 重载数据
 */
AssetsDeviceRcheckProc.prototype.reLoad = function () {
    var searchdata = {
        keyWord: null,
 ...
 * 关闭对话框
 */
AssetsDeviceRcheckProc.prototype.closeDialog = function (windowName) {
    layer.close(layer.getFrameIndex(windowName));
};
/**
 * 打开高级查询框
 */
AssetsDeviceRcheckProc.prototype.openSearchForm = function (searchDiv, par) {
    var _self = this;
    par = null;
    //if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsDeviceRcheckProc.prototype.searchData = function (dataGridColModel) {
 ...
 * 关键字段查询
 */
AssetsDeviceRcheckProc.prototype.searchByKeyWord = function () {
 ...
 * 隐藏查询框
 */
AssetsDeviceRcheckProc.prototype.hideSearchForm = function () {
    layer.close(searchDialogindex);
};
/**
 * 根据流程状态查询
 */
AssetsDeviceRcheckProc.prototype.initWorkFlow = function (status) {
    $('#bpmState').val(status);
    if (status == "start") {
        $("#assetsDeviceRcheckProc_modify").show();
        $("#assetsDeviceRcheckProc_del").show();
    } else {
        $("#assetsDeviceRcheckProc_modify").hide();
        $("#assetsDeviceRcheckProc_del").hide();
    }
    this.searchData();
};
/**
 * 清空查询条件
 */
AssetsDeviceRcheckProc.prototype.clearData = function () {
 ...
 * 打开视图选择框
 */
AssetsDeviceRcheckProc.prototype.openSelectView = function (selectBtn, contentWidth, contentHeight) {
    var _self = this;
    var _resizeFunc;
 ...
 * 添加视图页面
 */
AssetsDeviceRcheckProc.prototype.addView = function () {
    this.insertIndex = layer.open({
        ...
        skin: 'bs-modal', // bootstrap 风格皮肤 需加载skin
        maxmin: false, //开启最大化最小化按钮
        content: 'assets/device/usertablemodel/userTableModelController/operation/Add/AssetsDeviceRcheckProc/null'
    });
};
 ...
 * 保存视图方法
 */
AssetsDeviceRcheckProc.prototype.saveView = function (form, id) {
    var _self = this;
    avicAjax.ajax({
...
function UpdateTableHead(dataGridColModel, currentView) {
    var modelList = [];
    modelList.push('AssetsDeviceRcheckProc');
                    ...
                    document.onmouseup = function (event) {
                        ...
                        //元素移动
                            ...
                            if (dragIndex > newIndex) {
                                ...
                                jqGridObject = new AssetsDeviceRcheckProc(tableId, url, searchDialogId, searchFormId, keywordId, searchNames, dataGridColModel);
