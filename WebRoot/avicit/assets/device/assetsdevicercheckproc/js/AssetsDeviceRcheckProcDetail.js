 ...
 */
function AssetsDeviceRcheckProcDetail(form, AssetsDeviceRcheckPlan) {
    this._formId = "#" + form;
    this.AssetsDeviceRcheckPlan = AssetsDeviceRcheckPlan;
    this._formId = "#" + form;
    DefaultForm.call(this);
};
AssetsDeviceRcheckProcDetail.prototype = new DefaultForm();
AssetsDeviceRcheckProcDetail.prototype.formcode = "assetsDeviceRcheckProc";
/**
 * 初始化操作
 */
AssetsDeviceRcheckProcDetail.prototype.initFormData = function () {
    var _self = this;
    _self.isLoading = true;
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdevicercheckproc/assetsDeviceRcheckProcController/toDetailJsp.json',
        data: {
            id: _self.id
        },
        type: 'POST',
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($(_self._formId), result.assetsDeviceRcheckProcDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
 ...
 * 启动流程
 */
AssetsDeviceRcheckProcDetail.prototype.start = function (defineId, callback) {
    var _self = this;
    ...
    //子表校验
    var msg = _self.AssetsDeviceRcheckPlan.valid();
    if (msg && msg != "") {
        layer.alert(msg, {
            icon: 7,
            area: ['400px', '500px'], //宽高
    ...
    //子表数据
    var dataSub = _self.AssetsDeviceRcheckPlan.getSubData();
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdevicercheckproc/assetsDeviceRcheckProcController/operation/saveAndStartProcess',
        data: {
            ...
            if (result.flag == "success") {
                ...
                if (files.length != 0) {
                    ...
                    afterUploadEvent = function () {
                        ...
                        //子表刷新,不然更新时会缺少子表ID
                        _self.AssetsDeviceRcheckPlan.reLoad(result.startResult.formId);
                        // 启动成功后回调流程刷新按钮
 ...
 * 更新数据
 */
AssetsDeviceRcheckProcDetail.prototype.save = function (callback) {
    var _self = this;
    //表单校验
    var isValidate = $("#form").validate();
    if (!isValidate.checkForm()) {
    ...
    var dataSub =  _self.AssetsDeviceRcheckPlan.getSubData();
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdevicercheckproc/assetsDeviceRcheckProcController/operation/save',
        data: {
            data: dataVo,
            dataSub: dataSub
        },
            ...
            if (result.flag == "success") {
                ...
                if (files.length != 0) {
                    ...
                    afterUploadEvent = function () {
                        ...
                        layer.close(maskId);
                        //子表刷新,不然更新时会缺少子表ID
                        _self.AssetsDeviceRcheckPlan.reLoad(result.pid);
                        // 启动成功后回调流程刷新按钮
                        callback();
                        _self.initFormData();
                    };
 ...
 * 表单重载json对象数据
 */
AssetsDeviceRcheckProcDetail.prototype.setForm = function (formObj, jsonValue) {
    var obj = formObj;
 ...
 * 控制附件
 */
AssetsDeviceRcheckProcDetail.prototype.setAttachMagic = function (attachMagic) {
 ...
 * 流程控制-多附件必填
 */
AssetsDeviceRcheckProcDetail.prototype.setAttachRequired = function (tagId, required, obj) {
    eval("this.attachRequiredMap." + tagId + " = " + required);
};
AssetsDeviceRcheckProcDetail.prototype.attachRequiredMap = {};
AssetsDeviceRcheckProcDetail.prototype.validAttachRequired = function () {
 ...
 * 多附件增删控制
 */
AssetsDeviceRcheckProcDetail.prototype.setAttachCanAddOrDel = function (tagId, operability, obj) {
    if (operability) {
 ...
 */
AssetsDeviceRcheckProcDetail.prototype.controlSelfElement = function (tagId, operability, obj) {
    if (operability) {
        if (tagId == "assetsDeviceRcheckPlan_editable") {//判断是否允编辑子表数据
            //获取colModel数组对象
            var columnArray = $('#assetsDeviceRcheckPlan').jqGrid('getGridParam', 'colModel');
            //给每个colModel属性列设置可编辑
            $.each(columnArray, function (i, item) {
                $('#assetsDeviceRcheckPlan').setColProp(item.name, {editable: true});
            });
        } else {
            $("#" + tagId).show();
        }
 ...
 * @param obj 参数对象
 */
AssetsDeviceRcheckProcDetail.prototype.controlCustomSubTableForAccess = function (tagId, operability, obj) {
 ...
 * @param obj 参数对象
 */
AssetsDeviceRcheckProcDetail.prototype.controlCustomSubTableForOperability = function (tagId, operability, obj) {
    var tagArr = obj.tag.split("__");
    var subTableName = tagArr[0];
    var subTableId = $("table[name=" + subTableName + "]")[0].id
    var tag = "#" + subTableId;
 ...
 * @param obj 参数对象
 */
AssetsDeviceRcheckProcDetail.prototype.controlCustomSubTableForRequired = function (tagId, operability, obj) {
    //获取必填字段数组
    var notnullFiledArr = this.AssetsDeviceRcheckPlan.notnullFiled;
    var notnullFiledCommentArr = this.AssetsDeviceRcheckPlan.notnullFiledComment;
    var tagValue = tagId;
 ...
 */
AssetsDeviceRcheckProcDetail.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            checksNo: {
                maxlength: 255
...
function addAssetsRow(rowJson) {
    //获取待调拨台账定检计划列表中的数据
    var allData = $('#assetsDeviceRcheckPlan').jqGrid('getRowData');
    ...
    $('#assetsDeviceRcheckPlan').jqGrid('addRow', parameters);
...
function callBackFn(rows, endDate, requestType) {
    ...
    switch (requestType) {
        case "productModelSelect":
            avicAjax.ajax({
                url: 'platform/assets/device/assetsdevicerchecktemporary/assetsDeviceRcheckTemporaryController/operation/deleteAll',
                data: {},
                type: 'POST',
                dataType: 'JSON',
                success: function (result) {
                    avicAjax.ajax({
                        url: 'platform/assets/device/assetsdevicerchecktemporary/assetsDeviceRcheckTemporaryController/operation/saveOrSelect',
                        data: {
                        ...
                        success: function (result) {
                            ...
                            $('#assetsDeviceRcheckPlan').jqGrid(
                                'setGridParam', {
                                    url: 'platform/assets/device/assetsdevicerchecktemporary/assetsDeviceRcheckTemporaryController/operation/getAssetsDeviceRcheckTemporarysByPage',
                                    mtype: 'POST',
                                    multiselect: true,
                                    datatype: "json",
                                    toolbar: [true, 'top'],
...
function openProductModelLayer(singleSelect, requestType, callBackFn, param) {
    ...
    this.openIndex = layer.open({
        ...
        content: "avicit/assets/device/assetsdevicercheckproc/AssetsDeviceRcheckProcSelect.jsp?singleSelect=" + singleSelect + "&requestType=" + requestType + "&callBackFn=" + callBackFn
