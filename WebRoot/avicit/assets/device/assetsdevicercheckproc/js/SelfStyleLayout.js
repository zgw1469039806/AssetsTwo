 ...
 * formcode
 */
EformFlow.prototype.formcode = "assetsDeviceRcheckProc";
    ...
    avicAjax.ajax({
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($("#form"), result.assetsDeviceRcheckProcDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
...
EformFlow.prototype.start = function (defineId, callback) {
	...
	//子表数据
	$('#assetsDeviceRcheckPlan').jqGrid('endEditCell');
	var dataSub = JSON.stringify($('#assetsDeviceRcheckPlan').jqGrid('getRowData'));
			...
			if (result.flag == "success") {
				...
				if (files.length != 0) {
					...
					afterUploadEvent = function () {
						...
						$('#assetsDeviceRcheckPlan').jqGrid('setGridParam', {datatype: 'json', postData: searchdata}).trigger("reloadGrid");
