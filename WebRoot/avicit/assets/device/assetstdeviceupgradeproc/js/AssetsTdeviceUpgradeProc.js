 ...
 */
function AssetsTdeviceUpgradeProc(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "tdeviceUpgrade"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsTdeviceUpgradeProc.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getAssetsTdeviceUpgradeProcsByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsTdeviceUpgradeProc.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
AssetsTdeviceUpgradeProc.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsTdeviceUpgradeProc.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsTdeviceUpgradeProc.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
AssetsTdeviceUpgradeProc.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  				  				  				  				  		     			    				   					createdByPerson:{
						maxlength: 50
 ...
 * 保存方法
 */
AssetsTdeviceUpgradeProc.prototype.save=function(form,windowName,callback){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsTdeviceUpgradeProc.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
AssetsTdeviceUpgradeProc.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
AssetsTdeviceUpgradeProc.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
AssetsTdeviceUpgradeProc.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsTdeviceUpgradeProc.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
AssetsTdeviceUpgradeProc.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsTdeviceUpgradeProc_modify").show();
		$("#assetsTdeviceUpgradeProc_del").show();
	}else{
		$("#assetsTdeviceUpgradeProc_modify").hide();
		$("#assetsTdeviceUpgradeProc_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsTdeviceUpgradeProc.prototype.clearData =function(){
