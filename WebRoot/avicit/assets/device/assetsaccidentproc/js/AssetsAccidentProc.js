 ...
 */
function AssetsAccidentProc(datagrid, url, searchD, form, keyWordId, searchNames, dataGridColModel) {
    ...
    this._formCode = "assetsAccidentProc"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsAccidentProc.prototype.init = function () {
    var _self = this;
    //初始化流程帮助类
    flowListEditor = new FlowListEditor(this._formCode);
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getAssetsAccidentProcsByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsAccidentProc.prototype.insert = function () {
    //添加流程
 ...
 * 编辑页面
 */
AssetsAccidentProc.prototype.modify = function () {
 ...
 * 详情页面
 */
AssetsAccidentProc.prototype.detail = function (id, value) {
    flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsAccidentProc.prototype.openSearchForm = function (searchBtn, contentWidth, contentHeight) {
    var _self = this;
    var _resizeFunc;
 ...
 */
AssetsAccidentProc.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            accidentNo: {
                required: true,
 ...
 * 保存方法
 */
AssetsAccidentProc.prototype.save = function (form, windowName, callback) {
    var _self = this;
    avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsAccidentProc.prototype.del = function () {
    var _self = this;
 ...
 * 重载数据
 */
AssetsAccidentProc.prototype.reLoad = function () {
    var searchdata = {
        keyWord: null,
 ...
 * 关闭对话框
 */
AssetsAccidentProc.prototype.closeDialog = function (windowName, msg) {
    layer.close(layer.getFrameIndex(windowName));
    if (msg && msg !== '') {
        layer.msg(msg);
    }
 ...
 * 后台查询
 */
AssetsAccidentProc.prototype.searchData = function () {
 ...
 * 关键字段查询
 */
AssetsAccidentProc.prototype.searchByKeyWord = function () {
 ...
 * 根据流程状态查询
 */
AssetsAccidentProc.prototype.initWorkFlow = function (status) {
    $('#bpmState').val(status);
    if (status == "start") {
        $("#assetsAccidentProc_modify").show();
        $("#assetsAccidentProc_del").show();
    } else {
        $("#assetsAccidentProc_modify").hide();
        $("#assetsAccidentProc_del").hide();
    }
    this.searchData();
};
/**
 * 清空查询条件
 */
AssetsAccidentProc.prototype.clearData = function () {
