 ...
 */
function AssetsDeviceScrap(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "deviceScrap"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsDeviceScrap.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getAssetsDeviceScrapsByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsDeviceScrap.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
AssetsDeviceScrap.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsDeviceScrap.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsDeviceScrap.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
AssetsDeviceScrap.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  				  				  				  				  		     			    				   					createdByPerson:{
						maxlength: 50
 ...
 * 保存方法
 */
AssetsDeviceScrap.prototype.save=function(form,windowName,callback){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsDeviceScrap.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
AssetsDeviceScrap.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
AssetsDeviceScrap.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
AssetsDeviceScrap.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsDeviceScrap.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
AssetsDeviceScrap.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsDeviceScrap_modify").show();
		$("#assetsDeviceScrap_del").show();
	}else{
		$("#assetsDeviceScrap_modify").hide();
		$("#assetsDeviceScrap_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsDeviceScrap.prototype.clearData =function(){
