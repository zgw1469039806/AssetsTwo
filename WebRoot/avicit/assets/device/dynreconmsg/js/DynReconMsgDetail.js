 ...
 */
function DynReconMsgDetail(form, AssetsReconstructionCheck){
	this._formId = "#" + form;
	this.AssetsReconstructionCheck = AssetsReconstructionCheck;
	this._formId = "#" + form;
	DefaultForm.call(this);
};
DynReconMsgDetail.prototype = new DefaultForm();
DynReconMsgDetail.prototype.formcode = "assetsReconstructionMsg";
/**
 * 初始化操作
 */
DynReconMsgDetail.prototype.initFormData = function(){
	var _self = this;
	_self.isLoading = true;
	avicAjax.ajax({
		url : 'platform/assets/device/dynreconmsg/dynReconMsgController/toDetailJsp.json',
		data : {
			id : _self.id
		},
		type : 'POST',
		...
		success : function(result) {
			if (result.flag == "success") {
				_self.setForm($(_self._formId),result.dynReconMsgDTO);
			} else {
				layer.msg('数据加载失败！');
			}
		}
 ...
 * 启动流程
 */
DynReconMsgDetail.prototype.start = function(defineId, callback){
	var _self = this;
	...
	avicAjax.ajax({
		url : 'platform/assets/device/dynreconmsg/dynReconMsgController/operation/saveAndStartProcess',
		data : {
 ...
 * 更新数据
 */
DynReconMsgDetail.prototype.save=function(callback){
	var _self = this;
    //表单校验
	var isValidate = $("#form").validate();
	if (!isValidate.checkForm()) {
	...
	var dataSub = _self.AssetsReconstructionCheck.getSubData();
	avicAjax.ajax({
		url : 'platform/assets/device/dynreconmsg/dynReconMsgController/operation/save',
		data : {
			data : dataVo,
			dataSub : dataSub
		},
 ...
 * 表单重载json对象数据
 */
DynReconMsgDetail.prototype.setForm = function(formObj, jsonValue){
    var obj = formObj;
 ...
 * 控制附件
 */
DynReconMsgDetail.prototype.setAttachMagic = function(attachMagic) {
 ...
 * 流程控制-多附件必填
 */
DynReconMsgDetail.prototype.setAttachRequired = function(tagId,required,obj){
    eval("this.attachRequiredMap." + tagId + " = " + required);
};
DynReconMsgDetail.prototype.attachRequiredMap = {};
DynReconMsgDetail.prototype.validAttachRequired = function(){
 ...
 * 多附件增删控制
 */
DynReconMsgDetail.prototype.setAttachCanAddOrDel = function(tagId,operability,obj){
    if(operability) {
 ...
 */
DynReconMsgDetail.prototype.controlSelfElement = function(tagId, operability, obj){
	 if(operability){
 ...
 * @param obj 参数对象
 */
DynReconMsgDetail.prototype.controlCustomSubTableForAccess = function(tagId, operability, obj){
 ...
 * @param obj 参数对象
 */
DynReconMsgDetail.prototype.controlCustomSubTableForOperability = function(tagId, operability, obj){
	var tagArr = obj.tag.split("__");
	var subTableName = tagArr[0];
	var subTableId = $("table[name="+subTableName+"]")[0].id
	var tag = "#"+subTableId;
 ...
 * @param obj 参数对象
 */
DynReconMsgDetail.prototype.controlCustomSubTableForRequired = function(tagId, operability, obj){
	//获取必填字段数组
 ...
 */
DynReconMsgDetail.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  		     			    				   					applyYear:{
						maxlength: 50
