 ...
 */
function DynReconMsg(datagrid, url, searchD, form, keyWordId, searchNames, dataGridColModel) {
    if (!datagrid || typeof (datagrid) !== 'string' && datagrid.trim() !== '') {
        throw new Error("datagrid不能为空！");
    }
    var _url = "platform/assets/device/dynreconmsg/dynReconMsgController/operation/";
    this.getUrl = function () {
        return _url;
    }
    this._formCode = "dynReconMsg"; //表单的code，启动流程时需要
 ...
 */
var searchData ={
    keyWord : JSON.stringify({bpmState:"ended"}),
    param:null,
};
DynReconMsg.prototype.init = function () {
    var _self = this;
    //初始化流程帮助类
    flowListEditor = new FlowListEditor(this._formCode);
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getDynReconMsgsByPage.json',
        mtype: 'POST',
        datatype: "json",
        postData:searchData,
 ...
 * 添加页面
 */
DynReconMsg.prototype.insert = function () {
    //添加流程
 ...
 * 编辑页面
 */
DynReconMsg.prototype.modify = function () {
 ...
 * 详情页面
 */
DynReconMsg.prototype.detail = function (id, value) {
    flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
DynReconMsg.prototype.openSearchForm = function (searchBtn, contentWidth, contentHeight) {
    var _self = this;
    var _resizeFunc;
 ...
 */
DynReconMsg.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            applyYear: {
                maxlength: 50
 ...
 * 保存方法
 */
DynReconMsg.prototype.save = function (form, windowName, callback) {
    var _self = this;
    avicAjax.ajax({
 ...
 * 删除方法
 */
DynReconMsg.prototype.del = function () {
    var _self = this;
 ...
 * 重载数据
 */
DynReconMsg.prototype.reLoad = function () {
    var searchdata = {
        keyWord: null,
 ...
 * 关闭对话框
 */
DynReconMsg.prototype.closeDialog = function (windowName, msg) {
    layer.close(layer.getFrameIndex(windowName));
    if (msg && msg !== '') {
        layer.msg(msg);
    }
 ...
 * 后台查询
 */
DynReconMsg.prototype.searchData = function () {
 ...
 * 关键字段查询
 */
DynReconMsg.prototype.searchByKeyWord = function () {
 ...
 * 根据流程状态查询
 */
DynReconMsg.prototype.initWorkFlow = function (status) {
    $('#bpmState').val(status);
    if (status == "start") {
        $("#dynReconMsg_modify").show();
        $("#dynReconMsg_del").show();
    } else {
        $("#dynReconMsg_modify").hide();
        $("#dynReconMsg_del").hide();
    }
    this.searchData();
};
/**
 * 清空查询条件
 */
DynReconMsg.prototype.clearData = function () {
    clearFormData(this._formId);
    this.searchData();
};

DynReconMsg.prototype.submit = function () {
    var _self = this;
    var selectedRow = [];
