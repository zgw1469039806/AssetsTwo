...
<html>
<head>
    <!-- ControllerPath = "assets/device/assetsdevicereuse/assetsDeviceReuseController/toAssetsDeviceReuseManage" -->
    <title>管理</title>
...
<div id="panelnorth"
     data-options="region:'north',height:fixheight(1),onResize:function(a){$('#assetsDeviceReuse').setGridWidth(a);$('#assetsDeviceReuse').setGridHeight(getJgridTableHeight($('#panelnorth')));$(window).trigger('resize');}">
    <div id="toolbar_assetsDeviceReuse" class="toolbar">
        <div class="toolbar-left">
            <sec:accesscontrollist hasPermission="3" domainObject="formdialog_assetsDeviceReuse_button_add"
                                   permissionDes="添加">
                <a id="assetsDeviceReuse_insert" href="javascript:void(0)" class="btn btn-default form-tool-btn btn-sm"
                   role="button" title="添加"><i class="fa fa-plus"></i> 添加</a>
            </sec:accesscontrollist>
            <sec:accesscontrollist hasPermission="3" domainObject="formdialog_assetsDeviceReuse_button_edit"
                                   permissionDes="编辑">
                <a id="assetsDeviceReuse_modify" href="javascript:void(0)" class="btn btn-default form-tool-btn btn-sm"
                   style="display:none;" role="button" title="编辑"><i class="fa fa-file-text-o"></i> 编辑</a>
            </sec:accesscontrollist>
            <sec:accesscontrollist hasPermission="3" domainObject="formdialog_assetsDeviceReuse_button_delete"
                                   permissionDes="删除">
                <a id="assetsDeviceReuse_del" href="javascript:void(0)" class="btn btn-default form-tool-btn btn-sm"
        ...
        <div class="toolbar-right">
            ...
            <div class="input-group form-tool-search" style="width:125px">
                <input type="text" name="assetsDeviceReuse_keyWord" id="assetsDeviceReuse_keyWord" style="width:125px;"
                       class="form-control input-sm" placeholder="请输入查询条件">
                <label id="assetsDeviceReuse_searchPart" class="icon icon-search form-tool-searchicon"></label>
            </div>
            <div class="input-group-btn form-tool-searchbtn">
                <a id="assetsDeviceReuse_searchAll" href="javascript:void(0)" class="btn btn-defaul btn-sm"
                   role="button">高级查询 <span class="caret"></span></a>
            </div>
        </div>
    </div>
    <table id="assetsDeviceReuse"></table>
    <div id="assetsDeviceReusePager"></div>
</div>
<div id="centerpanel"
     data-options="region:'center',split:true,onResize:function(a){$('#assetsDeviceReusesub').setGridWidth(a); $('#assetsDeviceReusesub').setGridHeight(getJgridTableHeight($('#centerpanel')));$(window).trigger('resize');}">
    <div id="toolbar_assetsDeviceReusesub" class="toolbar">
        <div class="toolbar-right">
            <div class="input-group form-tool-search" style="width:125px">
                <input type="text" name="assetsDeviceReusesub_keyWord" id="assetsDeviceReusesub_keyWord"
                       style="width:125px;" class="form-control input-sm" placeholder="请输入查询条件">
                <label id="assetsDeviceReusesub_searchPart" class="icon icon-search form-tool-searchicon"></label>
            </div>
            <div class="input-group-btn form-tool-searchbtn">
                <a id="assetsDeviceReusesub_searchAll" href="javascript:void(0)" class="btn btn-defaul btn-sm"
                   role="button">高级查询 <span class="caret"></span></a>
            </div>
        </div>
    </div>
    <table id="assetsDeviceReusesub"></table>
    <div id="assetsDeviceReusesubPager"></div>
</div>
</body>
<!-- 主表高级查询 -->
    ...
    <form id="form">
        ...
        <table class="form_commonTable">
            ...
            <tr>
                ...
                <th width="10%">再用原因:</th>
                <td width="15%">
                    <textarea class="form-control input-sm" rows="3" title="再用原因" name="reuseReason"
                              id="reuseReason"></textarea>
                </td>
...
<script src="avicit/assets/device/assetsdevicereuse/js/AssetsDeviceReuse.js" type="text/javascript"></script>
<script src="avicit/assets/device/assetsdevicereuse/js/AssetsDeviceReusesub.js" type="text/javascript"></script>
<script type="text/javascript">
    var assetsDeviceReuse;
    var assetsDeviceReusesub;

    function formatValue(cellvalue, options, rowObject) {
        return '<a href="javascript:void(0);" onclick="assetsDeviceReuse.detail(\'' + rowObject.id + '\');">' + cellvalue + '</a>';
    }

    function formatDateForHref(cellvalue, options, rowObject) {
        var thisDate = format(cellvalue);
        return '<a href="javascript:void(0);" onclick="assetsDeviceReuse.detail(\'' + rowObject.id + '\');">' + thisDate + '</a>';
    }

    //刷新本页面
    window.bpm_operator_refresh = function () {
        assetsDeviceReuse.reLoad();
    };
    ...
    $(document).ready(function () {
        ...
        searchMainTips.push("再用单号");
        // searchMainNames.push("formState");
        // searchMainTips.push("单据状态");
        var searchMainC = searchMainTips.length == 2 ? '或' + searchMainTips[1] : "";
        $('#assetsDeviceReuse_keyWord').attr('placeholder', '请输入' + searchMainTips[0] + searchMainC);
        $('#assetsDeviceReuse_keyWord').attr('title', '请输入' + searchMainTips[0] + searchMainC);
        ...
        $('#assetsDeviceReusesub_keyWord').attr('placeholder', '请输入' + searchSubTips[0] + searchSubC);
        $('#assetsDeviceReusesub_keyWord').attr('title', '请输入' + searchSubTips[0] + searchSubC);
        var assetsDeviceReuseGridColModel = [
            {label: 'id', name: 'id', key: true, width: 75, hidden: true}
            , {label: '再用单号', name: 'procId', width: 150, formatter: formatValue}
            ...
            , {label: '再用原因', name: 'reuseReason', width: 150}
            , {label: '有关产品风险分析', name: 'productRiskAnalyse', width: 150}
            , {label: '再用总金额', name: 'totalReuseMoney', width: 150, hidden: true}
        ...
        assetsDeviceReuse = new AssetsDeviceReuse('assetsDeviceReuse', '${url}', 'form', assetsDeviceReuseGridColModel, 'searchDialog',
            function (pid) {
                assetsDeviceReusesub = new AssetsDeviceReusesub('assetsDeviceReusesub', '${surl}', "formSub", assetsDeviceReusesubGridColModel, 'searchDialogSub', pid, searchSubNames, "assetsDeviceReusesub_keyWord");
            },
            function (pid) {
                assetsDeviceReusesub.reLoad(pid);
            },
            searchMainNames,
            "assetsDeviceReuse_keyWord");
        //主表操作
        //添加按钮绑定事件
        $('#assetsDeviceReuse_insert').bind('click', function () {
            assetsDeviceReuse.insert();
        });
        //编辑按钮绑定事件
        $('#assetsDeviceReuse_modify').bind('click', function () {
            assetsDeviceReuse.modify();
        });
        //删除按钮绑定事件
        $('#assetsDeviceReuse_del').bind('click', function () {
            assetsDeviceReuse.del();
        });
        //打开高级查询框
        $('#assetsDeviceReuse_searchAll').bind('click', function () {
            assetsDeviceReuse.openSearchForm(this, $('#assetsDeviceReuse'));
        });
        //关键字段查询按钮绑定事件
        $('#assetsDeviceReuse_searchPart').bind('click', function () {
            assetsDeviceReuse.searchByKeyWord();
        });

        //根据流程状态加载数据
        $('#workFlowSelect').bind('change', function () {
            assetsDeviceReuse.initWorkFlow($(this).val());
        });
        //子表操作
        //打开高级查询
        $('#assetsDeviceReusesub_searchAll').bind('click', function () {
            assetsDeviceReusesub.openSearchForm(this, $('#assetsDeviceReusesub'));
        });
        //关键字段查询按钮绑定事件
        $('#assetsDeviceReusesub_searchPart').bind('click', function () {
            assetsDeviceReusesub.searchByKeyWord();
        });
