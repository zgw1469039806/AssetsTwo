 ...
 * formcode
 */
EformFlow.prototype.formcode = "assetsDeviceReuse";
    ...
    avicAjax.ajax({
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($("#form"), result.assetsDeviceReuseDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
...
EformFlow.prototype.start = function (defineId, callback) {
	...
	//子表数据
	$('#assetsDeviceReusesub').jqGrid('endEditCell');
	var dataSub = JSON.stringify($('#assetsDeviceReusesub').jqGrid('getRowData'));

    //判断子表是否有数据，如果没有数据禁止保存提交
    var Rowdata = ($('#assetsDeviceReusesub').jqGrid('getRowData'))
    if(Rowdata.length<1){
        layer.msg('请选择设备！');
        var maskId = layer.load();
                ...
                if (result.flag == "success") {
                    ...
                    if (files.length != 0) {
                        ...
                        afterUploadEvent = function () {
                            ...
                            $('#assetsDeviceReusesub').jqGrid('setGridParam', {datatype: 'json', postData: searchdata}).trigger("reloadGrid");
