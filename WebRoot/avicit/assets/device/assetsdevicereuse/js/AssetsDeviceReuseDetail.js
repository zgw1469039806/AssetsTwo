 ...
 */
function AssetsDeviceReuseDetail(form, AssetsDeviceReusesub) {
    this._formId = "#" + form;
    this.AssetsDeviceReusesub = AssetsDeviceReusesub;
    this._formId = "#" + form;
    DefaultForm.call(this);
};
AssetsDeviceReuseDetail.prototype = new DefaultForm();
AssetsDeviceReuseDetail.prototype.formcode = "assetsDeviceReuse";
/**
 * 初始化操作
 */
AssetsDeviceReuseDetail.prototype.initFormData = function () {
    var _self = this;
    _self.isLoading = true;
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdevicereuse/assetsDeviceReuseController/toDetailJsp.json',
        data: {
            id: _self.id
        },
        type: 'POST',
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($(_self._formId), result.assetsDeviceReuseDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
 ...
 * 启动流程
 */
AssetsDeviceReuseDetail.prototype.start = function (defineId, callback) {
    var _self = this;
    ...
    //子表校验
    var msg = _self.AssetsDeviceReusesub.valid();
    if (msg && msg != "") {
        layer.alert(msg, {
            icon: 7,
            area: ['400px', ''], //宽高
    ...
    //子表数据
    var dataSub = _self.AssetsDeviceReusesub.getSubData();
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdevicereuse/assetsDeviceReuseController/operation/saveAndStartProcess',
        data: {
            ...
            if (result.flag == "success") {
                ...
                if (files.length != 0) {
                    ...
                    afterUploadEvent = function () {
                        ...
                        //子表刷新,不然更新时会缺少子表ID
                        _self.AssetsDeviceReusesub.reLoad(result.startResult.formId);
                        // 启动成功后回调流程刷新按钮
 ...
 * 更新数据
 */
AssetsDeviceReuseDetail.prototype.save = function (callback) {
    var _self = this;
    //表单校验
    var isValidate = $("#form").validate();
    if (!isValidate.checkForm()) {
    ...
    var dataSub = _self.AssetsDeviceReusesub.getSubData();
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdevicereuse/assetsDeviceReuseController/operation/save',
        data: {
            data: dataVo,
            dataSub: dataSub
        },
            ...
            if (result.flag == "success") {
                ...
                if (files.length != 0) {
                    ...
                    afterUploadEvent = function () {
                        ...
                        layer.close(maskId);
                        //子表刷新,不然更新时会缺少子表ID
                        _self.AssetsDeviceReusesub.reLoad(result.pid);
                        // 启动成功后回调流程刷新按钮
                        callback();
                        _self.initFormData();
                    };
 ...
 * 表单重载json对象数据
 */
AssetsDeviceReuseDetail.prototype.setForm = function (formObj, jsonValue) {
    var obj = formObj;
 ...
 * 控制附件
 */
AssetsDeviceReuseDetail.prototype.setAttachMagic = function (attachMagic) {
 ...
 * 流程控制-多附件必填
 */
AssetsDeviceReuseDetail.prototype.setAttachRequired = function (tagId, required, obj) {
    eval("this.attachRequiredMap." + tagId + " = " + required);
};
AssetsDeviceReuseDetail.prototype.attachRequiredMap = {};
AssetsDeviceReuseDetail.prototype.validAttachRequired = function () {
 ...
 * 多附件增删控制
 */
AssetsDeviceReuseDetail.prototype.setAttachCanAddOrDel = function (tagId, operability, obj) {
    if (operability) {
 ...
 */
AssetsDeviceReuseDetail.prototype.controlSelfElement = function (tagId, operability, obj) {
    if (operability) {
        if (tagId == "assetsDeviceReusesub_editable") {//判断是否允编辑子表数据
            //获取colModel数组对象
            var columnArray = $('#assetsDeviceReusesub').jqGrid('getGridParam', 'colModel');
            //给每个colModel属性列设置可编辑
            $.each(columnArray, function (i, item) {
                $('#assetsDeviceReusesub').setColProp(item.name, {editable: true});
            });
        } else {
            $("#" + tagId).show();
        }
 ...
 * @param obj 参数对象
 */
AssetsDeviceReuseDetail.prototype.controlCustomSubTableForAccess = function (tagId, operability, obj) {
 ...
 * @param obj 参数对象
 */
AssetsDeviceReuseDetail.prototype.controlCustomSubTableForOperability = function (tagId, operability, obj) {
    var tagArr = obj.tag.split("__");
    var subTableName = tagArr[0];
    var subTableId = $("table[name=" + subTableName + "]")[0].id
    var tag = "#" + subTableId;
 ...
 * @param obj 参数对象
 */
AssetsDeviceReuseDetail.prototype.controlCustomSubTableForRequired = function (tagId, operability, obj) {
    //获取必填字段数组
    var notnullFiledArr = this.AssetsDeviceReusesub.notnullFiled;
    var notnullFiledCommentArr = this.AssetsDeviceReusesub.notnullFiledComment;
    var tagValue = tagId;
 ...
 */
AssetsDeviceReuseDetail.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            createdByPerson: {
                maxlength: 50
...
function addAssetsRow(rowJson) {
    //获取待调拨台账列表中的数据
    var allData = $('#assetsDeviceReusesub').jqGrid('getRowData');
    ...
    $('#assetsDeviceReusesub').jqGrid('addRow', parameters);
