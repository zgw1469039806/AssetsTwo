 ...
 */
function AssetsDeviceBorrow(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "deviceBorrow2"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsDeviceBorrow.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getAssetsDeviceBorrowsByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsDeviceBorrow.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
AssetsDeviceBorrow.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsDeviceBorrow.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsDeviceBorrow.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
AssetsDeviceBorrow.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  				  				  				  				  		     			    				   					createdByPerson:{
						maxlength: 50
 ...
 * 保存方法
 */
AssetsDeviceBorrow.prototype.save=function(form,windowName,callback){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsDeviceBorrow.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
AssetsDeviceBorrow.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
AssetsDeviceBorrow.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
AssetsDeviceBorrow.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsDeviceBorrow.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
AssetsDeviceBorrow.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsDeviceBorrow_modify").show();
		$("#assetsDeviceBorrow_del").show();
	}else{
		$("#assetsDeviceBorrow_modify").hide();
		$("#assetsDeviceBorrow_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsDeviceBorrow.prototype.clearData =function(){
