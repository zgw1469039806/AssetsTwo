 ...
 */
function AssetsDeviceAcheckProcDetail(form, AssetsDeviceAcheckPlan) {
    this._formId = "#" + form;
    this.AssetsDeviceAcheckPlan = AssetsDeviceAcheckPlan;
    this._formId = "#" + form;
    DefaultForm.call(this);
};
AssetsDeviceAcheckProcDetail.prototype = new DefaultForm();
AssetsDeviceAcheckProcDetail.prototype.formcode = "assetsDeviceAcheckProc";
/**
 * 初始化操作
 */
AssetsDeviceAcheckProcDetail.prototype.initFormData = function () {
    var _self = this;
    ...
    avicAjax.ajax({
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($(_self._formId), result.assetsDeviceAcheckProcDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
 ...
 * 启动流程
 */
AssetsDeviceAcheckProcDetail.prototype.start = function (defineId, callback) {
    var _self = this;
    ...
    //子表校验
    var msg = _self.AssetsDeviceAcheckPlan.valid();
    if (msg && msg != "") {
        layer.alert(msg, {
            icon: 7,
            area: ['400px', ''], //宽高
    ...
    //子表数据
    var dataSub = _self.AssetsDeviceAcheckPlan.getSubData();
    avicAjax.ajax({
            ...
            if (result.flag == "success") {
                ...
                if (files.length != 0) {
                    ...
                    afterUploadEvent = function () {
                        ...
                        //子表刷新,不然更新时会缺少子表ID
                        _self.AssetsDeviceAcheckPlan.reLoad(result.startResult.formId);
                        // 启动成功后回调流程刷新按钮
 ...
 * 更新数据
 */
AssetsDeviceAcheckProcDetail.prototype.save = function (callback) {
    var _self = this;
    //表单校验
    var isValidate = $("#form").validate();
    if (!isValidate.checkForm()) {
            ...
            if (result.flag == "success") {
                ...
                if (files.length != 0) {
                    ...
                    afterUploadEvent = function () {
                        ...
                        layer.close(maskId);
                        //子表刷新,不然更新时会缺少子表ID
                        _self.AssetsDeviceAcheckPlan.reLoad(result.pid);
                        // 启动成功后回调流程刷新按钮
                        callback();
                        _self.initFormData();
                    };
 ...
 * 表单重载json对象数据
 */
AssetsDeviceAcheckProcDetail.prototype.setForm = function (formObj, jsonValue) {
    var obj = formObj;
 ...
 * 控制附件
 */
AssetsDeviceAcheckProcDetail.prototype.setAttachMagic = function (attachMagic) {
 ...
 * 流程控制-多附件必填
 */
AssetsDeviceAcheckProcDetail.prototype.setAttachRequired = function (tagId, required, obj) {
    eval("this.attachRequiredMap." + tagId + " = " + required);
};
AssetsDeviceAcheckProcDetail.prototype.attachRequiredMap = {};
AssetsDeviceAcheckProcDetail.prototype.validAttachRequired = function () {
 ...
 * 多附件增删控制
 */
AssetsDeviceAcheckProcDetail.prototype.setAttachCanAddOrDel = function (tagId, operability, obj) {
    if (operability) {
 ...
 */
AssetsDeviceAcheckProcDetail.prototype.controlSelfElement = function (tagId, operability, obj) {
    if (operability) {
        if (tagId == "assetsDeviceAcheckPlan_editable") {//判断是否允编辑子表数据
            //获取colModel数组对象
            var columnArray = $('#assetsDeviceAcheckPlan').jqGrid('getGridParam', 'colModel');
            //给每个colModel属性列设置可编辑
            $.each(columnArray, function (i, item) {
                $('#assetsDeviceAcheckPlan').setColProp(item.name, {editable: true});
            });
        } else {
            $("#" + tagId).show();
        }
 ...
 * @param obj 参数对象
 */
AssetsDeviceAcheckProcDetail.prototype.controlCustomSubTableForAccess = function (tagId, operability, obj) {
 ...
 * @param obj 参数对象
 */
AssetsDeviceAcheckProcDetail.prototype.controlCustomSubTableForOperability = function (tagId, operability, obj) {
    var tagArr = obj.tag.split("__");
    var subTableName = tagArr[0];
    var subTableId = $("table[name=" + subTableName + "]")[0].id
    var tag = "#" + subTableId;
 ...
 * @param obj 参数对象
 */
AssetsDeviceAcheckProcDetail.prototype.controlCustomSubTableForRequired = function (tagId, operability, obj) {
    //获取必填字段数组
    var notnullFiledArr = this.AssetsDeviceAcheckPlan.notnullFiled;
    var notnullFiledCommentArr = this.AssetsDeviceAcheckPlan.notnullFiledComment;
    var tagValue = tagId;
 ...
 */
AssetsDeviceAcheckProcDetail.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            acheckCode: {
                maxlength: 50
...
function addAssetsRow(rowJson) {
    //获取待调拨台账定检计划列表中的数据
    var allData = $('#assetsDeviceAcheckPlan').jqGrid('getRowData');
    ...
    $('#assetsDeviceAcheckPlan').jqGrid('addRow', parameters);
...
function callBackFn(rows, endDate, requestType) {
    ...
    switch (requestType) {
        case "productModelSelect":
            avicAjax.ajax({
                url: 'platform/assets/device/assetsdeviceachecktemporary/assetsDeviceAcheckTemporaryController/operation/deleteAll',
                data: {},
                type: 'POST',
                dataType: 'JSON',
                success: function (result) {
                    avicAjax.ajax({
                        url: 'platform/assets/device/assetsdeviceachecktemporary/assetsDeviceAcheckTemporaryController/operation/saveOrSelect',
                        data: {
                        ...
                        success: function (result) {
                            var _self = this;
                            $('#assetsDeviceAcheckPlan').jqGrid(
                                'setGridParam', {
                                    url: 'platform/assets/device/assetsdeviceachecktemporary/assetsDeviceAcheckTemporaryController/operation/getAssetsDeviceAcheckTemporarysByPage',
                                    mtype: 'POST',
                                    multiselect: true,
                                    datatype: "json",
                                    toolbar: [true, 'top'],
...
function openProductModelLayer(singleSelect, requestType, callBackFn, param) {
    ...
    this.openIndex = layer.open({
        ...
        content: "avicit/assets/device/assetsdeviceacheckproc/AssetsDeviceAcheckProcSelect.jsp?singleSelect=" + singleSelect + "&requestType=" + requestType + "&callBackFn=" + callBackFn
