 ...
 */
function AssetsDeviceAcheckProc(datagrid, url, searchForm, dataGridColModel,searchDialog, afterInit, clickRowLoad, searchMainNames,demoMainDept_KeyWord) {
	...
	this._formCode = "assetsDeviceAcheckProc"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsDeviceAcheckProc.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	var issubinit = false;
	$(_self._datagridId).jqGrid({
				url : _self.getUrl() + 'getAssetsDeviceAcheckProcsByPage.json',
				mtype : 'POST',
				multiselect : true,
				datatype : "json",
				toolbar : [ true, 'top' ],
 ...
 * 添加页面
 */
AssetsDeviceAcheckProc.prototype.insert = function() {
	//添加流程
 ...
 * 编辑页面
 */
AssetsDeviceAcheckProc.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsDeviceAcheckProc.prototype.detail = function(id, value) {
	flowUtils.detail(id);
};
/**
 * 控件校验   规则：表单字段name与rules对象保持一致
 */
AssetsDeviceAcheckProc.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  				  		     			    				   					acheckCode:{
						maxlength: 50
 ...
 * 保存方法
 */
AssetsDeviceAcheckProc.prototype.save=function(form,windowName,dataSub){
	var _self = this;
 ...
 * 删除方法
 */
AssetsDeviceAcheckProc.prototype.del=function(){
    var _self = this;
 ...
 * 重载数据
 */
AssetsDeviceAcheckProc.prototype.reLoad=function(){
    var searchdata = {
		keyWord : null,
 ...
 * 关闭对话框
 */
AssetsDeviceAcheckProc.prototype.closeDialog=function(windowName){
	layer.close(layer.getFrameIndex(windowName));
};
/**
 * 打开高级查询框
 */
AssetsDeviceAcheckProc.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询	
 */
AssetsDeviceAcheckProc.prototype.searchData =function(dataGridColModel){
 ...
 * 关键字段查询
 */
AssetsDeviceAcheckProc.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsDeviceAcheckProc.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 根据流程状态查询
 */
AssetsDeviceAcheckProc.prototype.initWorkFlow = function(status) {
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsDeviceAcheckProc_modify").show();
		$("#assetsDeviceAcheckProc_del").show();
	}else{
		$("#assetsDeviceAcheckProc_modify").hide();
		$("#assetsDeviceAcheckProc_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsDeviceAcheckProc.prototype.clearData =function(){
 ...
 * 打开视图选择框
 */
AssetsDeviceAcheckProc.prototype.openSelectView = function (selectBtn, contentWidth, contentHeight) {
	var _self = this;
	var _resizeFunc;
 ...
 * 添加视图页面
 */
AssetsDeviceAcheckProc.prototype.addView = function () {
	this.insertIndex = layer.open({
		...
		skin: 'bs-modal', // bootstrap 风格皮肤 需加载skin
		maxmin: false, //开启最大化最小化按钮
		content: 'assets/device/usertablemodel/userTableModelController/operation/Add/AssetsDeviceAcheckProc/null'
	});
};
 ...
 * 保存视图方法
 */
AssetsDeviceAcheckProc.prototype.saveView = function (form, id) {
	var _self = this;
	avicAjax.ajax({
...
function UpdateTableHead(dataGridColModel, currentView) {
	var modelList = [];
	modelList.push('AssetsDeviceAcheckProc');
					...
					document.onmouseup = function (event) {
						...
						//元素移动
							...
							if (dragIndex > newIndex) {
								...
								jqGridObject = new AssetsDeviceAcheckProc(tableId, url, searchDialogId, searchFormId, keywordId, searchNames, dataGridColModel);
