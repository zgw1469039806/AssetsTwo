 ...
 */
function AssetsDeviceInventory(datagrid, url, searchForm, dataGridColModel, searchDialog, afterInit, clickRowLoad, searchMainNames, demoMainDept_KeyWord) {
    ...
    this._formCode = "assetsDeviceInventory"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsDeviceInventory.prototype.init = function () {
    var _self = this;
    //初始化流程帮助类
    flowListEditor = new FlowListEditor(this._formCode);
    var issubinit = false;
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getAssetsDeviceInventorysByPage.json',
        mtype: 'POST',
        multiselect: true,
        datatype: "json",
        toolbar: [true, 'top'],
 ...
 * 添加页面
 */
AssetsDeviceInventory.prototype.insert = function (formId) {
    //添加流程
    this.openIndex = layer.open({
        ...
        content: "avicit/assets/device/assetsdeviceinventory/AssetsDeviceInventoryAdd.jsp"

    });
};

function callBackFn01() {
    layer.closeAll();
    $("#assetsDeviceInventory").jqGrid('setGridParam', {
        datatype: 'json'
    }).trigger("reloadGrid");
}
 ...
 * 编辑页面
 */
AssetsDeviceInventory.prototype.modify = function () {
 ...
 * 详情页面
 */
AssetsDeviceInventory.prototype.detail = function (id, value) {
    flowUtils.detail(id);
};
/**
 * 控件校验   规则：表单字段name与rules对象保持一致
 */
AssetsDeviceInventory.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            inventoryName: {
                maxlength: 255
 ...
 * 保存方法
 */
AssetsDeviceInventory.prototype.save = function (form, windowName, dataSub) {
    var _self = this;
    var subFrame = $('iframe')[0].contentWindow;
    avicAjax.ajax({
        url: _self.getUrl() + "save",//platform/assets/device/assetsdeviceinventory/assetsDeviceInventoryController/operation/save
        data: {
 ...
 * 删除方法
 */
AssetsDeviceInventory.prototype.del = function () {
    var _self = this;
 ...
 * 重载数据
 */
AssetsDeviceInventory.prototype.reLoad = function () {
    var searchdata = {
        keyWord: null,
 ...
 * 关闭对话框
 */
AssetsDeviceInventory.prototype.closeDialog = function (windowName) {
    layer.close(layer.getFrameIndex(windowName));
};
/**
 * 打开高级查询框
 */
AssetsDeviceInventory.prototype.openSearchForm = function (searchDiv, par) {
    var _self = this;
    par = null;
    //if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsDeviceInventory.prototype.searchData = function () {
 ...
 * 关键字段查询
 */
AssetsDeviceInventory.prototype.searchByKeyWord = function () {
 ...
 * 隐藏查询框
 */
AssetsDeviceInventory.prototype.hideSearchForm = function () {
    layer.close(searchDialogindex);
};
/**
 * 根据流程状态查询
 */
AssetsDeviceInventory.prototype.initWorkFlow = function (status) {
    $('#bpmState').val(status);
    if (status == "start") {
        $("#assetsDeviceInventory_modify").show();
        $("#assetsDeviceInventory_del").show();
    } else {
        $("#assetsDeviceInventory_modify").hide();
        $("#assetsDeviceInventory_del").hide();
    }
    this.searchData();
};
/**
 * 清空查询条件
 */
AssetsDeviceInventory.prototype.clearData = function () {
