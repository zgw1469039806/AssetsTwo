 ...
 */
function AssetsDeviceInventoryDetail(form, AssetsDeviceInventorySub) {
    this._formId = "#" + form;
    this.AssetsDeviceInventorySub = AssetsDeviceInventorySub;
    this._formId = "#" + form;
};
AssetsDeviceInventoryDetail.prototype.formcode = "assetsDeviceInventory";
/**
 * 初始化操作
 */
AssetsDeviceInventoryDetail.prototype.initFormData = function () {
    var _self = this;
    _self.isLoading = true;
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdeviceinventory/assetsDeviceInventoryController/toDetailJsp.json',
        data: {
            id: _self.id
        },
        type: 'POST',
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($(_self._formId), result.assetsDeviceInventoryDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
 ...
 * 启动流程
 */
AssetsDeviceInventoryDetail.prototype.start = function (defineId, callback) {
    var _self = this;
    ...
    //子表校验
    var msg = _self.AssetsDeviceInventorySub.valid();
    if (msg && msg != "") {
        layer.alert(msg, {
            icon: 7,
            area: ['400px', ''], //宽高
    ...
    //子表数据
    var dataSub = _self.AssetsDeviceInventorySub.getSubData();
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdeviceinventory/assetsDeviceInventoryController/operation/saveAndStartProcess',
        data: {
            ...
            if (result.flag == "success") {
                ...
                if (files.length != 0) {
                    ...
                    afterUploadEvent = function () {
                        ...
                        //子表刷新,不然更新时会缺少子表ID
                        _self.AssetsDeviceInventorySub.reLoad(result.startResult.formId);
                        // 启动成功后回调流程刷新按钮
 ...
 * 更新数据
 */
AssetsDeviceInventoryDetail.prototype.save = function (callback) {
    var _self = this;
    //表单校验
    var isValidate = $("#form").validate();
    if (!isValidate.checkForm()) {
    ...
    var dataSub = _self.AssetsDeviceInventorySub.getSubData();
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdeviceinventory/assetsDeviceInventoryController/operation/save',
        data: {
            data: dataVo,
            dataSub: dataSub
        },
        ...
        success: function (result) {
            if (result.flag == "success") {
                ...
                // 		layer.close(maskId);
                // 		//子表刷新,不然更新时会缺少子表ID
                // 		_self.AssetsDeviceInventorySub.reLoad(result.pid);
                // 		// 启动成功后回调流程刷新按钮
                // 		callback();
                // 		_self.initFormData();
                // 	};
 ...
 * 更新数据（生成盘点清单）
 */
AssetsDeviceInventoryDetail.prototype.save01 = function (callback) {
    var _self = this;
    //表单校验
    var isValidate = $("#form").validate();
    if (!isValidate.checkForm()) {
    ...
    var dataSub = _self.AssetsDeviceInventorySub.getSubData();
    avicAjax.ajax({
        url: 'platform/assets/device/assetsdeviceinventory/assetsDeviceInventoryController/operation/save01',
        data: {
            data: dataVo,
            dataSub: dataSub
        },
 ...
 * 表单重载json对象数据
 */
AssetsDeviceInventoryDetail.prototype.setForm = function (formObj, jsonValue) {
    var obj = formObj;
 ...
 * 控制附件
 */
AssetsDeviceInventoryDetail.prototype.setAttachMagic = function (attachMagic) {
 ...
 * 流程控制-多附件必填
 */
AssetsDeviceInventoryDetail.prototype.setAttachRequired = function (tagId, required, obj) {
    eval("this.attachRequiredMap." + tagId + " = " + required);
};
AssetsDeviceInventoryDetail.prototype.attachRequiredMap = {};
AssetsDeviceInventoryDetail.prototype.validAttachRequired = function () {
 ...
 * 多附件增删控制
 */
AssetsDeviceInventoryDetail.prototype.setAttachCanAddOrDel = function (tagId, operability, obj) {
    if (operability) {
 ...
 */
AssetsDeviceInventoryDetail.prototype.controlSelfElement = function (tagId, operability, obj) {
    if (operability) {
        if (tagId == "assetsDeviceInventorySub_editable") {//判断是否允编辑子表数据
            //获取colModel数组对象
            var columnArray = $('#assetsDeviceInventorySub').jqGrid('getGridParam', 'colModel');
            //给每个colModel属性列设置可编辑
            $.each(columnArray, function (i, item) {
                $('#assetsDeviceInventorySub').setColProp(item.name, {editable: true});
            });
        } else {
            $("#" + tagId).show();
        }
 ...
 * @param obj 参数对象
 */
AssetsDeviceInventoryDetail.prototype.controlCustomSubTableForAccess = function (tagId, operability, obj) {
 ...
 * @param obj 参数对象
 */
AssetsDeviceInventoryDetail.prototype.controlCustomSubTableForOperability = function (tagId, operability, obj) {
    var tagArr = obj.tag.split("__");
    var subTableName = tagArr[0];
    var subTableId = $("table[name=" + subTableName + "]")[0].id
    var tag = "#" + subTableId;
 ...
 * @param obj 参数对象
 */
AssetsDeviceInventoryDetail.prototype.controlCustomSubTableForRequired = function (tagId, operability, obj) {
    //获取必填字段数组
    var notnullFiledArr = this.AssetsDeviceInventorySub.notnullFiled;
    var notnullFiledCommentArr = this.AssetsDeviceInventorySub.notnullFiledComment;
    var tagValue = tagId;
 ...
 */
AssetsDeviceInventoryDetail.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            inventoryName: {
                maxlength: 255
...
function addAssetsRow(rowJson) {
    //获取待调拨台账列表中的数据
    var allData = $('#assetsDeviceInventorySub').jqGrid('getRowData');
    //判断待调拨台账列表中的数据是否为空
    ...
    $('#assetsDeviceInventorySub').jqGrid('addRow', parameters);
    newRowIndex++;
};
    ...
    switch (requestType) {
        case "productModelSelect":
            for (i = 0; i < rows.length; i++) {
                ...
                rowJson.secretLevel = row.secretLevel;
 ...
 */
function openProductModelLayer_DeviceInventory(singleSelect, requestType, callBackFn, param) {
