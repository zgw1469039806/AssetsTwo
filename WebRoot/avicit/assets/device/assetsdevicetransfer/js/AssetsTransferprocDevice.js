function AssetsTransferprocDevice(datagrid, url, formSub, dataGridColModel,searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload){
 ...
 * 初始化操作
 */
AssetsTransferprocDevice.prototype.init=function(pid){
	var _self = this;
	if(_self.isReload == "true"){
			$(_self._datagridId).jqGrid({
			url: _self.getUrl()+'getAssetsTransferprocDevice',
	    	postData : {pid : _self.pid},
	        mtype: 'POST',
	        datatype: "json",
	        toolbar: [true,'top'],
	        colModel: assetsTransferprocDeviceGridColModel, 
...
//禁止时间和日期格式手输
	...
	//初始化校验字段
				     ...
				     _self.notnullFiledComment.push("流程ID");
				     				      _self.lengthValidFiled.push("procId");
				      _self.lengthValidFiledSize.push(100);
				      _self.lengthValidFiledComment.push("流程ID");
				      ...
				      _self.lengthValidFiledComment.push("密级");
				     				    															   					     				      _self.lengthValidFiled.push("isManage");
				      _self.lengthValidFiledSize.push(10);
				      _self.lengthValidFiledComment.push("是否统管设备");
				     				    															   					     				      _self.lengthValidFiled.push("isInWorkflow");
				      _self.lengthValidFiledSize.push(10);
				      _self.lengthValidFiledComment.push("是否在流程中");
...
AssetsTransferprocDevice.prototype.insert = function(){
	var _self = this;
 ...
 * @param 
 */
AssetsTransferprocDevice.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
AssetsTransferprocDevice.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 长度和非空校验
 */
 AssetsTransferprocDevice.prototype.valid=function(){
	    var _self = this;
	    $(_self._datagridId).jqGrid('endEditCell');
  ...
  * @param pid
  */
 AssetsTransferprocDevice.prototype.save = function(pid){
 	var _self = this;
 	_self.pid = pid;
 	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');	
  ...
  * 删除
  */
AssetsTransferprocDevice.prototype.del = function(){
 	var _self = this;
 	//删除之前结束单元格
 	$(_self._datagridId).jqGrid('endEditCell');
	...
	if(isAddRow && notAddRow){
		...
		$('#cb_assetsTransferprocDevice').click();
		return false;
	}
 	if(l > 0){
 ...
 * 重载数据
 */
AssetsTransferprocDevice.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
	pid = this.pid;
 ...
 * 获取子表数据
 */
AssetsTransferprocDevice.prototype.getSubData=function(){
	var _self = this;
	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 关闭对话框
 */
AssetsTransferprocDevice.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框
 */
AssetsTransferprocDevice.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsTransferprocDevice.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsTransferprocDevice.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsTransferprocDevice.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
AssetsTransferprocDevice.prototype.clearData =function(){
