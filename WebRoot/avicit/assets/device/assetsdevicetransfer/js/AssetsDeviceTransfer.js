 ...
 */
function AssetsDeviceTransfer(datagrid, url, searchForm, dataGridColModel, searchDialog, afterInit, clickRowLoad, searchMainNames, demoMainDept_KeyWord) {
    ...
    this._formCode = "assetsDeviceTransfer"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsDeviceTransfer.prototype.init = function () {
    var _self = this;
    //初始化流程帮助类
    flowListEditor = new FlowListEditor(this._formCode);
    var issubinit = false;
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getAssetsDeviceTransfersByPage.json',
        mtype: 'POST',
        multiselect: true,
        datatype: "json",
        toolbar: [true, 'top'],
 ...
 * 添加页面
 */
AssetsDeviceTransfer.prototype.insert = function () {
    //添加流程
 ...
 * 编辑页面
 */
AssetsDeviceTransfer.prototype.modify = function () {
 ...
 * 详情页面
 */
AssetsDeviceTransfer.prototype.detail = function (id, value) {
    flowUtils.detail(id);
};
/**
 * 控件校验   规则：表单字段name与rules对象保持一致
 */
AssetsDeviceTransfer.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            createdByDept: {
                maxlength: 50
 ...
 * 保存方法
 */
AssetsDeviceTransfer.prototype.save = function (form, windowName, dataSub) {
    var _self = this;
 ...
 * 删除方法
 */
AssetsDeviceTransfer.prototype.del = function () {
    var _self = this;
 ...
 * 重载数据
 */
AssetsDeviceTransfer.prototype.reLoad = function () {
    var searchdata = {
        keyWord: null,
 ...
 * 关闭对话框
 */
AssetsDeviceTransfer.prototype.closeDialog = function (windowName) {
    layer.close(layer.getFrameIndex(windowName));
};
/**
 * 打开高级查询框
 */
AssetsDeviceTransfer.prototype.openSearchForm = function (searchDiv, par) {
    var _self = this;
    par = null;
    //if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsDeviceTransfer.prototype.searchData = function () {
 ...
 * 关键字段查询
 */
AssetsDeviceTransfer.prototype.searchByKeyWord = function () {
 ...
 * 隐藏查询框
 */
AssetsDeviceTransfer.prototype.hideSearchForm = function () {
    layer.close(searchDialogindex);
};
/**
 * 根据流程状态查询
 */
AssetsDeviceTransfer.prototype.initWorkFlow = function (status) {
    $('#bpmState').val(status);
    if (status == "start") {
        $("#assetsDeviceTransfer_modify").show();
        $("#assetsDeviceTransfer_del").show();
    } else {
        $("#assetsDeviceTransfer_modify").hide();
        $("#assetsDeviceTransfer_del").hide();
    }
    this.searchData();
};
/**
 * 清空查询条件
 */
AssetsDeviceTransfer.prototype.clearData = function () {
