 ...
 * formcode
 */
EformFlow.prototype.formcode = "assetsDeviceTransfer";
    ...
    avicAjax.ajax({
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($("#form"), result.assetsDeviceTransferDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
...
EformFlow.prototype.start = function (defineId, callback) {
	...
	//子表数据
	$('#assetsTransferprocDevice').jqGrid('endEditCell');
	var dataSub = JSON.stringify($('#assetsTransferprocDevice').jqGrid('getRowData'));

    //判断子表是否有数据，如果没有数据禁止保存提交
    var Rowdata = ($('#assetsTransferprocDevice').jqGrid('getRowData'))
    if(Rowdata.length<1){
        layer.msg('请选择设备！');
        var maskId = layer.load();
        layer.close(maskId);    //去掉转圈等待
    }else {
        avicAjax.ajax({
                ...
                if (result.flag == "success") {
                    ...
                    if (files.length != 0) {
                        ...
                        afterUploadEvent = function () {
                            ...
                            $('#assetsTransferprocDevice').jqGrid('setGridParam', {datatype: 'json', postData: searchdata}).trigger("reloadGrid");
