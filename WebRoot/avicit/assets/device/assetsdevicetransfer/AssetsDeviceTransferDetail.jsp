...
<%@ page import="avicit.platform6.api.sysuser.SysUserDeptPositionAPI" %>
<%@ page import="avicit.platform6.api.sysuser.impl.SysUserDeptPositionAPImpl" %>
<%@ page import="avicit.platform6.core.spring.SpringFactory" %>
<%@ page import="avicit.platform6.api.sysuser.impl.SysDeptAPImpl" %>
<%@ page import="avicit.platform6.api.sysuser.dto.SysUserDeptPosition" %>
<%
...
<html>
<head>
    <!-- ControllerPath = "assets/device/assetsdevicetransfer/assetsDeviceTransferController/operation/toDetailJsp" -->
...
<div id="formTab" style="display: none">
    <p style="text-align: center;">
        <span style="font-size: 32px; text-align: center;">设备移交申请单</span>
    </p>
    <!-- 业务表单 Start -->
    <form id='form'>
        ...
        <table class="form_commonTable">
            <tr>
                <th width="10%">
                    <label for="procId">移交单号:</label></th>
                <td width="15%">
            ...
            <tr>
                ...
                <td width="15%">
                    <div class="input-group  input-group-sm">
                        ...
                        <input class="form-control" placeholder="请选择用户" type="text"
                               id="receiverAlias" name="receiverAlias">
                        <span class="input-group-addon">
										         <i class="glyphicon glyphicon-user"></i>
										      </span>
                    </div>
                ...
                <td width="15%">
                    <div class="input-group  input-group-sm">
                        ...
                        <input class="form-control" placeholder="请选择部门" type="text"
                               id="receiverDeptAlias" name="receiverDeptAlias">
                        <span class="input-group-addon">
										        <i class="glyphicon glyphicon-equalizer"></i>
										      </span>
                    </div>
            ...
            <tr>
                ...
                <td width="90%" colspan="7">
                                        <textarea class="form-control input-sm" rows="5" type="text"
                                                  name="transferReason" id="transferReason"/> </textarea>
                </td>
            </tr>
            <!--
            <tr>
            ...
            <tr>
                ...
                <td width="15%">
                    <input class="form-control input-sm" type="text" name="receiverTel"
                           id="receiverTel"/>
                </td>
                <th width="10%">
                    <label for="receivePositionId">接收安装地点ID:</label></th>
                <td width="15%">
                    <input class="form-control input-sm" type="text" name="receivePositionId"
            ...
            <tr>
                <th width="99%" colspan="<%=4 * 2 %>">
                    <div id="toolbar_AssetsTransferprocDevice" class="toolbar">
                        <div class="toolbar-left">
                            ...
                            <a class="btn btn-default form-tool-btn btn-sm bpm_self_class"
                               ...
                               name="assetsTransferprocDevice_editable"
                               id="assetsTransferprocDevice_editable">是否可编辑</a>
                        </div>
                    </div>
                    <table id="assetsTransferprocDevice" name="ASSETS_TRANSFERPROC_DEVICE"
                           class="customform_subtable_bpm_auth"></table>
                    <pt6:h5resource label="统一编号" name="unifiedId"
                                    ref_table="ASSETS_TRANSFERPROC_DEVICE"></pt6:h5resource>
                    ...
                    <pt6:h5resource label="设备规格" name="deviceSpec"
                                    ref_table="ASSETS_TRANSFERPROC_DEVICE"></pt6:h5resource>
                    <pt6:h5resource label="责任人" name="ownerId"
                                    ref_table="ASSETS_TRANSFERPROC_DEVICE"></pt6:h5resource>
                    <pt6:h5resource label="责任人部门" name="ownerDept"
                                    ref_table="ASSETS_TRANSFERPROC_DEVICE"></pt6:h5resource>
                    <pt6:h5resource label="使用人" name="userId"
                                    ref_table="ASSETS_TRANSFERPROC_DEVICE"></pt6:h5resource>
                    <pt6:h5resource label="使用人部门" name="userDept"
                                    ref_table="ASSETS_TRANSFERPROC_DEVICE"></pt6:h5resource>
                    ...
                    <pt6:h5resource label="密级" name="secretLevel"
                                    ref_table="ASSETS_TRANSFERPROC_DEVICE"></pt6:h5resource>
                    <pt6:h5resource label="是否统管设备" name="isManage"
                                    ref_table="ASSETS_TRANSFERPROC_DEVICE"></pt6:h5resource>
                    <pt6:h5resource label="是否在流程中" name="isInWorkflow"
                                    ref_table="ASSETS_TRANSFERPROC_DEVICE"></pt6:h5resource>
                </th>
            </tr>
...
<script type="text/javascript"
        src="avicit/assets/device/assetsdevicetransfer/js/AssetsDeviceTransferDetail.js"></script>
<script type="text/javascript" src="avicit/assets/device/assetsdevicetransfer/js/AssetsTransferprocDevice.js"></script>
<script type="text/javascript" src="avicit/assets/device/assetsdevicetransfer/js/SelfStyleLayout.js"></script>
<!-- 自动编码的js -->
...
<script type="text/javascript">
    ...
    function initOnceInAdd() {
        avicAjax.ajax({
            url: "platform/assets/device/assetsdevicetransfer/assetsDeviceTransferController/getLookUpCode",
            type: 'post',
            dataType: 'json',
            async: false,
            success: function (r) {
    ...
    var assetsTransferprocDeviceGridColModel = null;
    $(document).ready(function () {
        ...
        ***********************************************************************************/
        ...
        //自动生成设备移交编号
        var autoCode = new AutoCode("DEVICE_TRANSFER_PROC", "procId", false, "autoCodeValue", "123");
        ...
        assetsTransferprocDeviceGridColModel = [
            ...
            //     width: 150,
            //     edittype: 'custom',
            //     editoptions: {custom_element: userElem, custom_value: userValue, forId: 'userId', hidden: true}
            // }
            ...
            //     width: 150,
            //     edittype: 'custom',
            //     editoptions: {custom_element: deptElem, custom_value: deptValue, forId: 'userDept', hidden: true}
            // }
        ...
        var assetsTransferprocDevice = new AssetsTransferprocDevice('assetsTransferprocDevice', surl,
            "formSub",
            assetsTransferprocDeviceGridColModel,
            'searchDialogSub', pid, searchSubNames, 'assetsTransferprocDevice_keyWord', isReload);
        //创建业务操作JS
        var assetsDeviceTransferDetail = new AssetsDeviceTransferDetail('form', assetsTransferprocDevice);
        //创建流程操作JS
        new FlowEditor(assetsDeviceTransferDetail);
        ...
        //绑定表单验证规则
        assetsDeviceTransferDetail.formValidate($('#form'));
        //初始化附件上传组件
        ...
        // $('#receiverAlias').on('blur', function (e) {
        //  var receiver =  $("#receiver").val();
        //  // document.form.receiver.value = receiver;
        //     var deptName = null;
        //     var deptId = null;
        //         $("receiverDeptAlias").val(deptName);
        //         $("receiverDeptAlias").val(deptId);
        //     nullInput(e);
        // });
    ...
    window.onload = function () {
        $('#cb_assetsTransferprocDevice').attr("disabled", false);
    }
</script>
</body>
</html>