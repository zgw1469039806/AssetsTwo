 ...
 */
function AssetsReconstructionCheck(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "assetsReconstructionCheck"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsReconstructionCheck.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getAssetsReconstructionChecksByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 编辑页面
 */
AssetsReconstructionCheck.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsReconstructionCheck.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsReconstructionCheck.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
AssetsReconstructionCheck.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  		     			    				   				   reconstructionId:{
						required:true,
 ...
 * 保存方法
 */
AssetsReconstructionCheck.prototype.save=function(form,windowName,callback){
	var _self = this;
	avicAjax.ajax({
 ...
 * 重载数据
 */
AssetsReconstructionCheck.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
AssetsReconstructionCheck.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 根据流程状态查询
 */
AssetsReconstructionCheck.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsReconstructionCheck_modify").show();
		$("#assetsReconstructionCheck_del").show();
	}else{
		$("#assetsReconstructionCheck_modify").hide();
		$("#assetsReconstructionCheck_del").hide();
	}
	this.searchData();
};
/**
