function AssetsDeviceMcheckPlan(datagrid, url, formSub, dataGridColModel,searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload){
 ...
 * 初始化操作
 */
AssetsDeviceMcheckPlan.prototype.init=function(pid){
	var _self = this;
	if(_self.isReload == "true"){
			$(_self._datagridId).jqGrid({
			url: _self.getUrl()+'getAssetsDeviceMcheckPlan',
	    	postData : {pid : _self.pid},
	        mtype: 'POST',
	        datatype: "json",
	        toolbar: [true,'top'],
	        colModel: assetsDeviceMcheckPlanGridColModel, 
		...
		$(_self._datagridId).jqGrid({
			...
			cellEdit:false,
	        cellsubmit: 'clientArray',
			pager: "assetsDeviceMcheckPlanPager",
	    });
	}else{
		$(_self._datagridId).jqGrid({
	    	url: _self.getUrl()+'getAssetsDeviceMcheckPlan',
	...
	//初始化校验字段
				     ...
				     _self.notnullFiledComment.push("设备名称");
				      ...
				      _self.lengthValidFiledComment.push("保养方式");
				     				    															   					     				      _self.lengthValidFiled.push("maintainCycle");
				      _self.lengthValidFiledSize.push(50);
				      _self.lengthValidFiledComment.push("保养周期");
				     				    															   					     				     				    															   					     				     				    															   					     				      _self.lengthValidFiled.push("procId");
				      _self.lengthValidFiledSize.push(50);
				      _self.lengthValidFiledComment.push("年度保养表ID");
				     				    															   					     				      _self.lengthValidFiled.push("maintainId");
				      _self.lengthValidFiledSize.push(50);
				      _self.lengthValidFiledComment.push("台账年度保养表ID");
...
AssetsDeviceMcheckPlan.prototype.insert = function(){
	var _self = this;
 ...
 * @param 
 */
AssetsDeviceMcheckPlan.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
AssetsDeviceMcheckPlan.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 长度和非空校验
 */
 AssetsDeviceMcheckPlan.prototype.valid=function(){
	    var _self = this;
	    $(_self._datagridId).jqGrid('endEditCell');
  ...
  * @param pid
  */
 AssetsDeviceMcheckPlan.prototype.save = function(pid){
 	var _self = this;
 	_self.pid = pid;
 	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');	
  ...
  * 删除
  */
AssetsDeviceMcheckPlan.prototype.del = function(){
 	var _self = this;
 	//删除之前结束单元格
 	$(_self._datagridId).jqGrid('endEditCell');
	...
	if(isAddRow && notAddRow){
		...
		$('#cb_assetsDeviceMcheckPlan').click();
		return false;
	}
 	if(l > 0){
 ...
 * 重载数据
 */
AssetsDeviceMcheckPlan.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
	pid = this.pid;
 ...
 * 获取子表数据
 */
AssetsDeviceMcheckPlan.prototype.getSubData=function(){
	var _self = this;
	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 关闭对话框
 */
AssetsDeviceMcheckPlan.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框
 */
AssetsDeviceMcheckPlan.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询
 */
AssetsDeviceMcheckPlan.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsDeviceMcheckPlan.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsDeviceMcheckPlan.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
AssetsDeviceMcheckPlan.prototype.clearData =function(){
