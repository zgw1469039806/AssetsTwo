 ...
 * formcode
 */
EformFlow.prototype.formcode = "assetsDeviceMcheckProc";
    ...
    avicAjax.ajax({
        ...
        success: function (result) {
            if (result.flag == "success") {
                _self.setForm($("#form"), result.assetsDeviceMcheckProcDTO);
            } else {
                layer.msg('数据加载失败！');
            }
        }
...
EformFlow.prototype.start = function (defineId, callback) {
	...
	//子表数据
	$('#assetsDeviceMcheckPlan').jqGrid('endEditCell');
	var dataSub = JSON.stringify($('#assetsDeviceMcheckPlan').jqGrid('getRowData'));
    ...
    avicAjax.ajax({
		...
		success: function (result) {
			if (result.flag == "success") {
					...
					$('#assetsDeviceMcheckPlan').jqGrid('setGridParam', {datatype: 'json', postData: searchdata}).trigger("reloadGrid");
