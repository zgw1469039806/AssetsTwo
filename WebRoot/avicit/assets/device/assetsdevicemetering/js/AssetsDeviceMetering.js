 ...
 */
function AssetsDeviceMetering(datagrid, url, searchForm, dataGridColModel,searchDialog, afterInit, clickRowLoad, searchMainNames,demoMainDept_KeyWord) {
	...
	this._formCode = "deviceMetering1"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsDeviceMetering.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	var issubinit = false;
	$(_self._datagridId).jqGrid({
				url : _self.getUrl() + 'getAssetsDeviceMeteringsByPage.json',
				mtype : 'POST',
				multiselect : true,
				datatype : "json",
				toolbar : [ true, 'top' ],
...
AssetsDeviceMetering.prototype.insert = function () {
	//添加流程
	// flowListEditor.addFlow();
	_bpm_listeditor_start("4028808e745292b5017452b0533a6ce4-1", "设备计量")
};
AssetsDeviceMetering.prototype.insert_staff = function () {
	//添加流程
	// flowListEditor.addFlow();
	_bpm_listeditor_start("4028808e745cd8f601745d0aace80dba-1", "设备计量")
};

/**
 * 编辑页面
 */
AssetsDeviceMetering.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsDeviceMetering.prototype.detail = function(id, value) {
	flowUtils.detail(id);
};
/**
 * 控件校验   规则：表单字段name与rules对象保持一致
 */
AssetsDeviceMetering.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  				  				  				  				  		     			    				   			       applicantIdAlias:{
						required:true,
 ...
 * 保存方法
 */
AssetsDeviceMetering.prototype.save=function(form,windowName,dataSub){
	var _self = this;
 ...
 * 删除方法
 */
AssetsDeviceMetering.prototype.del=function(){
    var _self = this;
 ...
 * 重载数据
 */
AssetsDeviceMetering.prototype.reLoad=function(){
    var searchdata = {
		keyWord : null,
 ...
 * 关闭对话框
 */
AssetsDeviceMetering.prototype.closeDialog=function(windowName){
	layer.close(layer.getFrameIndex(windowName));
};
/**
 * 打开高级查询框
 */
AssetsDeviceMetering.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询	
 */
AssetsDeviceMetering.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsDeviceMetering.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
AssetsDeviceMetering.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 根据流程状态查询
 */
AssetsDeviceMetering.prototype.initWorkFlow = function(status) {
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsDeviceMetering_modify").show();
		$("#assetsDeviceMetering_del").show();
	}else{
		$("#assetsDeviceMetering_modify").hide();
		$("#assetsDeviceMetering_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsDeviceMetering.prototype.clearData =function(){
