function DynDeviceTool(datagrid, url, formSub, dataGridColModel,searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord, isReload){
 ...
 * 初始化操作
 */
DynDeviceTool.prototype.init=function(pid){
	var _self = this;
	if(_self.isReload == "true"){
			$(_self._datagridId).jqGrid({
			url: _self.getUrl()+'getDynDeviceTool',
	    	postData : {pid : _self.pid},
	        mtype: 'POST',
	        datatype: "json",
	        toolbar: [true,'top'],
	        colModel: dynDeviceToolGridColModel, 
...
//禁止时间和日期格式手输
	...
	//初始化校验字段
    																											   					     				      _self.lengthValidFiled.push("procedureFile");
				      _self.lengthValidFiledSize.push(50);
				      _self.lengthValidFiledComment.push("计量文件规范号");
				     				    															   					     				      _self.lengthValidFiled.push("fkSubColId");
				      _self.lengthValidFiledSize.push(50);
				      _self.lengthValidFiledComment.push("外键");
...
DynDeviceTool.prototype.insert = function(){
	var _self = this;
 ...
 * @param 
 */
DynDeviceTool.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
DynDeviceTool.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 长度和非空校验
 */
 DynDeviceTool.prototype.valid=function(){
	    var _self = this;
	    $(_self._datagridId).jqGrid('endEditCell');
  ...
  * @param pid
  */
 DynDeviceTool.prototype.save = function(pid){
 	var _self = this;
 	_self.pid = pid;
 	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');	
  ...
  * 删除
  */
DynDeviceTool.prototype.del = function(){
 	var _self = this;
 	//删除之前结束单元格
 	$(_self._datagridId).jqGrid('endEditCell');
 ...
 * 重载数据
 */
DynDeviceTool.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
	pid = this.pid;
 ...
 * 获取子表数据
 */
DynDeviceTool.prototype.getSubData=function(){
	var _self = this;
	$(_self._datagridId).jqGrid('endEditCell');
 	var data = $(_self._datagridId).jqGrid('getRowData');
 ...
 * 关闭对话框
 */
DynDeviceTool.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框
 */
DynDeviceTool.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
 ...
 * 高级查询
 */
DynDeviceTool.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
DynDeviceTool.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
DynDeviceTool.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
DynDeviceTool.prototype.clearData =function(){
