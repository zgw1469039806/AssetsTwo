 ...
 */
function DocumentPackage(datagrid, url, searchForm, dataGridColModel, searchDialog, afterInit, clickRowLoad, searchMainNames, demoMainDept_KeyWord) {
 ...
 * 回车查询
 */
DocumentPackage.prototype.init = function () {
    var _self = this;
    var issubinit = false;
    $(_self._datagridId).jqGrid({
        url: _self.getUrl() + 'getDocumentPackagesByPage.json',
        mtype: 'POST',
        multiselect: true,
        datatype: "json",
        toolbar: [true, 'top'],
 ...
 * 添加页面
 */
DocumentPackage.prototype.insert = function () {
 ...
 * 编辑页面
 */
DocumentPackage.prototype.modify = function () {
 ...
 * 详情页面
 */
DocumentPackage.prototype.detail = function (id) {
 ...
 */
DocumentPackage.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            createdBy: {
                required: true,
 ...
 * 保存方法
 */
DocumentPackage.prototype.save = function (form, subform, id, callback) {
    var _self = this;
    avicAjax.ajax({
        url: _self.getUrl() + "save",
        data: {
            data: JSON.stringify(serializeObject(form)),
            subdata: JSON.stringify(serializeObject(subform))
        },
        type: 'post',
        dataType: 'json',
 ...
 * 删除方法
 */
DocumentPackage.prototype.del = function () {
    var _self = this;
    var rows = $(_self._datagridId).jqGrid('getGridParam', 'selarrrow');
    var ids = [];
    var l = rows.length;
 ...
 * 重载数据
 */
DocumentPackage.prototype.reLoad = function () {
 ...
 * 关闭对话框
 */
DocumentPackage.prototype.closeDialog = function (id) {
    if (id == "insert") {
 ...
 * 打开高级查询框
 */
DocumentPackage.prototype.openSearchForm = function (searchDiv, par) {
    var _self = this;
    par = null;
 ...
 * 去后台查询
 */
DocumentPackage.prototype.searchData = function (dataGridColModel) {
 ...
 * 关键字段查询
 */
DocumentPackage.prototype.searchByKeyWord = function () {
 ...
 * 隐藏查询框
 */
DocumentPackage.prototype.hideSearchForm = function () {
    layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
DocumentPackage.prototype.clearData = function () {
 ...
 * 返回主表id
 */
DocumentPackage.prototype.getSelectID = function () {
 ...
 * 更新列表视图
 */
function UpdateTableHead(dataGridColModel){
    var modelList = [];
    modelList.push('DocumentPackage');
    modelList.push('MyView');
            ...
            if (event.which == 1) {
                ...
                if (targetSrc.indexOf('<span class="ui-jqgrid-resize ui-jqgrid-resize-ltr"') == 0) {
                    ...
                    document.onmouseup = function (event) {
                        ...
                        if (firstDragWidth) {
                            UpdateTableHead(dataGridColModel);
                    ...
                    document.onmouseup = function (event) {
                        ...
                        //元素移动
                            ...
                            if (dragIndex > newIndex) {
                                ...
                                jqGridObject = new DocumentPackage(tableId, url, searchFormId, dataGridColModel, searchDialogId,
                                    function (pid) {
