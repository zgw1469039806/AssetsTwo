 ...
 *
 */
function DynSdCollecUpld(datagrid, url, searchD, form, keyWordId, searchNames, dataGridColModel) {
    ...
    this._formCode = "dynsdcollecupload"; //表单的code，启动流程时需要
...
//初始化操作
DynSdCollecUpld.prototype.init = function () {
    var _self = this;
    //初始化流程帮助类
    flowListEditor = new FlowListEditor(this._formCode);
    $(_self._datagridId).jqGrid({
        url: this.getUrl() + 'getDynSdCollecUpldsByPage.json',
        mtype: 'POST',
        datatype: "json",
...
//添加页面
DynSdCollecUpld.prototype.insert = function () {
    //添加流程
    flowListEditor.addFlow();
};
//编辑页面
DynSdCollecUpld.prototype.modify = function () {
...
//详细页
DynSdCollecUpld.prototype.detail = function (id, value) {
    flowUtils.detail(id);
};
//打开高级查询框
DynSdCollecUpld.prototype.openSearchForm = function (searchBtn, contentWidth, contentHeight) {
    var _self = this;
    var _resizeFunc;
...
DynSdCollecUpld.prototype.formValidate = function (form) {
    form.validate({
        rules: {
            author: {
                maxlength: 50
...
//保存功能
DynSdCollecUpld.prototype.save = function (form, windowName, callback) {
    var _self = this;
    avicAjax.ajax({
...
//删除
DynSdCollecUpld.prototype.del = function () {
...
//重载数据
DynSdCollecUpld.prototype.reLoad = function () {
    var searchdata = {
        keyWord: null,
...
//关闭对话框
DynSdCollecUpld.prototype.closeDialog = function (windowName, msg) {
    layer.close(layer.getFrameIndex(windowName));
    if (msg && msg !== '') {
        layer.msg(msg);
    }
...
//高级查询
DynSdCollecUpld.prototype.searchData = function () {
    var searchdata = {
        keyWord: null,
        param: JSON.stringify(serializeObject($(this._formId)))
    }
...
//关键字段查询
DynSdCollecUpld.prototype.searchByKeyWord = function () {
...
//根据状态查询
DynSdCollecUpld.prototype.initWorkFlow = function (status) {
    $('#bpmState').val(status);
    if (status == "start") {
        $("#dynSdCollecUpld_modify").show();
        $("#dynSdCollecUpld_del").show();
    } else {
        $("#dynSdCollecUpld_modify").hide();
        $("#dynSdCollecUpld_del").hide();
    }
    this.searchData();
};
/*清空查询条件*/
DynSdCollecUpld.prototype.clearData = function () {
