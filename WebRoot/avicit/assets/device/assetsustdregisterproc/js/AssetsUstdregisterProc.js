 ...
 */
function AssetsUstdregisterProc(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "assetsUstdregisterProc"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsUstdregisterProc.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getAssetsUstdregisterProcsByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsUstdregisterProc.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
AssetsUstdregisterProc.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsUstdregisterProc.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
 ...
 * 打开高级查询框
 */
AssetsUstdregisterProc.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
AssetsUstdregisterProc.prototype.formValidate = function (form) {
	form.validate({
		rules: {
			registerNo: {
				required: true,
 ...
 * 保存方法
 */
AssetsUstdregisterProc.prototype.save=function(form,windowName,callback){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsUstdregisterProc.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
AssetsUstdregisterProc.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
AssetsUstdregisterProc.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
AssetsUstdregisterProc.prototype.searchData =function(dataGridColModel){
 ...
 * 关键字段查询
 */
AssetsUstdregisterProc.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
AssetsUstdregisterProc.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsUstdregisterProc_modify").show();
		$("#assetsUstdregisterProc_del").show();
	}else{
		$("#assetsUstdregisterProc_modify").hide();
		$("#assetsUstdregisterProc_del").hide();
	}
	this.searchData();
};
 ...
 * 清空查询条件
 */
AssetsUstdregisterProc.prototype.clearData =function(){
 ...
 * 打开视图选择框
 */
AssetsUstdregisterProc.prototype.openSelectView = function (selectBtn, contentWidth, contentHeight) {
	var _self = this;
	var _resizeFunc;
 ...
 * 添加视图页面
 */
AssetsUstdregisterProc.prototype.addView = function () {
	this.insertIndex = layer.open({
		...
		skin: 'bs-modal', // bootstrap 风格皮肤 需加载skin
		maxmin: false, //开启最大化最小化按钮
		content: 'assets/device/usertablemodel/userTableModelController/operation/Add/AssetsUstdregisterProc/null'
	});
};
 ...
 * 保存视图方法
 */
AssetsUstdregisterProc.prototype.saveView = function(form,id){
	var _self = this;
	avicAjax.ajax({
...
function UpdateTableHead(dataGridColModel, currentView){
	var modelList = [];
	modelList.push('AssetsUstdregisterProc');
 ...
 * 为表头元素添加拖拽、鼠标右键事件
 */
function BuildDrag(tableId, url, searchDialogId, searchFormId, keywordId, searchNames, dataGridColModel, jqGridObject, toolBar){
					...
					document.onmouseup = function (event) {
						...
						//元素移动
							...
							if (dragIndex > newIndex) {
								...
								jqGridObject = new AssetsUstdregisterProc(tableId, url, searchDialogId, searchFormId, keywordId, searchNames, dataGridColModel);

								//添加表头拖拽
								BuildDrag(tableId, url, searchDialogId, searchFormId, keywordId, searchNames, dataGridColModel, jqGridObject, toolBar);
