 ...
 */
function AssetsFrmlossProc(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "assetsFrmlossProc"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
AssetsFrmlossProc.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getAssetsFrmlossProcsByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
AssetsFrmlossProc.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
AssetsFrmlossProc.prototype.modify=function(){
 ...
 * 详情页面
 */
AssetsFrmlossProc.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
AssetsFrmlossProc.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
AssetsFrmlossProc.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  		     			    				   				   frmlossNo:{
						required:true,
 ...
 * 保存方法
 */
AssetsFrmlossProc.prototype.save=function(form,windowName,callback){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
AssetsFrmlossProc.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
AssetsFrmlossProc.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
AssetsFrmlossProc.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
AssetsFrmlossProc.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
AssetsFrmlossProc.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
AssetsFrmlossProc.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#assetsFrmlossProc_modify").show();
		$("#assetsFrmlossProc_del").show();
	}else{
		$("#assetsFrmlossProc_modify").hide();
		$("#assetsFrmlossProc_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
AssetsFrmlossProc.prototype.clearData =function(){
