...
SysProfileValue.prototype.loadById=function(pid){
	...
	(this._datagrid&&this._datagrid.datagrid({url : this.getUrl() + "/" + pid+".json"}))||this.init(pid);
};
...
SysProfileValue.prototype.openSqlSelector=function(index,field,value){
	...
	this._sqlDialog = new CommonDialog("sql","700","400",this.getUrl()+'/operation/choose/sql/'+index,"选择页面",false,true,false);
...
SysProfileValue.prototype.openSelector=function(index, field, value){
	...
	if(levelCode == 1){//地点
		this._siteDialog = new CommonDialog("site","700","400",this.getUrl()+'/operation/choose/site/'+index+'/'+nowRow.levelValue,"地点选择页面",false,true,false);
		this._siteDialog.show()
		return true;
	}else if(levelCode== 2){//应用
		this._appDialog = new CommonDialog("app","700","400",this.getUrl()+'/operation/choose/app/'+index+'/'+nowRow.levelValue,"应用选择页面",false,true,false);
...
SysProfileValue.prototype.save=function(pid){
	...
	$.ajax({
		 url:'platform/cc/sysprofile/operation/save/profileValue.json',
	...
	var l =rows.length;
	  ...
	  $.messager.confirm('请确认','您确定要删除当前所选的数据？',function(b){
		 if(b){
			 ...
			 $.ajax({
				 url:'platform/cc/sysprofile/operation/delete/profileValue.json',
