...
SysApplication.prototype.init=function(){
	...
	this._datagrid=$(this._datagridId).datagrid({
		...
		onAfterEdit :function(rowIndex, rowData, changes){
			...
			for(;l--;){
				var row = rows[l];
				if(row.applicationCode.toUpperCase()==rowData.applicationCode.toUpperCase() &&dg.datagrid('getRowIndex',row)!==rowIndex){
					alert("应用编码必须唯一,不区分大小写!");
...
SysApplication.prototype.save=function(){
	...
	var rows = this._datagrid.datagrid('getChanges'),l =rows.length;
	
	if(l > 0){
		for(;l--;){
			if(rows[l].basepath.length>90){
				alert("应用名称【"+rows[l].applicationName+"】,应用地址超长!");
				this._indexEditing=-1;
				return false;
			}
		}
