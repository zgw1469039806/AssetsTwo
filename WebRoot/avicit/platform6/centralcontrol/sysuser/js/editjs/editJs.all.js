...
function saveMapDept(id){
	var rows = dgMapDept.datagrid('getRows'),l = rows.length,length=l,row,cache={};
	for(;l--;){
		row =rows[l];
		if(!row.deptName){
			$.messager.show({
				 title : '提示',
				 msg : '第'+(l+1)+'条数据中,部门名称不能为空!'
			 });
			return false;
		}
		if(!row.positionName){
...
function editDeptPos(index, field,value){
	if (endEditing()){
		 var nowRow=$('#dgMapDept').datagrid('getRows')[index];
		 if(field ==='deptName'){
			 comSelect = new GridCommonSelector("dept",'dgMapDept',index,"deptId",{targetId:'deptId'},function(rowIndex,resultData){
					var obj ={
							deptId: resultData.deptId,
							deptName: resultData.deptName,
							validFlag:resultData.deptId?1:0,
							orgIdentity:resultData.orgIdentity
							};
					if(nowRow.orgIdentity && nowRow.orgIdentity !==resultData.orgIdentity){
						obj.positionId='';
						obj.positionName='';
					}
				 	$('#dgMapDept').datagrid('updateRow',{
							index: index,
							row:obj
						});
					},500,400,null,1,null,null,"2");//
			 ...
			 comSelect = new GridCommonSelector("position",'dgMapDept',index,"deptId",{targetId:'positionId'},function(rowIndex,resultData){
					$('#dgMapDept').datagrid('updateRow',{
							index: index,
							row: {
								positionId: resultData.positionId,
								positionName: resultData.positionName
							}
						});
					},JSON.stringify({orgIdentity:nowRow.orgIdentity}),400,400,1,null,null,"-2");//
				comSelect.init(nowRow);
		 }
	 }
};
...
function callBackRole(index,data){
	...
	$(ed.target).combobox("setValue",data[0].roleName);
	...
	$('#datagrid-row-r1-2-'+index).find(".datagrid-cell-c1-roleName").text(data[0].roleName);
};
/*重新刷新用户角色关系数据*/
