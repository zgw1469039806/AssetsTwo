...
function progress(options){
    ...
    this.z = (options && options.zindex)?parseFloat(options.zindex) : parseFloat(this.options.zindex);
    this.showBack = (options && options.showBack != undefined)?options.showBack : this.options.showBack;
...
progress.prototype.init = function(){
    ...
    this.winBg = document.createElement('div');

    addClass(this.proBox,'proBox');
    addClass(this.proBg,'proBg');
    addClass(this.proPre,'proPre');
    addClass(this.proFont,'proFont');
    addClass(this.winBg,'proWinBack');

    this.winBg.setAttribute("style","width:100%;height:100%;position:fixed;z-index:"+this.z+";top:0;left:0;background: rgba(0, 0, 0, 0.25);filter:Alpha(opacity=25);");

    this.proBox.setAttribute("style","width:"+this.w+"px; height:"+this.h+"px; position:fixed;top:50%;left:50%;margin-left:-"+this.w/2+"px;margin-top:-"+this.h/2+"px; overflow:hidden; box-shadow:0 0 5px #FFFFFF; -moz-box-shadow:0 0 5px #FFFFFF; -webkit-box-shadow:0 0 5px #FFFFFF; -o-box-shadow:0 0 5px #FFFFFF;border-radius:5px;-webkit-border-radius:5px;z-index:"+(this.z+1));
...
progress.prototype.showBody = function(){
    var body = this.proBox;
    var bg = this.winBg;
    if(this.showBack){
...
progress.prototype.del = function(){
    var that = this;
    setTimeout(function(){
		document.body.removeChild(that.proBox);
		document.body.removeChild(that.winBg);
        //that.proBox.remove();
        //that.winBg.remove();
    },500);
};
