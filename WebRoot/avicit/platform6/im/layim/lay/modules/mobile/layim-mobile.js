...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ if(d.mine && data.belongTo != d.mine.id){  '+ (options.type === 'history'?"return;":"") +' } length++; }}'
  ...
  var elemTpl = ['<div class="layui-layim">'
    ...
    ,'<div class="layim-tab-content">'
      ,'<ul class="layim-list-top">'
        ...
        ,'<li layim-event="newFriend"><i class="layui-icon">&#xe654;</i>添加朋友<i class="layim-new" id="LAY_layimNewFriend"></i></li>'
        ,'<li layim-event="newGroup"><i class="layui-icon">&#xe613;</i>添加群组<i class="layim-new" id="LAY_layimNewGroup"></i></li>'
        ,'<li layim-event="newFriends"><i class="layui-icon">&#xe613;</i>新的朋友<i class="layim-new" id="LAY_layimNewFriends"></i></li>'
        ,'{{# } if(d.base.isgroup){ }}'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ...
      ,'<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'<span class="layui-icon layim-tool-image" title="上传图片" layim-event="image">&#xe60d;<input type="file" name="file" accept="avicit/platform6/im/image/*"></span>'
        ,'{{# }; }}'
  ...
  var cache = {message: {}, chat: []}, init = function(options){
    ...
    ,obj = {
      ...
      var local = layui.data('layim-mobile')[mine.id] || {}, obj = {
        ...
        ,newFriends:data.newFriends	|| [] //好友请求信息
      };
  ...
  var addList = function(data){
    ...
    if(cache[data.type]){
      if(data.type === 'friend'){
        ...
        if(has) return layer.msg(/*'您已是 ['+ (data.groupname||'') +'] 的群成员'*/"群组创建成功",{anim: 6});
  ...
  var anim = 'layui-anim-upbit', events = { 
    ...
    ,newFriends: function(){
      popPanel({
        title: '新的朋友'
        ,tpl: ['<div class="layui-layim-list layim-list-newFriends">'
          ,listTpl({
            type: 'newFriends'
            ,item: 'd.newFriends'
          })
    ...
    ,detail: function(){
      ...
      Chat.mucQueryOccupants(thatChat.data.id, 
      function(arr){
          popPanel({
            title: '群组人员列表'
            ,tpl: ['<div class="layui-layim-list layim-list-friend">'
            ,listTpl({
                type: 'friend'
                ,item: 'd.data'
              })
