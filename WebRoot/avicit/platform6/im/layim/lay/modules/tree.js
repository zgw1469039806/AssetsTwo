...
layui.define('jquery', function(exports){
  ...
  //树节点解析
  Tree.prototype.tree = function(elem, children,pid){
    var that = this, options = that.options
    var nodes = children || options.nodes;
    layui.each(nodes, function(index, item){
      var hasChild = item.existsChilds;
    ...
    if(pid){
      var newNodes = that.addChildren(options.nodes,nodes,pid);
...
//点击节点回调
Tree.prototype.addChildren = function(nodes,childrenNodes,pid){
  if(pid){
    var that = this;
    layui.each(nodes, function(index, item){
      if(item.id==pid){
        ...
        if("children" in item){
            var children = that.addChildren(item.children,childrenNodes,pid);
    ...
    var open = function(){
      if(elem.data('spread')){
       ...
       // a.find('.layui-icon').html(icon.branch[0]);
         ...
         //触发点击节点回调
       typeof options.open === 'function' && options.open(that,item, elem); 
