$(function() {
	...
	var myChart2 = echarts.init(document.getElementById('netMonitor'));
	var now = new Date();
	var res = [];
	var res2 = [];
	var len = 20;
			...
			series : [
					{
						name : '下行速率',
						type : 'line',
						data : [ 30, 25, 35, 45, 30, 35, 15, 23, 10, 40, 60, 55,
								16, 43, 45, 64, 59, 41, 37, 0 ]
					},
					{
						name : '上行速率',
						type : 'line',
						data : [ 25, 30, 25, 35, 20, 29, 25, 29, 18, 30, 50, 45,
								24, 45, 34, 56, 35, 41, 37, 24 ]
					}]
		};
	...
	$.ajax({
	 ...
	 clearInterval(timeTicket);
 	 var timeTicket = setInterval(function() {
		...
		$.ajax({
			type : "POST",
			url : "platform/modules/system/monitor/usage",
			async : false,
			dataType : 'json',
			success : function(json) {
				// 动态数据接口 addData
				...
				myChart2.addData([
				[ 0, // 系列索引
				 json.netRxUsage, // 新增数据
				false, // 新增数据是否从队列头部插入
				false// 是否增加队列长度，false则自定删除原有数据，队头插入删队尾，队尾插入删队头
				],
				[ 1, // 系列索引
				 json.netTxUsage, // 新增数据
				false, // 新增数据是否从队列头部插入
				false,
				axisData
				]]);
