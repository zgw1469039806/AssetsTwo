...
<script src="avicit/platform6/mda_easyui/mdasearchconfig/js/JSon.js" type="text/javascript"></script>
...
<script type="text/javascript">
    function ok_onclick(start){
       ...
       // var start = 0;
        var rows = 20;
       var url="mdaSearchEasyUIController/operation/search?start="+start+"&rows="+rows+"&searchword="+searchwords;
       console.log('----------url:'+url);
    ...
    function supic(jsonData){
        ...
        alert(data);
        var URL = "mdaSearchEasyUIController/operation/show/3/"+data;
  ...
  //进入索引数据管理按钮绑定事件
    function doJump4URL(URL) {
    	var indexMana = new CommonDialog("insert","1200","600",URL,"自定义URL跳转",false,true,false);
    	indexMana.show();
    }	
  //进入索引数据管理按钮绑定事件
    function solrconfigFlag(solrcoreID) {
	  
	  var URL = 'platform6/mda/mdacollections/mdaCollectionsEasyUIController/toSolrCoreManage/'+solrcoreID+'.';
    //	document.getElementById("ifrm").src= URL;
	  	var indexDeta = new CommonDialog("insert","1200","600",URL,"搜索数据详情",false,true,false);
	  	indexDeta.show();
    }	
...
<div id="f1" name="f1"  modelAttribute="search" >
			...
			<tr align="center">
				<td align="center">
				  <div class="row" style="width:550px;left:6%; margin:0 0 0 0; position:absolute; top:5%;" >
						<div >
					      <input id="sousuo" name="word" class="easyui-validatebox" style= "width: 450px;height: 30px;float: left;" value="${searchwords}" placeholder="请输入关键字。。。" type="text" >
					      <span >
					        <button id="subok" style= "height: 34px;float: left;background-color:#1ab394;border: none;" type="button"  onclick="ok_onclick('${searchResult.start}')"><span><font style="margin:5px;color: #f9f9f9;" >搜索一下</font></font></font></span></button>
					      </span>
					    </div>
					</div>
				</td>
	...
	<div style="clear:both"></div>
	 ...
	 <table style="margin-left:75px;margin-top:35px;padding:13px 0 3px 4px;font-size:15px;">
...
$(document).ready(function () {
	  //document.getElementById("subok").style.background-color="red";
