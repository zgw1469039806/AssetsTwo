 ...
 * 
 */
function MdaSearchconfig(datagrid,url,searchD,form){
...
//初始化操作
MdaSearchconfig.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getMdaSearchconfigsByPage.json"
		});
};
//添加页面
MdaSearchconfig.prototype.insert=function(){
	this.nData = new CommonDialog("insert","850","500",this.getUrl()+'Add/null',"添加",false,true,false);
	this.nData.show();
};
//编辑页面
MdaSearchconfig.prototype.modify=function(){
...
//详细页
MdaSearchconfig.prototype.detail=function(id){
	this.nData = new CommonDialog("edit","850","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
	this.nData.show();
};
//保存功能
MdaSearchconfig.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
MdaSearchconfig.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
MdaSearchconfig.prototype.reLoad=function(){
	this._datagrid.datagrid('load',{ param : JSON.stringify(serializeObject(this.searchForm))});
};
//关闭对话框
MdaSearchconfig.prototype.closeDialog=function(id){
	$(id).dialog('close');
};
//打开查询框
MdaSearchconfig.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
MdaSearchconfig.prototype.searchData =function(){
...
//隐藏查询框
MdaSearchconfig.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
MdaSearchconfig.prototype.clearData =function(){
...
MdaSearchconfig.prototype.format=function(value){
	if(value){
		if($.browser.msie){
...
MdaSearchconfig.prototype.formatDateTime=function(value){
	if(value){
		if($.browser.msie){
			if(typeof(value) == "number"){
...
MdaSearchconfig.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
    this._datagrid.datagrid('load',conditions);
};
MdaSearchconfig.prototype.status=[];
Platform6.fn.lookup.getLookupType('MDA_STATUS',function(r){r&&( MdaSearchconfig.prototype.status=r);});  
