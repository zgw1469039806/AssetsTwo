         ...
         <table class="form_commonTable" id="commontable1" style="height: 100%;">
          <tr>
            ...
            <c:if test="${!empty parseFields }">
              <c:forEach items="${parseFields}" var="fieldlist">
              <tr class="data">
                <c:forEach items="${fieldlist}" var="field">
                  <td style="border: 1px solid gray;">
                   <input  class="inputbox easyui-validatebox" type="text" value="${field }"  style="width: 100%;"  disabled name="parseFieldNameList" />
	             ...
	             <tr class="data">
	                 <td style="border: 1px solid gray;"><input readonly="readonly" value="${field}"  class="inputbox easyui-validatebox " type="text"  style="width: 100%;"  name="indexFieldNameList"  /></td>
	                	<c:if test="${field == 'CRAWLDATE'}">
	                	 <td style="border: 1px solid gray;"><input readonly="readonly" value="new Date().toLocaleString()"  class="inputbox easyui-validatebox " type="text"  style="width: 100%;"  name="indexFieldValueList"  /></td>
		              		 </c:if>	
	                	<c:if test="${field != 'CRAWLDATE'}">
...
<script type="text/javascript">
  ...
  function saveForm() {
    ...
    $.ajax({
      url:"platform/mdaDatasourceEasyUIController/operation/saveSetTow/db/${tmpID}",
      data : {
          ...
          "savetype":_saveType,"sourceId":sourceId},
      type : 'post',
      success : function(r){
        if (r == "success"){
          parent.closWinc();
            ...
            $('#classifyidsAlias').focus(
                    function() {
                      $("#crawlitem_classify").css(
                              "display", "block");
                      $.ajax({
                                url : "platform/mdaDatasourceEasyUIController/getZtree",
                                type : "POST",
                                success : function(
                                        retVal) {
                                  $.fn.zTree
