...
<script type="text/javascript">
	...
	$(function() {
		if (!"${mdaClassifyDTO.time}" == "") {
			$('#time').datebox(
					'setValue',
					parserColumnTime("${mdaClassifyDTO.time}").format(
							"yyyy-MM-dd"));
		}
	})
	function closeForm() {
		parent.mdaClassify.closeDialog("#modify");
	}
	function saveForm() {
	...
	<div data-options="region:'center',split:true,border:false">
		<form id='form' class='form'>
			<input type="hidden" name='id' id='id'
				value='<c:out  value='${mdaClassifyDTO.id}'/>' /> <input
				type="hidden" name="version"
				value='<c:out  value='${mdaClassifyDTO.version}'/>' />
			<table class="form_commonTable">
				<tr>
					...
					<td width="39%"><input title="NAME" class="easyui-validatebox"
						...
						style="width: 99%;" type="text" name="name" id="name"
						value='<c:out value='${mdaClassifyDTO.name}'/>' /></td>
					<th width="10%"
					...
					<td width="39%"><pt6:syslookup name="status" id="status"
							...
							defaultValue='${mdaClassifyDTO.status}'
				...
				<tr>
					...
					<td width="39%"><input title="TIME" class="easyui-datebox"
						...
						id="time" value='<c:out  value='${mdaClassifyDTO.time}'/>' /></td>
					<th width="10%"
						style="word-break: break-all; word-warp: break-word;">
					...
					<td width="39%"><input title="CLASS_INDEX_TREE_NO"
						...
						type="text" name="classIndexTreeNo" id="classIndexTreeNo"
						value='<c:out value='${mdaClassifyDTO.classIndexTreeNo}'/>' /></td>
				</tr>
				<tr>
				</tr>
			</table>
