	...
	_self.setting = {
			...
			callback: {
				onClick: function(event, treeId, treeNode){ //绑定左右联动事件
					clearFormData("#form");
					/*debugger;
					var treeHeight=$(".treeHeight").height()-10+"px";
					$("#_tree-center").css("height",treeHeight);*/
					$("#businessDomain").val("");
					if(treeNode.attributes.applicationCode){
						var keyWord = $("#monitorApiInfo_keyWord").val() == $("#monitorApiInfo_keyWord").attr(
	        			"placeholder") ? "" : $("#monitorApiInfo_keyWord").val();
						$("#appCode").val(treeNode.attributes.applicationCode);
						$("#appCode").attr("readonly","readonly");
						$("#appName").val(treeNode.text);
						$("#appName").attr("readonly","readonly");
	            		var param = {};
	            		param["appCode"] = treeNode.attributes.applicationCode;
	            		var  parentBusNode=treeNode.getParentNode();
	            	...
	            	}else if(treeNode.attributes.organizationCode=="root"){
	            		$("#appName").removeAttr("readonly");
	            		$("#appCode").removeAttr("readonly");
	            		chosenBusDomain="root";
	            		var keyWord = $("#monitorApiInfo_keyWord").val() == $("#monitorApiInfo_keyWord").attr(
	        			"placeholder") ? "" : $("#monitorApiInfo_keyWord").val();
	            		...
	            		$("#appName").removeAttr("readonly");
	            		$("#appCode").removeAttr("readonly");
	            		var chosenTreeNodeBus = treeNode.attributes.organizationCode;
	            		chosenBusDomain=treeNode.attributes.organizationCode;
	            		var keyWord = $("#monitorApiInfo_keyWord").val() == $("#monitorApiInfo_keyWord").attr(
	        			"placeholder") ? "" : $("#monitorApiInfo_keyWord").val();
	            		
	            		var param = {};
	            		param["businessDomain"] = chosenTreeNodeBus;
	            		param["appCode"] = "";
	            		param["apiName"]=keyWord;
		            	var searchdata = {
		            		keyWord : JSON.stringify(param),
		            	...
		            	$("#businessDomain").val(chosenTreeNodeBus);
	            	}
			    },
	    ...
	    $.get("apicenter/apiorganization/apiOrganizationController/gettree/2",{},function(r){
	    	_self.tree = $.fn.zTree.init($(_self._ztreeId),_self.setting,r);
	    	if(r && r.length > 0){
	    		var rootNode = r[0];
...
//查询框查询
MonitorOrganization.prototype.onseach = function(ecode, value){
	var _self = this;
	if(ecode == 13){
...
//表单加载json对象数据
MonitorOrganization.prototype.setForm = function (formObj, jsonValue) {
        var obj = formObj;
...
//添加平级节点
MonitorOrganization.prototype.insert = function(){
	    var _self = this;
...
//添加子节点
MonitorOrganization.prototype.insertSub = function(){
	var _self = this;
...
//编辑节点
MonitorOrganization.prototype.edit = function(flag){
	var _self = this;
	var selectedNodes = _self.tree.getSelectedNodes();
	if(flag=='1'){
...
//保存表单
MonitorOrganization.prototype.save = function(){
	    var _self = this;
		var selectedNodes = _self.tree.getSelectedNodes();
		if(null == selectedNodes[0] || undefined == selectedNodes[0]){
			layer.alert('请选择节点后进行操作', {
...
//删除节点
MonitorOrganization.prototype.del=function(){
	  var _self = this;
	  var selectedNodes = _self.tree.getSelectedNodes();
	   if(selectedNodes[0]._parentId=== '-1'){
	        layer.alert('不能删除根节点！', {
...
//清除表单数据
MonitorOrganization.prototype.clearData = function(form){
	clearFormData(form);
}

//添加根节点(如果需要修改根节点数据通过前台页面设置)
MonitorOrganization.prototype.insertRoot = function() {
	var _self = this; 
	avicAjax.ajax({
...
MonitorOrganization.prototype.formvaliad = function(){
    var _self = this;
