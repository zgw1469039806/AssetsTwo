...
plupload.addFileFilter('mime_types', function(filters, file, cb) {
	if (filters.length && !filters.regexp.test(file.name)) {
		this.trigger('Error', {
			...
			message : plupload.translate('文件扩展名错误'),
			file : file
		});
		cb(false);
	} else {
...
plupload.addFileFilter('max_file_size', function(maxSize, file, cb) {
	...
	if (file.size !== undef && maxSize && file.size > maxSize) {
		this.trigger('Error', {
			...
			message : plupload.translate('文件大小错误'),
			file : file
		});
		cb(false);
	} else {
		...
		while (ii--) {
			...
			if (file.name === this.files[ii].name && file.size === this.files[ii].size) {
				this.trigger('Error', {
					...
					message : plupload.translate('文件选择重复'),
					file : file
				});
				cb(false);
				return;
			...
			initControls.call(self, settings, function(inited) {
				if (inited) {
					...
					self.trigger('Error', {
						...
						message : plupload.translate('初始化错误')
					});
				}
			});
		}
	...
	function onUploadFile(up, file) {
		...
		if (file.name != null) {
			...
			$.get(baseurl+"/ckeck", {
				...
				parentRegisterId:form_id,
				parentTableCode:form_code
			}, function(data) {
				if (!isNaN(data.off))
					offset = data.off;
				else
		...
		function handleError() {
			if (retries-- > 0) {
				...
				up.trigger('Error', {
					...
					message : plupload.translate('HTTP连接错误'),
					file : file,
