 ...
 * @returns
 */
function MdaClassify(ztreeId, url, form, searchD, searchbtn){
...
//查询框查询
MdaClassify.prototype.onseach = function(ecode, value){
	var _self = this;
	if(ecode == 13){
		if(value == null||value == ""){
			_self.tree = $.fn.zTree.init($(_self._ztreeId),_self.setting, []);
...
//保存表单
MdaClassify.prototype.save = function(){
	    var _self = this;
		var selectedNodes = _self.tree.getSelectedNodes();
		if(null == selectedNodes[0] || undefined == selectedNodes[0]){
			layer.alert('请选择节点后进行操作', {
		...
		avicAjax.ajax({
			 url:_self.getUrl()+'/saveMdaClassify',
...
MdaClassify.prototype.formvaliad = function(){
    var _self = this;
