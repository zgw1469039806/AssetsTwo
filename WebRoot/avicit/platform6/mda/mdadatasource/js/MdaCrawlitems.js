function MdaCrawlitems(datagrid, url, formSub, dataGridColModel,searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord){
...
MdaCrawlitems.prototype.init=function(pid){
	...
	$(_self._datagridId).jqGrid({
    	url: this.getUrl()+'getMdaCrawlitems',
...
//添加页面
MdaCrawlitems.prototype.setFun=function(pid){
	if(this.pid == null || this.pid == ""){
	   layer.alert('请选择一条主表数据！', {
				  icon: 7,
				  area: ['400px', ''], //宽高
...
//重载数据
MdaCrawlitems.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
...
//打开高级查询框
MdaCrawlitems.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
	//if(!par) par = $(window);
