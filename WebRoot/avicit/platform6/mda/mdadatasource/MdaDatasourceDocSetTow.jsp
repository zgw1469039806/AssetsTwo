				...
				<tr>
					...
					<td colspan="2">
					   <table class="form_commonTable" id="commontable1" style="height: 100%;">
					    <tr>
					       <th width="10%" style="border: 1px solid gray;text-align: center;"><label for="name">文件名称:</label></th>
					       <th width="10%" style="border: 1px solid gray;text-align: center;"><label for="name">文件路径:</label></th>
					       <th width="10%" style="border: 1px solid gray;text-align: center;"><label for="name">文件内容:</label></th>
					       <th width="10%" style="border: 1px solid gray;text-align: center;"><label for="name">文件作者:</label></th>
					       <th width="10%" style="border: 1px solid gray;text-align: center;"><label for="name">文件最后修改时间:</label></th>
                        </tr>
					      <c:if test="${!empty parseFieldsArr }">
					         <tr class="data">
				             		<c:forEach items="${parseFieldsArr }" var="field">
				                		 <td style="border: 1px solid gray;"><input readonly="readonly" value="${field}"  class="form-control input-sm " type="text"  style="width: 100%;"  name="parseFieldNameList"  /></td>
			                    	 	 <input  class="form-control input-sm" type="hidden" value=" "  style="width: 100%;"  name="parseFieldValueList"  />
				              		</c:forEach>
					          </tr>
			             </c:if>
					   </table>
...
<%-- 	    <tr>
					...
					<td colspan="2">
					   <table class="form_commonTable" id="commontable1" style="height: 100%;">
					    <tr>
					       ...
					       <th width="10%" style="border: 1px solid gray;text-align: center;"><label for="name">正则表达式:</label></th>
					     ...
					     <%--   <c:if test="${!empty parseFields }">
					      ...
					      <c:if test="${empty indexFields }">
					       <c:if test="${!empty fieldsArr }">
					             ...
					             <tr class="data">
					                 <td style="border: 1px solid gray;"><input readonly="readonly" value="${field}"  class="form-control input-sm " type="text"  style="width: 100%;"  name="indexFieldNameList"  /></td>
					         		 <c:if test="${field == 'CRAWLDATE'}">
					                	 <td style="border: 1px solid gray;"><input readonly="readonly" value="new Date().toLocaleString()" class="form-control input-sm " type="text"  style="width: 100%;"  name="indexFieldValueList"  /></td>
					              		 </c:if>	
					         		 <c:if test="${field != 'CRAWLDATE'}">
	...
	<script type="text/javascript">
		...
		function saveForm() {
			var form=$("#form");
			var cols="",indexs="",dateType="";
// 			$("#commontable1").find(".data").each(function(){
// 				var _name = $(this).find("td").eq(0).find("input").val();
// 				var _value = $(this).find("td").eq(1).find("input").val();
// 				if(_name.length >0 && _value.length >0){
// 					if(cols.length==0){
// 						cols=_name+","+_value;
// 					}else{
// 						cols=cols+"#"+_name+","+_value;
// 					}
// 				}
// 			});
// 			$("#commontable2").find(".data").each(function(){
// 				var _name = $(this).find("td").eq(0).find("input").val();
// 				var _value = $(this).find("td").eq(1).find("input").val();
// 				if(_name.length >0 && _value.length >0){
// 					if(indexs.length==0){
...
// 			});
            ...
            $("input[name='parseFieldValueList']").each(function(){
                var _value=$(this).val();
                if(_value.length > 0) {
                	var result = _value.replace(/,/g,'@');
                    parseFieldValueList.push(result);
                 }else {
                     layer.alert('字段值不能为空',{
                         icon: 7,
                         area: ['400px', ''], //宽高
			...
			var type="2";
			$.ajax({
				 url:"platform/platform6/mda/mdadatasource/mdaDatasourceController/operation/saveSetTow/doc/${tmpID}",
				 data : {"savetype":_saveType,"sourceId":sourceId,"type":type,"dateType":dateType,
