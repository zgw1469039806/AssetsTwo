	...
	$.fn.changeui = function(opt) {
		...
		function changeCss(uiName){
			...
			if (uiName.indexOf('.css') < 0) {
				mainSrc = uiName + '/index/style.css?v='+new Date().getTime();
				childSrc = uiName +'/skin/style.css?v='+new Date().getTime();
			}else{
				mainSrc = uiName;
				childSrc = uiName;
			}
			if(!cssDom) return;
			cssDom.attr('href', option.url + mainSrc);
			// 保存样式cookie
			...
			if(option.childDom){
				$(option.childDom).find('iframe').each(function(){
					...
					iCssDom.attr('href', option.url + childSrc);
				});
			}
		}
