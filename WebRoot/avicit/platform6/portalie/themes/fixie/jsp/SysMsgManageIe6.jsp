		  	                        ...
		  	                        <option value="send">我的发送</option>
   									...
   									<div class="ext-selector-div">
										<input
											...
											title="请输入标题" id="keyWord"> <span id="searchPart"
											class="ext-input-right-icon ext-icon search"></span>
									</div>
									<!-- <div class="ext-selector-div">
										<input
											class="easyui-validatebox ext-selector-input keySearchClass"
				...
				<td width="39%">
					<div class="ext-selector-div">
						<input type="hidden" id="sendUser" name="sendUser"> <input
							class="easyui-validatebox ext-selector-input" placeholder="请选择用户"  title="请选择用户"
							id="sendUserAlias" name="sendUserAlias" readonly/>
							 <span class="ext-input-right-icon icon-all-file ext-input-right-icon-from" id="userbtn"></span> 
						</span>
					</div>
				</td>
				...
				<td width="39%">
					<div class="ext-selector-div">
						<input type="hidden" id="sendDeptid" name="sendDeptid"> <input
							class="easyui-validatebox" placeholder="请选择部门" 
							id="sendDeptidAlias" name="sendDeptidAlias">
							 <span class="ext-input-right-icon icon-all-file ext-input-right-icon-from" id="deptbtn"> </span> 
					</div>
				</td>
			</tr>
			<tr>
...
<script type="text/javascript"
	src="avicit/platform6/portal/themes/fixie/js/SysMsgIe6.js"></script>
...
function formatReadValue(cellvalue, options, rowObject) {
	...
	$("#readButton").on("click", function() {	
			...
			$.messager.confirm('提示','确定要标记已读吗?',function(b){
				if(b){
					for (var i = 0; i < rows.length; i++) {
						var rowData = rows[i];
						var sys_msg= {
	...
	$("#unreadButton").on("click", function() {	
			...
			$.messager.confirm('提示','确定要标记未读吗?',function(b){
				if(b){
					for (var i = 0; i < rows.length; i++) {
						var rowData = rows[i];
						var sys_msg= {
