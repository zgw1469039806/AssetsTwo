 ...
 * 打开配置页面
 */
var _userDefineColumnDialog;
function userDefineColumn(tableId,width,height,title) {
	width = width || "550";
	height = height || "350";
	title = title || "自定义显示列";
	userDefineColumnFieldsOptions = getGridAllColumnFieldsOptions(tableId);
	var url = "avicit/platform6/commonie/fixie/userDefinedColumn.jsp?dialogId=userDefineColumnDialog&tableId=" + tableId;	
	_userDefineColumnDialog = openDialog({opentype: 2, width: width, height: height, title:title, url:url});
}

function colseUserDefineColumnDialog(){
	layer.close(_userDefineColumnDialog);
}
...
function dealWrapResult(wrapResult,tableId){
	if(wrapResult && (wrapResult.columns || wrapResult.frozenColumns)){
		$.ajax({
			...
			data : {tableName : userDefinePathName + "/" + tableId},
			type : 'post',
			dataType : 'json',
			async:false,
			success : function(r){
...
function saveUserdefinedColumnInfo(tableId) {
    ...
    $.ajax({
        ...
        data: {
        	tableName : userDefinePathName + "/" + tableId,columns : columnArrayJson
        },
        type: 'post',
        dataType: 'json',
        success: function(r) {
            if (r.flag == "success") {
            	layer.alert('自定义表格信息保存成功！', {
	    	    		icon: 7,
	    	    		area: ['400px', ''], //宽高
	    	    		closeBtn: 0,
	    	    		btn: ['关闭'],
...
function deleteUserdefinedColumnInfo(tableId) {
	layer.confirm('确定要删除当前表格的配置信息吗?',  {icon: 3, title:"提示", area: ['400px', '']}, function(index){
		 if(index){
			 $.ajax({
			        ...
			        success: function(r) {
			            if (r.flag == "success") {
			            	layer.confirm('要刷新页面才能看到效果，现在刷新页面吗？',  {icon: 3, title:"提示", area: ['400px', '']}, function(c){
			    				 if(c){
...
function exportExcelServerData(tableId) {
		layer.confirm('是否确认导出Excel文件?',  {icon: 3, title:"提示", area: ['400px', '']}, function(index){
			 if(index){
			// 封装参数
			userDefineColumnFieldsOptions = getGridAllColumnFieldsOptions(tableId);
			var columnArray = new Array();
