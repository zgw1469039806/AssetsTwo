			...
			if($("#workflowSelectorSource").length > 0 ){
				...
				if(conditions.isBranch()){//多个分支时
					for(var i = 0 ; i < getDatas.getBranchLength() ; i++){
						...
						drawSecondLayerTabs(dataJson,dataJson.nextTask[branchNo],secondLayerTabs,branchNo);//绘制第二层
					...
					var branchNo = 0;
					//drawSecondLayerTabs(dataJson.nextTask[branchNo],secondLayerTabs,branchNo);//绘制第二层
					drawSecondLayerTabs(dataJson,dataJson.nextTask[0],tabs,branchNo);
					drawSecondLayerTabsForTargetDataGrid('selectorUserTabForTargetDataGrid_' + branchNo,branchNo);
					//选中第一标签页
					tabs.tabs('select',0);
				}
 ...
 * 绘制第二层
 */
function drawSecondLayerTabs(taskUserSelect,data,tabs,branchNo){
...
/*	if(data.roleList != null && data.roleList.length > 0){//绘制角色标签页
	...
	if(data.roleList != null && data.roleList.length > 0){//绘制自定义角色标签页
		drawer.drawTab(tabs,'tabs_roleTree_' + branchNo , '角色' ,'roleTree_' + branchNo , 'icon-role');
		drawer.drawCustomRoleTabContent(taskUserSelect,data,data.roleList,'roleTree_' + branchNo,tabNo++);//绘制角色标签页内容
	}
...
//	    			$('#selectorUserTabForTargetDataGrid_' + branchNo).datagrid('appendRow',record);	    		
	 ...
	 * 绘制自定义角色标签页
	 */
	this.drawCustomRoleTabContent = function(taskUserSelect,nextActivity,data,objectId,tabNo){
		var datas = [];
		var roleIds = '';
		for(var i = 0 ; i< data.length ; i++){
			if(i==(data.length-1)){
		...
		$.ajax({
			...
			success: function(treeList){
				if(treeList && treeList.length){
					...
					createComponent.createTreeComponent(objectId,treeList,'',beforeExpandUrl,tabNo,'role',nextActivity);
					var paramDeptIds = "";
...
//		    		processSelectUserComponentEvent.refreshSelectedTree(branchNo);
				} else if(node.attributes.nodeType == 'position' || node.attributes.nodeType == 'group'){
