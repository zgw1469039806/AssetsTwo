; (function ($, window, document, undefined) {
    ...
    //样式插件函数
    $.fn.myStyle=function(option){//this代指$()
        //默认的样式
        var defaults={
            "background":"#2b2b2b"
        }
...
function TimeLineExt(options){
	...
	if(this.data){
        ...
        for(var i=0; i<this.data.length;i++){
        	dom = "<dl><dt>" + this.data[i].groupName + "</dt>";
        	for(var k = 0; k < this.data[i].datas.length; k++){
       			var template = this.template().concat();
	        	for(var l = 0; l < this.data[i].datas[k].length; l++){
	            	template = template.replaceAll("#" + l + "#",this.data[i].datas[k][l]);
	        	}
	        	dom = dom + template;
        	}
        	dom = dom +"</dl>";
