                    ...
                    if(ts.p.pgbuttons===true) {
                        ...
                        if(ts.p.hasColSet){
                            pgl += "<td id='"+po[4]+"' class='"+btc+"' title='"+ pot[4] +"'" + "><span " + getstyle(stylemodule, 'icon_set', false, iconbase)+"></span>"+
                                "</td><td class='jq-btn-menu'><div class='row submenu' style='z-index:5;'></div></td>";
                        }
                    ...
                    if(ts.p.pgbuttons===true) {
                        ...
                        $("#set"+$.jgrid.jqID(tp)).click( function(event) {
                            ...
                            subMenuDom.append(
                                ...
                                "<div style='padding: 0 6px;box-sizing: border-box;' class='col-xs-7'>列名称</div><div style='padding: 0 6px;box-sizing: border-box;' class='col-xs-2'>隐藏</div>" +
                                "<div style='padding: 0 25px;box-sizing: border-box;text-align: center;' class='col-xs-3'>排序</div></div>"
                            );

                            var template = '<div style="border-bottom: 1px solid #e8e8e8;background: #ffffff;" class="col-xs-12 userviewkads" id=\"kads_#1#\">'+
                                '<div class="col-xs-7" style="padding: 0 6px;box-sizing: border-box;overflow: hidden; white-space: nowrap; text-overflow: ellipsis;word-break: break-all;">#0#</div><div style="padding: 0 9px;box-sizing: border-box;" class="col-xs-2"><input id="#1#"   class="' + jqGridID +
                                'columnSetting#6#" data=\'{\"columnLabel\":\"#0#\", \"columnField\": \"#1#\", \"columnShow\":#2#, \"columnWidth\":\"#3#\"}\' type="checkbox"  #4#  style="#5#"></div><div style="padding: 0 25px;box-sizing: border-box;" class="col-xs-3"><i style="margin-right: 2px;" class="icon icon-layer-up iconLayerSort"></i><i class="icon icon-layer-down iconLayerSort"></i><div>'+
                                '</div>';
                            for(var i = 0; i < cm.length; i++ ){
                                ...
                                subMenuDom.append(temp);
                            }

                            subMenuDom.append(
                                "<div class='col-xs-12 toolbar-right' style='padding: 4px 0;'>" +
                                "<a style='float: right;' id='" + jqGridID + "_COL_CLOSE' href='javascript:void(0)' class='btn btn-primary form-tool-btn btn-sm' role='button' title='关闭'><i class='glyphicon glyphicon-remove-sign'></i> 关闭</a>" +
                                "<a style='float: right;' id='" + jqGridID + "_COL_CLEAR' href='javascript:void(0)' class='btn btn-primary form-tool-btn btn-sm' role='button' title='取消选中'><i class='glyphicon glyphicon-minus-sign'></i> 取消选中</a>" +
                                "<a style='float: right;' id='" + jqGridID + "_COL_SAVE' href='javascript:void(0)' class='btn btn-primary form-tool-btn btn-sm btn-point' role='button' title='保存'><i class='glyphicon glyphicon-ok-sign'></i> 保存</a></div>"
                            );
                            ...
                            $('.icon-layer-up').bind('click',function(){
                                var $this = $(this);
                                var curTr = $this.parents('.col-xs-12');
                                var prevTr = $this.parents('.col-xs-12').prev();
                                if(prevTr[0]["className"] == 'col-xs-12 alert-info userviewkads' || prevTr.length == 0){
                                    layer.msg('已经是第一行！');
                                    return;
                                }else{
                                    prevTr.before(curTr);
                            ...
                            $('.icon-layer-down').bind('click',function(){
                                var $this = $(this);
                                var curTr = $this.parents(".col-xs-12");
                                var nextTr = $this.parents(".col-xs-12").next();
                                if(nextTr[0]["className"] == 'col-xs-12 toolbar-right' || nextTr.length == 0){
                                    layer.msg('已经是最后一行！');
                                    return;
                                }else {
                                    nextTr.after(curTr);
                            ...
                            $("#"+ jqGridID + "_COL_SAVE").click( function(event) {
                                ...
                                $(ts).jqGrid("saveOrEditDtoToRedis", {columnArrayJson: columnArrayJson,jqGridwidth: jqGridwidth});
