	...
	view = {
		...
		cancelPreSelectedNode: function (setting, node) {
			...
			for (var i=0, j=list.length-1; j>=i; j--) {
				if (!node || node === list[j]) {
					//console.log(1)
					$("#" + list[j].tId + consts.id.A).removeClass(consts.node.CURSELECTED).parent().removeClass('current');
					if (node) {
		...
		selectNode: function(setting, node, addFlag) {
			...
			// treeview样式补丁
			$("#" + node.tId + consts.id.A).addClass(consts.node.CURSELECTED).parent().addClass('current');
